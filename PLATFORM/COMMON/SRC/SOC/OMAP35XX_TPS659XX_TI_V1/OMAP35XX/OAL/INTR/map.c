    ...
    )
{
    ...
    for (i = 0; i < SYSINTR_MAXIMUM; i++)
        ...
        for (j = 0; j < OMAP_IRQ_PER_SYSINTR; j++)
            {
            s_oalSysIntr2Irq[i][j] = OAL_INTR_IRQ_UNDEFINED;
            }
        }
    for (i = 0; i < OMAP_IRQ_MAXIMUM; i++)
        {
        s_oalIrq2SysIntr[i] = SYSINTR_UNDEFINED;
        }
    ...
    )
{
    ...
    for (i = 0; i < count; i++)
        ...
        if (pIrqs[i] >= OMAP_IRQ_MAXIMUM)
            ...
            for (j = 0; j < i; j++)
                {
                s_oalSysIntr2Irq[sysIntr][j] = OAL_INTR_IRQ_UNDEFINED;
                }
            sysIntr = SYSINTR_UNDEFINED;
            goto cleanUp;
            }
    ...
    while (i < OMAP_IRQ_PER_SYSINTR)
        {
        s_oalSysIntr2Irq[sysIntr][i++] = OAL_INTR_IRQ_UNDEFINED;
        }
    ...
    )
{
    ...
    for (i = 0; i < OMAP_IRQ_PER_SYSINTR; i++)
        ...
        if (s_oalIrq2SysIntr[irq] == sysIntr)
            {
            s_oalIrq2SysIntr[irq] = SYSINTR_UNDEFINED;
            }
        }
