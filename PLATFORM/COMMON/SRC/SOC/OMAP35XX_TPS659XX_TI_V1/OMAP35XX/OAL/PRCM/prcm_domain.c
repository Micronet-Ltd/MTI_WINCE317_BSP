    ...
    )
{
  ...
  //##### Michael, Test print
	if(powerDomain >= POWERDOMAIN_COUNT) 
	  return(FALSE);
...
BOOL DomainInitialize()
{
	...
	for (i = 0; i < POWERDOMAIN_COUNT; ++i)
	{
		...
		if(!pCmRegs || !pPrmRegs)
		{
			RETAILMSG(0, (_T("DomainInitialize: Fatal error %x %x\r\n"), pCmRegs, pPrmRegs));
			continue;
		}
    ...
    )
{
    ...
    for (i = 0; i < pClockStates->count; ++i)
        {
        if (pClockStates->rgClockDomains[i].clockDomain == clockDomain)
            {
