...
void DumpPRCM();
void SetAutoIdleMode(BOOL enable);
    ...
    )
{
    ...
    if (WaitForSingleObject(pDevice->hIntrEvent, 
		...
		RETAILMSG(1, (L"ERROR: TransmitCommand: "
            L"Timeout in Tx\r\n"
            ));
...
//	RETAILMSG(1, (L"TransmitCommand: *pStatus = 0x%02X\r\n", *pStatus));
...
cleanUp:
    DEBUGMSG(ZONE_FUNCTION, (L"-TransmitCommand(cmd=0x%02X)\r\n", cmd));
	//RETAILMSG(1, (L"-TransmitCommand(cmd=0x%02X, rc = 0x%x)\r\n", cmd,rc));
    return rc;
}
    ...
    )
{
	...
	/*RETAILMSG(1, (L"+HDQ_Init(%s, 0x%08x)\r\n", 
        szContext, pBusContext
        ));
		*/
    ...
    )
{
    ...
    case IOCTL_CONTEXT_RESTORE:
			...
			//RETAILMSG(1, (L"HDQ_IOControl: IOCTL_CONTEXT_RESTORE_NOTIFY\r\n"));
...
/*	RETAILMSG(1, (L"+HDQ_Write(0x%08x, 0x%02x, 0x%x)\r\n",
        context, pBuffer, size
        ));
*/
    // Check if we get correct context
    ...
    if (TransmitCommand(pInstance, pData[0] | HDQ_WRITE_CMD, &status) == FALSE)
        {
		RETAILMSG(1, (L"HDQ_Write: TransmitCommand failed status = %x\r\n", status));
        goto cleanUp;
        }
...
//	DumpPRCM();
    ...
    if (TransmitCommand(pInstance, pData[0], &status) == FALSE)
        {
		RETAILMSG(1, (L"ERROR:TransmitCommand failed \r\n"
                    ));
...
void DumpPRCM()
{
    ...
    // Map Power, Reset, and Clock Manager (PRCM) Registers
	 ...
	 UINT32 reg;
	 reg = INREG32(&pPRCMRegs->CM_AUTOIDLE1_CORE);
     reg&=~0x00400000;
//	 OUTREG32(&pPRCMRegs->CM_AUTOIDLE1_CORE,reg);
	 reg = INREG32(&pPRCMRegs->CM_FCLKEN1_CORE);
	 reg|=0x00400000;
//	 OUTREG32(&pPRCMRegs->CM_FCLKEN1_CORE,reg);
	 reg = INREG32(&pPRCMRegs->CM_ICLKEN1_CORE);
	 reg|=0x00400000;
//	 OUTREG32(&pPRCMRegs->CM_ICLKEN1_CORE,reg);

     RETAILMSG(1, (L"   CM_AUTOIDLE1_CORE   = 0x%08X\r\n",INREG32(&pPRCMRegs->CM_AUTOIDLE1_CORE)   ));
     RETAILMSG(1, (L"   CM_FCLKEN1_CORE     = 0x%08X\r\n", INREG32(&pPRCMRegs->CM_FCLKEN1_CORE)  ));
     RETAILMSG(1, (L"   CM_ICLKEN1_CORE     = 0x%08X\r\n", INREG32(&pPRCMRegs->CM_ICLKEN1_CORE)   ));
     RETAILMSG(1, (L"   CM_IDLEST1_CORE     = 0x%08X\r\n", INREG32(&pPRCMRegs->CM_IDLEST1_CORE)   ));
	 RETAILMSG(1, (L"   CM_CLKSTST_CORE     = 0x%08X\r\n", INREG32(&pPRCMRegs->CM_CLKSTST_CORE)   ));
	 RETAILMSG(1, (L"   CM_CLKSTCTRL_CORE   = 0x%08X\r\n", INREG32(&pPRCMRegs->CM_CLKSTCTRL_CORE)   ));
...
void SetAutoIdleMode(BOOL enable)
{
    ...
    // Map Power, Reset, and Clock Manager (PRCM) Registers
	 ...
	 if(enable)
		 ...
		 reg&=~0x00400000;
	 OUTREG32(&pPRCMRegs->CM_AUTOIDLE1_CORE,reg);
	 RETAILMSG(0, (L"   CM_AUTOIDLE1_CORE   = 0x%08X, enable = %d \r\n",INREG32(&pPRCMRegs->CM_AUTOIDLE1_CORE),enable   ));
}
