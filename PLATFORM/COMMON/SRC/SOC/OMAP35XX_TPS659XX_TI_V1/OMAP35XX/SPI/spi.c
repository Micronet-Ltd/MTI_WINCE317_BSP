...
DWORD SPI_Init(LPCTSTR szContext, LPCVOID pBusContext)
{
    ...
    switch(pDevice->memBase[0])
    {
      case OMAP_MCSPI1_REGS_PA:
           pDevice->dwPort = SPIPORT_1;
           break;
      case OMAP_MCSPI2_REGS_PA:
           pDevice->dwPort = SPIPORT_2;
           break;
      case OMAP_MCSPI3_REGS_PA:
           pDevice->dwPort = SPIPORT_3;
           break;
      case OMAP_MCSPI4_REGS_PA:
           pDevice->dwPort = SPIPORT_4;
           break;
    }
    ...
    )
{
    ...
    DEBUGMSG(ZONE_FUNCTION, (L"+SPI_WriteReadCS(0x%08x, In = %d, Out = %d)\r\n", context, inSize, outSize));
	...
	RETAILMSG(0, (L"\r\n+SPI_WriteReadCS CHCONF = 0x%08X (0x%08X)\r\n", INREG32(&pSPIChannelRegs->MCSPI_CHCONF), configOrig ) );
	    ...
	    // Restore after ???
	moduleCtrlOrig = INREG32(&pDevice->pSPIRegs->MCSPI_MODULCTRL);
	RETAILMSG(0, (L"+SPI_WriteReadCS MODULCTRL = 0x%08X (0x%08X)\r\n", INREG32(&pDevice->pSPIRegs->MCSPI_MODULCTRL), moduleCtrlOrig ));
	...
	if( outSize > 0 )
	{
     ...
     //SETREG32(&pInstance->pSPIChannelRegs->MCSPI_CHCONF, MCSPI_CHCONF_FORCE | MCSPI_CHCONF_TRM_TXONLY );
	}
	...
	while( bytesCnt > 0 )
	{
		...
		{
		  //SETREG32(&pInstance->pSPIChannelRegs->MCSPI_CHCONF, MCSPI_CHCONF_WL(16) );
		...
		{
		  //SETREG32(&pInstance->pSPIChannelRegs->MCSPI_CHCONF, MCSPI_CHCONF_WL(8) );
        ...
        if( dwWait == 0 )
        {
			...
			// Restore registers
			OUTREG32(&pDevice->pSPIRegs->MCSPI_MODULCTRL, moduleCtrlOrig);
			OUTREG32(&pInstance->pSPIChannelRegs->MCSPI_CHCONF, configOrig); 
	...
	while( bytesCnt > 0 )
    {
        ...
        if( dwWait == 0 )
        {
            DEBUGMSG(ZONE_ERROR, (L"SPI_WriteReadCS read timeout\r\n"));
...
BOOL SPI_IOControl(
    ...
    case IOCTL_DDK_GET_DRIVER_IFC:
        ...
        if (IsEqualGUID(pInBuffer, &DEVICE_IFC_SPI_GUID)) {
			...
			ifc.pfnWriteReadCS = SPI_WriteReadCS; // ##### Michael
    ...
    // ##### Michael
	case IOCTL_SPI_WRITEREAD_CS:
        if (pOutSize != NULL) *pOutSize = inSize;
        if (pInBuffer == NULL || pOutBuffer == NULL || ((inSize == 0) && (outSize == 0)) ) 
		{
            SetLastError(ERROR_INVALID_PARAMETER);
            break;
        }

        bRetVal = SPI_WriteReadCS(context, inSize, pInBuffer, outSize, pOutBuffer);
        break;
