...
{
    DWORD rc = -1;
    if (devId < OMAP_DEVICE_GENERIC && s_fnPreDeviceStateChange != NULL)
        {
...
{
    DWORD rc = -1;
    if (devId < OMAP_DEVICE_GENERIC && s_fnPostDeviceStateChange != NULL)
        {
    ...
    )
{
    ...
    // dvfs policy manager
    if (pInfo->devId != OMAP_DEVICE_GENERIC)
        {
    ...
    )
{
    ...
    // from the collection
    //
    if (pChild->DriverId() == OMAP_DEVICE_GENERIC)
        {
