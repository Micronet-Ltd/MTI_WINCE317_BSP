    ...
    )
{
    ...
    switch(pContext->IrqNum)
        {
        case IRQ_SDMA0:
            pContext->pMaskRegister = &pContext->pDmaRegs->DMA4_IRQENABLE_L0;
            pContext->pStatusRegister = &pContext->pDmaRegs->DMA4_IRQSTATUS_L0;
            break;

        case IRQ_SDMA1:
            pContext->pMaskRegister = &pContext->pDmaRegs->DMA4_IRQENABLE_L1;
            pContext->pStatusRegister = &pContext->pDmaRegs->DMA4_IRQSTATUS_L1;
            break;

        case IRQ_SDMA2:
            pContext->pMaskRegister = &pContext->pDmaRegs->DMA4_IRQENABLE_L2;
            pContext->pStatusRegister = &pContext->pDmaRegs->DMA4_IRQSTATUS_L2;
            break;

        case IRQ_SDMA3:
            pContext->pMaskRegister = &pContext->pDmaRegs->DMA4_IRQENABLE_L3;
            pContext->pStatusRegister = &pContext->pDmaRegs->DMA4_IRQSTATUS_L3;
            break;

        default:
            ERRORMSG(TRUE, (
                        L"DmaDiAllocateInterrupt: IrqNum %d not known\r\n"
                        ,pContext->IrqNum
                       ));
            goto cleanUp;
        }
