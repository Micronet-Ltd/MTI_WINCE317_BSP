    ...
    )
{
    ...
    // set RINTM
    //
    pDevice->shadowRegs.SPCR2 |= pDevice->pConfigInfo->RxSyncError;
    ...
    )
{
    ...
    // McBSP1,4,5 do not have sidetone support, so the memBase will be NULL
    if (pDevice->memBase[2] == NULL)
    {
        goto cleanUp;
    }
    ...
    // Map BSP controller
    //
    pa.QuadPart = pDevice->memBase[2];
    pSideToneRegs = (OMAP35XX_MCBSP_REGS_ST_t*)
        MmMapIoSpace(pa, pDevice->memLen[2], FALSE);
