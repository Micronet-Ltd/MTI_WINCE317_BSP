    ...
    )
{
    ...
    // Get the McBSP type
    //
    GetMcbspType(pDevice, pDevice->memBase[0]);
...
//------------------------------------------------------------------------------
//
//  Function:  GetMcbspType
//
//  get the mcbsp number based on the base address passed
//
void
GetMcbspType(
    McBSPDevice_t *pDevice,
    ...
    )
{
    ...
    switch (dwAddress)
        {
        case MCBSP1_BASE_ADDRESS:
            pDevice->mcbspType = kMcBSP1;
            break;

        case MCBSP2_BASE_ADDRESS:
            pDevice->mcbspType = kMcBSP2;
            break;

        case MCBSP3_BASE_ADDRESS:
            pDevice->mcbspType = kMcBSP3;
            break;

        case MCBSP4_BASE_ADDRESS:
            pDevice->mcbspType = kMcBSP4;
            break;

        case MCBSP5_BASE_ADDRESS:
            pDevice->mcbspType = kMcBSP5;
            break;

        default:
            DEBUGMSG(ZONE_ERROR, (L"ERROR: GetMcbspType: "
                L"Wrong mcbsp base address \r\n"
                ));
            break;
        }
...
//------------------------------------------------------------------------------
//
//  Function:  ConfigureDevConfForClks
//
//  Configure the CONTROL_DEVCONF0 register based on the need of CLKS from
//  either functional clock or MCBSP_CLKS pin.
//
void
ConfigureDevConfForClks(
    McBSPDevice_t *pDevice,
    BOOL bEnable
    )
{
    ...
    switch (pDevice->mcbspType)
        {
        case kMcBSP1:
        case kMcBSP2:
            pa.QuadPart = DEVCONF0_ADDRESS;
            break;

        case kMcBSP3:
        case kMcBSP4:
        case kMcBSP5:
            pa.QuadPart = DEVCONF1_ADDRESS;
            break;
    ...
    pAddress = (UINT32 *)MmMapIoSpace(pa, sizeof(DWORD), FALSE);
    if (pAddress == NULL)
        {
        DEBUGMSG(ZONE_ERROR, (L"ERROR: MCP:EnableClocks: "
            L"Failed map devconf1 register\r\n"
            ));
        goto cleanup;
        }
    ...
    MmUnmapIoSpace((void *)pAddress, sizeof(DWORD));
    ...
    )
{
    ...
    switch (cmd)
        ...
        case kExternalDrvrDx_Start:
            ...
            if ((pDevice->pMcbspProfile != NULL) &&
                (pDevice->pMcbspProfile->GetMode() != pDevice->mcbspProfile))
                {
