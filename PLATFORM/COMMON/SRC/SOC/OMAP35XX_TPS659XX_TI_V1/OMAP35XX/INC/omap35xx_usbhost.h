...
// Get USB device extended info from HC driver
#define IOCTL_GET_USB_DEVICE_EXT_INFO    \
    CTL_CODE(FILE_DEVICE_UNKNOWN, 0x410, METHOD_BUFFERED, FILE_ANY_ACCESS)

typedef struct tagUSB_DEVICE_EXT_INFO
{
    UINT8 Tier;
    UINT8 Port;
} USB_DEVICE_EXT_INFO;

BOOL HcdMdd_GetDeviceInfo(LPVOID lpvUhcdObject, UCHAR address, PUCHAR port, PUCHAR tier);
