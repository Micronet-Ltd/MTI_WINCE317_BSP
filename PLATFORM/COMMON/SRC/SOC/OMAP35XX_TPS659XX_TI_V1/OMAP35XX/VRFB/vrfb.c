...
static const DEVICE_REGISTRY_PARAM s_deviceRegParams[] = {
    {
        ...
        fieldsize(Device_t, dwNumDisplayContexts), (VOID*)4
    }
};
    ...
    )
{
    ...
    size = pDevice->memLen[0];
    pDevice->pVRFBRegs = (OMAP_VRFB_REGS*)MmMapIoSpace(pa, size, FALSE);
    ...
    )
{
    ...
    if (pDevice->pVRFBRegs != NULL) 
    {
        MmUnmapIoSpace((VOID*)pDevice->pVRFBRegs, pDevice->memLen[0]);
    }                    
