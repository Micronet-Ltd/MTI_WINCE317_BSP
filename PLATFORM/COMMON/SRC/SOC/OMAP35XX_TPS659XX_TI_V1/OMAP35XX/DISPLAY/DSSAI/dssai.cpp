...
LPCWSTR g_szRootBusDevice = (L"BUS1:");
...
OMAPDisplayController::~OMAPDisplayController()
{
    ...
    //  Release all clocks
    BusClockRelease( m_hRootBus, OMAP_DEVICE_DSS );
    BusClockRelease( m_hRootBus, OMAP_DEVICE_DSS1 );         
    BusClockRelease( m_hRootBus, OMAP_DEVICE_DSS2 );         
    BusClockRelease( m_hRootBus, OMAP_DEVICE_TVOUT );         
    ...
    if (m_pPrcmDssCMRegs != NULL) 
        MmUnmapIoSpace((VOID*)m_pPrcmDssCMRegs, sizeof(OMAP_PRCM_DSS_CM_REGS));
    ...
    if(m_pPrcmClkCM)
        MmUnmapIoSpace((VOID*)m_pPrcmClkCM, sizeof(OMAP_PRCM_CLOCK_CONTROL_CM_REGS));
    if(m_pPrcmClkPRM)
        MmUnmapIoSpace((VOID*)m_pPrcmClkPRM, sizeof(OMAP_PRCM_CLOCK_CONTROL_PRM_REGS));
}
...
OMAPDisplayController::InitController(BOOL bEnableGammaCorr, BOOL bEnableWaitForVerticalBlank)
{
    ...
    size = sizeof(OMAP_PRCM_CLOCK_CONTROL_CM_REGS);
    m_pPrcmClkCM = (OMAP_PRCM_CLOCK_CONTROL_CM_REGS *)MmMapIoSpace(pa, size, 0);
    if(!m_pPrcmClkCM)
	{
        RETAILMSG(1, (L"ERROR: OMAPDisplayController::InitController: Failed map PRCM_CLK_CTL registers\r\n"));
        goto cleanUp;
	}
    pa.QuadPart = OMAP_PRCM_CLOCK_CONTROL_PRM_REGS_PA;
    size = sizeof(OMAP_PRCM_CLOCK_CONTROL_PRM_REGS);
    m_pPrcmClkPRM = (OMAP_PRCM_CLOCK_CONTROL_PRM_REGS *)MmMapIoSpace(pa, size, 0);
    if(!m_pPrcmClkPRM)
	{
        RETAILMSG(1, (L"ERROR: OMAPDisplayController::InitController: Failed map PRCM_PRM_CTL registers\r\n"));
        goto cleanUp;
	}
    ...
    size = sizeof(OMAP_PRCM_DSS_CM_REGS);
    m_pPrcmDssCMRegs = (OMAP_PRCM_DSS_CM_REGS*)MmMapIoSpace(pa, size, FALSE);
    if (m_pPrcmDssCMRegs == NULL)
        {
        DEBUGMSG(ZONE_ERROR, (L"ERROR: OMAPDisplayController::InitController: "
             L"Failed map DSS CM registers\r\n"
            ));
        goto cleanUp;
        }
    ...
    //  Open handle to root bus power driver
    m_hRootBus = CreateFile(g_szRootBusDevice, 0, 0, NULL, 0, 0, 0 );
    if (m_hRootBus == NULL)
        {
        DEBUGMSG(ZONE_ERROR, (L"ERROR: OMAPDisplayController::InitController: "
             L"Failed open root bus driver handle\r\n"
            ));
        goto cleanUp;
        }
    ...
    // map DSI Pll regs
    pa.QuadPart = OMAP_DSI_PLL_REGS_PA;
    ...
    m_eDssFclkSource = OMAP_DSS_FCLK_DSS1ALWON;
    m_eDssFclkValue  = OMAP_DSS_FCLKVALUE_NORMAL;

/* MN changes */	ResetDISPC();
...
OMAPDisplayController::InitLCD()
{
    ...
    m_dwVsyncPeriod = 1000/60;
    
    //Initialize and Configure the DSIPLL.
    //Don't switch the clocks yet. The switch 
    //requires the LCD Panel to be in active. 
    //EnablePipeline will do a switch from DSS1 to DSS2
    // Request Rootbus to turn on DSS1 and DSS2 clocks
    count       = 2;
    clockSrc[0] = kDSS1_ALWON_FCLK;
    clockSrc[1] = kDSS2_ALWON_FCLK;
...
OMAPDisplayController::DssProcessInterrupt()
{
                     ...
                     DISPC_IRQSTATUS_SYNCLOSTDIGITAL))
    {
		...
		/* MN changes */RETAILMSG(1, (L"SYNCLOST DSSIRQ:%x\r\n\r\n", irqStatus));
    ...
    else
    {
        ...
        irqStatus &= ~(DISPC_IRQSTATUS_VSYNC | DISPC_IRQSTATUS_EVSYNC_EVEN | DISPC_IRQSTATUS_EVSYNC_ODD);
    }
    ...
    SETREG32( &m_pDispRegs->DISPC_IRQSTATUS, irqStatus);
...
//------------------------------------------------------------------------------
BOOL OMAPDisplayController::SetPowerLevel(DWORD dwPowerLevel, int hld_bkl)
{
    BOOL            bResult = TRUE;
    DWORD   dwTimeout;
	...
	RETAILMSG(0, (L"OMAPDisplayController::SetPowerLevel(D%d)\r\n", dwPowerLevel));
        ...
        case D2:
            ...
            if( m_dwPowerLevel == D3 || m_dwPowerLevel == D4 )
            {
                ...
                if( m_bTVEnable )
                {
                    ...
                    RequestClock( OMAP_DEVICE_TVOUT );
				...
				if(hld_bkl)
				{
					...
					//if(g_preq)
					//	ReleasePowerRequirement(g_preq);
					//g_preq = 0;
        ...
        case D4:
            ...
            if( m_dwPowerLevel == D0 || m_dwPowerLevel == D1 || m_dwPowerLevel == D2 )
            {
				if(hld_bkl)
				{
					//if(g_preq)
					//	ReleasePowerRequirement(g_preq);
					//g_preq = SetPowerRequirement(L"BKL1:", (CEDEVICE_POWER_STATE)dwPowerLevel, POWER_NAME, 0, 0);
					SetDevicePower(L"BKL1:", POWER_NAME, (CEDEVICE_POWER_STATE)dwPowerLevel);
				}
                //  Disable TV out
                if( g_dwDestinationRefCnt[OMAP_DSS_DESTINATION_TVOUT] > 0 )
                {
                    ...
                    //  Release the clock
                    ReleaseClock( OMAP_DEVICE_TVOUT );
                }
    ...
    )
{
    ...
    // Select DSS1 and DSS2 source clock for
    // doing a switch between DSI and DSS FCLKs
    count       = 2;
    clockSrc[0] = kDSS1_ALWON_FCLK;
    clockSrc[1] = kDSS2_ALWON_FCLK;
    SelectDSSSourceClocks( count, clockSrc);
    ...
    if( bEnable )
    {
        ...
        // Select DSS2 clock only 
        clockSrc[0] = kDSS2_ALWON_FCLK;
    ...
    else
    {
        ...
        // Select DSS1 clock alone and turn of DSS2
        clockSrc[0] = kDSS2_ALWON_FCLK;
    }
    ...
    // Select the active clock source
    count       = 1;
    SelectDSSSourceClocks( count, clockSrc);
...
OMAPDisplayController::ResetDSS()
{
    ...
    RequestClock( OMAP_DEVICE_DSS );         
    RequestClock( OMAP_DEVICE_DSS1 );
    RequestClock( OMAP_DEVICE_DSS2 );
    RequestClock( OMAP_DEVICE_TVOUT );
    ...
    //  Release clocks
    ReleaseClock( OMAP_DEVICE_TVOUT );
    ReleaseClock( OMAP_DEVICE_DSS2 );
    ReleaseClock( OMAP_DEVICE_DSS1 );
    ReleaseClock( OMAP_DEVICE_DSS );         
    ...
    )
{
    ...
    //  Request clock be enabled from root bus
...
OMAPDisplayController::EnableOverlayOptimization(BOOL bEnable)
{
        ...
        (INREG32( &m_pDispRegs->DISPC_CONFIG) & DISPC_CONFIG_LCDALPHABLENDERENABLE) == 0)
    {
        ...
        DWORD dwPosY = (INREG32(&m_pDispRegs->tDISPC_VID1.POSITION) >> 16) & 0xFFFF; 
    ...
    )
{
    ...
    // Switching clock is a 2 step process.
    // Step 1: If the clock src used is DSI, then switch to DSS
    // Step 2: Program the FCLK value 
     
    // Switch to OMAP_DSS_FCLK_DSS1ALWON source
    SwitchDssFclk( OMAP_DSS_FCLK_DSS1ALWON, eDssFclkValue );
    
    if ( eDssFclkSource == OMAP_DSS_FCLK_DSS2ALWON )
    {
        // Program the Dsi Pll to new Func clock value
        ConfigureDsiPll( eDssFclkValue );
        // Switch to the required clock source
        SwitchDssFclk( eDssFclkSource, eDssFclkValue );
        // Set the active clock source and release the inactive source
        clockSrc[0] = kDSS2_ALWON_FCLK;      
    }
    else
    {
        // Active clock source is DSS1_ALWON_FCLK.
        clockSrc[0] = kDSS1_ALWON_FCLK;
    }
    
    // Keep the FCLK used by DSS and release the unused FCLK. 
    count = 1;
    SelectDSSSourceClocks( count, clockSrc );
...
OMAPDisplayController::ConfigureDsiPll(ULONG clockFreq)
{
    ...
    ULONG sysClk  = SYSCLK;             //26Mhz (Could read from the PRCM...)
    ULONG value;
    ULONG dsiPhyClock;
    ULONG highFreqDiv = 0;
    ...
    )
{
	...
	RETAILMSG(0, (L"OMAPDisplayController::SwitchDssFclk FCLK source(%d)\r\n", eFclkSrc));
    if ( eFclkSrc == OMAP_DSS_FCLK_DSS1ALWON )
    {
		...
		__try
		{
			UINT32 sys_clk;
			UINT32 M, D;
			switch(INREG32(&m_pPrcmClkPRM->PRM_CLKSEL) & 0x7)
			{
				case 0:
					sys_clk = 12000000;
					break;
			...
			M = (INREG32(&m_pPrcmClkCM->CM_CLKSEL2_PLL)>>8) & 0x7FF;
			D = INREG32(&m_pPrcmClkCM->CM_CLKSEL2_PLL) & 0x7F;
			dpll4clk = (2 * M)*(sys_clk /(D+1));
			RETAILMSG(0, (L"OMAPDisplayController::SwitchDssFclk DPLL4ALWON (%d,%d,%d->%d)\r\n", sys_clk, M, D, dpll4clk));
			dpll4clk /= (INREG32(&m_pPrcmDssCMRegs->CM_CLKSEL_DSS)) & 0x1F;
			RETAILMSG(1, (L"OMAPDisplayController::SwitchDssFclk DSS CLK (%d)\r\n", dpll4clk));
		}
		__except(1)
		{
			RETAILMSG(1, (L"OMAPDisplayController::SwitchDssFclk exception (%X)\r\n",m_pPrcmClkCM));
		}
    }
    ...
    pcd = (dpll4clk + m_dwPixelClock/2) / m_dwPixelClock ;
    reg = DISPC_DIVISOR_LCD(lcd) | DISPC_DIVISOR_PCD(pcd);
	RETAILMSG(0, (L"OMAPDisplayController::SwitchDssFclk PCLK(%d -> %d) PCD (%X)\r\n", m_dwPixelClock, dpll4clk, reg));
...
// ---------------------------------------------------------------------------
BOOL
OMAPDisplayController::SelectDSSSourceClocks(ULONG count, ULONG *pClockSrc)
{
    BOOL rc = FALSE;
    ...
    // initialize clockInfo for configuration of source clocks
    clockInfo.devId    = OMAP_DEVICE_DSS;
    clockInfo.count    = count;

    for ( i = 0; i < count; i++)
    {
        clockInfo.rgSourceClocks[i] = pClockSrc[i];
    }

    // Call bus driver
    rc = DeviceIoControl(m_hRootBus,
    ...
    RETAILMSG((rc == FALSE),(L"ERROR: OMAPDisplayController::InitController:"
                 L"DSS2 clock src enable failed\r\n"
                 ));
