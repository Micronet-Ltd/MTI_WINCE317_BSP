...
static const DEVICE_REGISTRY_PARAM g_deviceRegParams[] = {
    {
        ...
        fieldsize(OMAPDDGPEGlobals, m_dwDSSIRQ), (VOID*)IRQ_DSS
    }, {
        ...
        fieldsize(OMAPDDGPEGlobals, m_dwEnableWaitForVerticalBlank), (VOID*)1
    }, {
        L"EnableNeonBlts", PARAM_DWORD, FALSE, offset(OMAPDDGPEGlobals, m_dwEnableNeonBlts),
        fieldsize(OMAPDDGPEGlobals, m_dwEnableNeonBlts), (VOID*)1
    }
    ...
    )
{
    ...
    unsigned int     tcrr;
    OMAP_DISPC_REGS	 *pDispRegs = 0;
    PHYSICAL_ADDRESS pa;
    IOCTL_HAL_PRCMDEVICEENABLECLOCKS_IN  pcrmDevEnClks = {0};
//    HANDLE m_hTled = TLEDOpen();

//	TLEDClose(m_hTled);
	SetDevicePower(L"BKL1:", POWER_NAME, D1);
	StallExecution(50000);
	SetDevicePower(L"BKL1:", POWER_NAME, D2);
	StallExecution(20000);
	SetDevicePower(L"BKL1:", POWER_NAME, D4);
//	if(!g_preq)
//		g_preq = SetPowerRequirement(L"BKL1:", D4, POWER_NAME, 0, 0);

	StallExecution(200000);

    // enable all the interface and functional clocks
	pcrmDevEnClks.bEnable	= 1;
	pcrmDevEnClks.devId		= OMAP_DEVICE_DSS;
	KernelIoControl(IOCTL_HAL_PRCMDEVICEENABLECLOCKS, (LPVOID)&pcrmDevEnClks, sizeof(pcrmDevEnClks), 0, 0, &size);
	pcrmDevEnClks.devId		= OMAP_DEVICE_DSS1;
	KernelIoControl(IOCTL_HAL_PRCMDEVICEENABLECLOCKS, (LPVOID)&pcrmDevEnClks, sizeof(pcrmDevEnClks), 0, 0, &size);
	pcrmDevEnClks.devId		= OMAP_DEVICE_DSS2;
	KernelIoControl(IOCTL_HAL_PRCMDEVICEENABLECLOCKS, (LPVOID)&pcrmDevEnClks, sizeof(pcrmDevEnClks), 0, 0, &size);
	pcrmDevEnClks.devId		= OMAP_DEVICE_TVOUT;
	KernelIoControl(IOCTL_HAL_PRCMDEVICEENABLECLOCKS, (LPVOID)&pcrmDevEnClks, sizeof(pcrmDevEnClks), 0, 0, &size);

	pa.QuadPart = OMAP_DISC1_REGS_PA;
    size = 1024;
    pDispRegs = (OMAP_DISPC_REGS*)MmMapIoSpace(pa, size, FALSE);
    ...
    if(pDispRegs)
	{
		...
		OUTREG32(&pDispRegs->DISPC_SYSCONFIG, DISPC_SYSCONFIG_AUTOIDLE |
											  SYSCONFIG_SMARTIDLE |
											  SYSCONFIG_ENAWAKEUP |
											  SYSCONFIG_CLOCKACTIVITY_I_ON |
											  SYSCONFIG_SMARTSTANDBY);

        MmUnmapIoSpace((VOID*)pDispRegs, size);
	}

    // restore old clock settings
	pcrmDevEnClks.bEnable	= 0;
	pcrmDevEnClks.devId		= OMAP_DEVICE_DSS;
	KernelIoControl(IOCTL_HAL_PRCMDEVICEENABLECLOCKS, (LPVOID)&pcrmDevEnClks, sizeof(pcrmDevEnClks), 0, 0, &size);
	pcrmDevEnClks.devId		= OMAP_DEVICE_DSS1;
	KernelIoControl(IOCTL_HAL_PRCMDEVICEENABLECLOCKS, (LPVOID)&pcrmDevEnClks, sizeof(pcrmDevEnClks), 0, 0, &size);
	...
	pcrmDevEnClks.devId		= OMAP_DEVICE_TVOUT;
	KernelIoControl(IOCTL_HAL_PRCMDEVICEENABLECLOCKS, (LPVOID)&pcrmDevEnClks, sizeof(pcrmDevEnClks), 0, 0, &size);
    // call to disable DSS clocks twice because the DSS clocks may
    // have been enabled by the bootloader.  As a result the refcount for
    // the clocks may have initialized to 1 rather than zero.
	pcrmDevEnClks.devId		= OMAP_DEVICE_DSS;
	KernelIoControl(IOCTL_HAL_PRCMDEVICEENABLECLOCKS, (LPVOID)&pcrmDevEnClks, sizeof(pcrmDevEnClks), 0, 0, &size);
	pcrmDevEnClks.devId		= OMAP_DEVICE_DSS1;
	KernelIoControl(IOCTL_HAL_PRCMDEVICEENABLECLOCKS, (LPVOID)&pcrmDevEnClks, sizeof(pcrmDevEnClks), 0, 0, &size);
...
OMAPDDGPE::OMAPDDGPE()
{
    ...
    if (!AdvertiseInterface(&guid, L"DDI.DLL", TRUE))
    {
    ...
    if( m_pDisplayContr->InitController(m_bEnableGammaCorr, g_Globals.m_dwEnableWaitForVerticalBlank) == FALSE )
    {
...
#endif
/* MN changes */	//Sleep(200);
//	unsigned short *pBuf = (unsigned short *)m_pPrimarySurface->Buffer();
/* MN changes */	//SetDevicePower(L"BKL1:", POWER_NAME, PwrDeviceUnspecified);
/* MN changes */	if(g_preq)
/* MN changes */	{
/* MN changes *///		ReleasePowerRequirement(g_preq);
/* MN changes */		g_preq = 0;
	}
}
    ...
    { 
        switch( blitParameters->rop4 )
        {
            ...
            case 0xCCCC:  // SRCCOPY
                ...
                {
                    //  Check the BLT flags
                    //if( (blitParameters->bltFlags & (BLT_STRETCH|BLT_ALPHABLEND|BLT_DSTTRANSPARENT)) == 0 )
    ...
    )
{
    ...
    switch (iEsc)
    {
        ...
        case GETPOWERMANAGEMENT:
            RETAILMSG(0, (L"OMAPDDGPE::DrvEscape: GETPOWERMANAGEMENT\r\n"));
        ...
        case SETPOWERMANAGEMENT:
            RETAILMSG(0, (L"OMAPDDGPE::DrvEscape: SETPOWERMANAGEMENT\r\n"));
            if( (cjIn >= sizeof(VIDEO_POWER_MANAGEMENT)) && (pvIn != NULL) )
            {
                ...
                {
					RETAILMSG(0, (L"SETPOWERMANAGEMENT(%s)\r\n", (VideoPowerOn)?	L"VideoPowerOn":
																 (VideoPowerStandBy)?L"VideoPowerStandBy":
																 (VideoPowerSuspend)?L"VideoPowerSuspend":
																 L"VideoPowerOff"
						));
...
//							SetDevicePower(L"BKL1:", POWER_NAME, PwrDeviceUnspecified);
                        ...
                        case VideoPowerSuspend:
							//if(g_preq)
							//	ReleasePowerRequirement(g_preq);
							//g_preq = SetPowerRequirement(L"BKL1:", D3, POWER_NAME, 0, 0);
//							SetDevicePower(L"BKL1:", POWER_NAME, D3);
...
//							SetDevicePower(L"BKL1:", POWER_NAME, D4);
        ...
        case IOCTL_POWER_CAPABILITIES:
            RETAILMSG(0, (L"OMAPDDGPE::DrvEscape: IOCTL_POWER_CAPABILITIES\r\n"));
        ...
        case IOCTL_POWER_GET:
            RETAILMSG(0, (L"OMAPDDGPE::DrvEscape: IOCTL_POWER_GET\r\n"));
            if(pvOut != NULL && cjOut == sizeof(CEDEVICE_POWER_STATE))
            {
                __try
                {
        ...
        case IOCTL_POWER_SET:
            RETAILMSG(0, (L"OMAPDDGPE::DrvEscape: IOCTL_POWER_SET\r\n"));
            if(pvOut != NULL && cjOut == sizeof(CEDEVICE_POWER_STATE))
            {
                __try
                {
                    ...
                    if( VALID_DX(dx) )
                    {
                        RETAILMSG(0, (L"OMAPDDGPE::DrvEscape: IOCTL_POWER_SET = to D%u\r\n", dx));
                        m_pDisplayContr->SetPowerLevel(dx, 1);
                        rc = ESC_SUCCESS;
                    }
                }
                __except(EXCEPTION_EXECUTE_HANDLER)
