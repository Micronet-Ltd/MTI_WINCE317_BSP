    ...
    if( pd->bIsDifferentPixelFormat )
    {
        ...
        else if( pDDSCaps->dwCaps & DDSCAPS_PRIMARYSURFACE )
        {
            ...
            if(!(pDDPixelFormat->dwFlags & DDPF_ALPHAPREMULT))
            {
                ...
                {
                    //  Check pixel sizes
                	if( (pDDPixelFormat->dwRGBBitCount == 16) || (pDDPixelFormat->dwRGBBitCount == 24) ||
                    	(pDDPixelFormat->dwRGBBitCount == 32) )
                    {
                        switch( pDDGPE->GetPrimaryPixelFormat() )
                        {
                        	...
                        	case OMAP_DSS_PIXELFORMAT_RGB24:
                            	pd->ddRVal = (IsRGB24(pDDPixelFormat)) ? DD_OK : DDERR_UNSUPPORTEDFORMAT;
                                break;
        ...
        else
        {
            ...
            if((pDDSCaps->dwCaps & DDSCAPS_OVERLAY) && !(pDDPixelFormat->dwFlags & DDPF_ALPHAPREMULT))
            {
                ...
                if( pDDPixelFormat->dwFlags & DDPF_RGB )
                {
                    ...
                    {
                        //  Check pixel formats
                        if( IsRGB16(pDDPixelFormat) || IsRGB24(pDDPixelFormat) ||
    ...
    else
    {
        ...
        if( pDDPixelFormat->dwFlags & DDPF_RGB )
        {
                ...
                (pDDPixelFormat->dwRGBBitCount == 32) )
            {
                ...
                else if( IsRGB24(pDDPixelFormat) )
                {
                    dssPixelFormat = OMAP_DSS_PIXELFORMAT_RGB24;
                }
        ...
        if( dssPixelFormat != OMAP_DSS_PIXELFORMAT_UNKNOWN )
        {
            ...
            for( iSurf = 0; iSurf < pd->dwSCnt; iSurf++ )
            {
                ...
                if (scResult != S_OK)
                {
                    DEBUGMSG (GPE_ZONE_ERROR, (L"ERROR: Unable to allocate surface\r\n"));
                    pd->ddRVal = DDERR_OUTOFVIDEOMEMORY;
                    goto cleanUp;
                }
    ...
    )
{
    ...
    if(pDDGPE->FlipInProgress() && 
        (pSurfCurr != NULL && pDDGPE->SurfaceFlipping(pSurfCurr)))
    {   
        if( (pd->dwFlags & DDFLIP_WAITNOTBUSY) || 
            (pd->dwFlags & DDFLIP_WAITVSYNC))
        {            
    ...
    )
{
    ...
    {               
        if((pSurf != NULL && pDDGPE->SurfaceFlipping(pSurf)))
        {
            pd->ddRVal = DDERR_WASSTILLDRAWING;        
        }
    }   
    else if ( (pd->dwFlags & DDGFS_CANFLIP) &&
                pDDGPE->SurfaceFlipping(pSurf))
    {        
        pd->ddRVal = DDERR_WASSTILLDRAWING;             
    }
    else 
    ...
    )
{
    ...
    if(pDDGPE->InVBlank())
        lpgsld->ddRVal = DDERR_VERTICALBLANKINPROGRESS;
    else 
        lpgsld->ddRVal = DD_OK;
    ...
    )
{
    ...
    if (pDDGPE->SurfaceFlipping(pSurf))
    {
    ...
    )
{
    ...
    OMAP_DSS_COLORKEY   eColorKey = OMAP_DSS_COLORKEY_GLOBAL_ALPHA_GFX;
