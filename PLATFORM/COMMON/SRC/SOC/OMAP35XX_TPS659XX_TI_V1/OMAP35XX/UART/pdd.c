...
typedef struct {
    DWORD memBase[2];                   // PA of UART and DMA registers
    DWORD memLen[2];                    // Size of register arrays
    DWORD irq;                          // IRQ
...
//static DWORD s_XcvrInUseCount = 0;

static DWORD s_OpenHandleCount = 0;
// ##### Guard 's_OpenHandleCount' variable !
CRITICAL_SECTION s_OpenHandleCS = {-2, 0, 0, 0, 0};
...
static const DEVICE_REGISTRY_PARAM s_deviceRegParams[] = {
    {
        L"MemBase", PARAM_MULTIDWORD, TRUE, offset(UARTPDD, memBase),
            fieldsize(UARTPDD, memBase), NULL
    }, {
        L"MemLen", PARAM_MULTIDWORD, TRUE, offset(UARTPDD, memLen),
            fieldsize(UARTPDD, memLen), NULL
    }, {
        L"Irq", PARAM_DWORD, TRUE, offset(UARTPDD, irq),
            fieldsize(UARTPDD, irq), NULL
    }, {
        ...
        L"RtsCtsEnable", PARAM_DWORD, FALSE, offset(UARTPDD, dwRtsCtsEnable),
            fieldsize(UARTPDD, dwRtsCtsEnable), (VOID*)1
    }
   };
//------------------------------------------------------------------------------
//  Local Defines
           ...
           )
{
    ...
    if (enable)
    {
        ...
        InterlockedIncrement(&pPdd->RxTxRefCount);
    }
    else
    {
        InterlockedDecrement(&pPdd->RxTxRefCount);
        if(!pPdd->RxTxRefCount) 
         ...
         )
{
    BOOL rc = FALSE;
    int nTimeout = 1000;
    CEDEVICE_POWER_STATE StartingState = pPdd->currentDX;
    
    RETAILMSG(1, (TEXT("UART:SetPower: D%d (curDx=D%d)\r\n"), dx, pPdd->currentDX));
    ...
    if (pPdd->currentDX != dx)
    {
        if (pPdd->currentDX <= D2 && dx >= D3)
        {
            ...
            {
                RETAILMSG(0, (TEXT("UART:SetPower: stop DMA\r\n")));
			...
			// since UART1 and UART3 share the same gpio to enable transceiver so we need to make sure its
            if (pPdd->hGpio != NULL)
            {   
				...
				EnterCriticalSection(&s_OpenHandleCS);

				if(s_OpenHandleCount == 0)
				{
				...
				LeaveCriticalSection(&s_OpenHandleCS);
            }
            ...
            if (pPdd->hPowerThread != NULL)
            {
				...
				pri = CeGetThreadPriority(pPdd->hPowerThread);
				CeSetThreadPriority(pPdd->hPowerThread, CeGetThreadPriority(GetCurrentThread()) - 1);


				RETAILMSG(0, (L"UART::SetPower: Notify power thread %x %d %d\r\n", pPdd->memBase[0],
					CeGetThreadPriority(GetCurrentThread()), GetTickCount()));
                SetEvent(pPdd->hPowerEvent);
                ...
                do
                {
					if(WaitForSingleObject(pPdd->hPowerEvent, 10) != WAIT_OBJECT_0)
						break;
					RETAILMSG(0, (L"UART::SetPower: Not accepted by power thread %x %d\r\n",pPdd->memBase[0], GetTickCount()));
				}while(dwCount++ < 100);
				CeSetThreadPriority(pPdd->hPowerThread, pri);

				RETAILMSG(0, (L"UART::SetPower: Accepted by power thread %x %d\r\n",pPdd->memBase[0], GetTickCount()));
            ...
            RETAILMSG(0, (TEXT("UART:SetPower: Set parent bus to (D%d)\r\n"), dx));
        ...
        else if (pPdd->currentDX >= D3 && dx <= D2)
        {
            ...
            {
                RETAILMSG(1, (L"UART:SetPower: Starting DMA\r\n"));
    ...
    RETAILMSG(0,(L"UART:SetPower Leave D%d\r\n", pPdd->currentDX));
    return rc;
}
...
static VOID InitializeUART(UARTPDD *pPdd)
{
   ...
   // OUTREG8(&pUartRegs->IER, UART_IER_SLEEP_MODE);
	...
	// ##### MichaelS. Not need enable interrupt on Init !
...
static VOID RestoreUARTContext(UARTPDD *pPdd)
{
    ...
    if (pPdd->open == TRUE)
    {
		...
		// ##### MichaelS. Enable 'UART_IER_RHR' too!
...
skipTXDMA:
    ...
    if (pPdd->hRxDmaChannel != NULL)
    {
        ...
        // boost the Rx IST Thread priority
      //  CeSetThreadPriority(pPdd->hRxThread, 100);
...
skipRXDMA:
    ...
    // Map interrupt
    if (!KernelIoControl(
        IOCTL_HAL_REQUEST_SYSINTR, irqs, size, &pPdd->sysIntr,
	...
	// ##### Initializa this cs on 1st HWInit calling ('-2')
	if(-2 == s_OpenHandleCS.LockCount)
		InitializeCriticalSection(&s_OpenHandleCS);
         ...
         )
{
	...
	DeleteCriticalSection(&s_OpenHandleCS);
...
//
DWORD PowerThreadProc(void *pParam)
{
    ...
    while(TRUE)
    {
		RETAILMSG(0, (L"UART::PowerThreadProc: Wait for power event %x %d\r\n", pPdd->memBase[0], GetTickCount()));
		WaitForSingleObject(pPdd->hPowerEvent, nTimeout);

        if(pPdd->bExitPowerThread)
		{
			ResetEvent(pPdd->hPowerEvent);
			break;
		}
        // serialize access to power state changes
		RETAILMSG(0, (L"UART::PowerThreadProc: power event is got %x %d %d\r\n", pPdd->memBase[0],
			CeGetThreadPriority(GetCurrentThread()), GetTickCount()));
	...
	__try
	{
        ...
        if (pPdd->RxTxRefCount == 0)
        {
            ...
            {
				RETAILMSG(0, (L"UART::PowerThreadProc: Force idle %x %d\r\n", pPdd->memBase[0], GetTickCount()));
                // check to make sure the uart clocks are on
                if (pPdd->currentDX >= D3) 
                {
					// turn on force idle, to allow full retention
					OUTREG8(&pPdd->pUartRegs->SYSC, UART_SYSC_IDLE_FORCE|UART_SYSC_WAKEUP_ENABLE|UART_SYSC_AUTOIDLE);
                }
                nTimeout = INFINITE;
            }
            else
            {
				RETAILMSG(0, (L"UART::PowerThreadProc: disable auto-idle %x %d\r\n", pPdd->memBase[0], GetTickCount()));
	...
	__except(1)
	{
		RETAILMSG(0, (L"UART::PowerThreadProc: exception %x %d\r\n", pPdd->memBase[0], GetTickCount()));
		nTimeout = INFINITE;
	}
        LeaveCriticalSection(&pPdd->hwCS);
		...
		RETAILMSG(0, (L"UART::PowerThreadProc: free power Notifying thread %x %d\r\n",pPdd->memBase[0], GetTickCount()));
        ResetEvent(pPdd->hPowerEvent);
    }
	RETAILMSG(0, (L"UART::PowerThreadProc: Leave %x %d\r\n", pPdd->memBase[0], GetTickCount()));
    return 1;
}
...
BOOL HWOpen(VOID *pvContext)
{
    ...
    RETAILMSG(0, (L"HWOpen - Membase=0x%08x\r\n", pPdd->memBase[0]));
	...
	if(pPdd->index == 2)
	{
		RETAILMSG(1, (L"HWOpen - reset BT module\r\n"));
		
		gvp_bt_reset = GPIOOpen();
		GPIOSetMode(gvp_bt_reset, PCA_GPIO_21, GPIO_DIR_OUTPUT); // Power nPD
		GPIOSetMode(gvp_bt_reset, PCA_GPIO_31, GPIO_DIR_OUTPUT);
		GPIOClrBit(gvp_bt_reset, PCA_GPIO_21);
		GPIOClrBit(gvp_bt_reset, PCA_GPIO_31);
		GPIOSetBit(gvp_bt_reset, PCA_GPIO_21);
		Sleep(100);
		GPIOSetBit(gvp_bt_reset, PCA_GPIO_31);
	}
#endif
...
static
ULONG HWClose(VOID *pvContext)
{
    ULONG rc = -1;
    UARTPDD *pPdd = (UARTPDD*)pvContext;

    DEBUGMSG(ZONE_CLOSE||ZONE_FUNCTION, (L"+HWClose(0x%08x)\r\n", pvContext));
    RETAILMSG(0, (L"HWClose - Membase=0x%08x\r\n", pPdd->memBase[0]));
	...
	if(pPdd->index == 2)
	{
		RETAILMSG(1, (L"HWClose - put BT module to reset\r\n"));
		
		GPIOClrBit(gvp_bt_reset, PCA_GPIO_31);
		GPIOClose(gvp_bt_reset);
	}
#endif
           ...
           )
{
    ...
    if (pPdd->autoRTS)
    {
		...
		// #####, MichaelS. And what about 'UART_IER_LINE' and RxDMA mode???
        pPdd->intrMask &= ~UART_IER_RHR;
        OUTREG8(&pUartRegs->IER, pPdd->intrMask);
