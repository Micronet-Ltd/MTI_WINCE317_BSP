...
///////////////////////////////////////////////////////////////////////////////
STDAPI_(BOOL) DllEntry(HINSTANCE  hInstance, ULONG Reason, LPVOID pReserved)
{
...
//	if(pController->m_dwSlot == MMCSLOT_2)
...
extern "C" BOOL SHC_IOControl(DWORD hOpenContext, DWORD dwCode, PBYTE pBufIn, DWORD dwLenIn, PBYTE pBufOut, DWORD dwLenOut, PDWORD pdwActualOut)
{
    BOOL rc = FALSE;
	UINT32 modem_on = 0;
    CSDIOControllerBase * pController = (CSDIOControllerBase*) hOpenContext; 
    ...
    if (pController == NULL)
    {
        DEBUGMSG(SDCARD_ZONE_ERROR, (L"ERROR: SHC_IOControl: Incorrect context paramer\r\n"));
        goto cleanUp;
    }
    ...
    switch (dwCode)
    {
        case IOCTL_POWER_CAPABILITIES: 
            ...
            if(pBufOut && dwLenOut >= sizeof (POWER_CAPABILITIES) && pdwActualOut) 
            {
                    __try 
                    {
                        ...
                        PowerCaps->DeviceDx = (DX_MASK(D0) | DX_MASK(D2) | DX_MASK(D3) | DX_MASK(D4));
        ...
        case IOCTL_POWER_QUERY: 
            rc = TRUE;
            DEBUGMSG(SDCARD_ZONE_INFO,(L"SHC: Received IOCTL_POWER_QUERY\r\n"));
            break;
...
//							pController->SetPower(D0);
							if(pController->m_ActualPowerState > D2 && pController->m_dwSlot == MMCSLOT_2)
							{
								RETAILMSG(1, (L"SHC_IOControl:IOCTL_POWER_SET:D%d resume %d, start 'resume_thread_2'\r\n", ReqDx, GetTickCount()));
							}
							else if(/*pController->m_ActualPowerState > D2 &&*/ pController->m_dwSlot == MMCSLOT_1)
							{
								pController->SetPower(D0);
								if(!pController->SDCardDetect())
									SetEvent(pController->m_hCardDetectEvent);
							}
                            break;
                        ...
                        case D4:
						{
                            // Move to PowerDown() in order to give client driver a chance to shutdown first
                            //pController->SetPower(D4);

							RETAILMSG(0, (L"SHC_IOControl:IOCTL_POWER_SET:D3/D4 eject slot %d %d\r\n", pController->m_dwSlot, GetTickCount()));
