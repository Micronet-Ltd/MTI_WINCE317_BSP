...
static const DEVICE_REGISTRY_PARAM s_deviceRegParams[] = {
    {
        ...
        fieldsize(CSDIOControllerBase, m_dwSDHighSpeedSupport), (VOID*)1
    }, {
...
static VOID DumpRequest(PSD_BUS_REQUEST pRequest)
{
    RETAILMSG(1, (TEXT("DumpCurrentRequest: 0x%08X\r\n"), pRequest));
    RETAILMSG(1, (TEXT("\t Command: %d\r\n"),  pRequest->CommandCode));
    RETAILMSG(1, (TEXT("\t ResponseType: %d\r\n"),  pRequest->CommandResponse.ResponseType));
    RETAILMSG(1, (TEXT("\t NumBlocks: %d\r\n"),  pRequest->NumBlocks));
    RETAILMSG(1, (TEXT("\t BlockSize: %d\r\n"),  pRequest->BlockSize));
    RETAILMSG(1, (TEXT("\t HCParam: %d\r\n"),    pRequest->HCParam));
...
VOID CSDIOControllerBase::SetClockRate(PDWORD pdwRate)
{
	...
	RETAILMSG(1, (TEXT("CSDIOControllerBase::SetClockRate: %d(Hz)/%d is %d(Hz)\r\n"), MMCSD_CLOCK_INPUT, dwDiv, dwClockRate));
    ...
    *pdwRate = MMCSD_CLOCK_INPUT / dwDiv;
    ...
    RETAILMSG(0,(TEXT("SDHCSetRate - Actual clock rate = %d, MMCHS_SYSCTL = 0x%x\r\n"), *pdwRate, INREG32(&m_pbRegisters->MMCHS_SYSCTL)));
}
...
BOOL CSDIOControllerBase::HandleCardDetectInterrupt(DWORD dwStatus)
{
    ...
    {
        #ifdef ENABLE_RETAIL_OUTPUT
            RETAILMSG(1, (TEXT("CardDetectInterrupt: Card removed %d\r\n"), m_dwSlot));
        #endif
    ...
    if (dwStatus & EXT_MMCHS_STAT_CD_INSERT_INTR)
    {
        ...
        #ifdef ENABLE_RETAIL_OUTPUT
            RETAILMSG(1, (TEXT("CardDetectInterrupt: Card inserted %d\r\n"), m_dwSlot));
        #endif
...
BOOL CSDIOControllerBase::Init( LPCTSTR pszActiveKey )
{
    ...
    m_dwCPURev = CPU_REVISION_UNKNOWN;
...
DWORD CSDIOControllerBase::SDHCControllerIstThreadImpl()
{
    ...
    SDHCCONTROLLERIST_STATE eNextState;
    ...
    )
{
    ...
    if( dwIntrStatus & MMCHS_STAT_CTO ) // command response timeout
        ...
        RETAILMSG(0, (TEXT("SDHControllerIstThread() - Got command response timeout!\r\n")));
        }
    if( dwIntrStatus & MMCHS_STAT_DTO ) // data timeout
        {
        status = SD_API_STATUS_RESPONSE_TIMEOUT;
        dwOverwrite |= MMCHS_STAT_DTO;
        RETAILMSG(0, (TEXT("SDHControllerIstThread() - Got data timeout!\r\n")));
        }
    if( dwIntrStatus & MMCHS_STAT_DCRC ) // data CRC error
        {
        status = SD_API_STATUS_RESPONSE_TIMEOUT;
        dwOverwrite |= MMCHS_STAT_DCRC;
        RETAILMSG(0, (TEXT("SDHControllerIstThread() - Got data CRC error!\r\n")));
        }
    ...
    )
{
    ...
    if(dwIntrStatus & (MMCHS_STAT_BRR | MMCHS_STAT_TC))
        ...
        if( dwIntrStatus & MMCHS_STAT_DTO )
            {
            RETAILMSG(0, (TEXT("SDIDMAReceive() - exit: STAT register indicates MMC_STAT_DTO error.\r\n")));
            status = SD_API_STATUS_RESPONSE_TIMEOUT;
            }
        if( dwIntrStatus & MMCHS_STAT_DCRC ) // DATA CRC Error
            {
            RETAILMSG(0, (TEXT("SDIDMAReceive() - exit: STAT register indicates MMC_STAT_DCRC error.\r\n")));
            status = SD_API_STATUS_RESPONSE_TIMEOUT;
            }
        }
    ...
    )
{
    ...
    if (dwIntrStatus & MMCHS_STAT_TC)
        ...
        if( dwIntrStatus & MMCHS_STAT_DTO )
            {
            RETAILMSG(0, (TEXT("DataTransmitCompletedHandler() - exit: STAT register indicates MMC_STAT_DTO error.\r\n")));
            status = SD_API_STATUS_RESPONSE_TIMEOUT;
            }
        if( dwIntrStatus & MMCHS_STAT_DCRC ) // DATA CRC Error
            {
            RETAILMSG(0, (TEXT("DataTransmitCompletedHandler() - exit: STAT register indicates MMC_CRC_ERROR_RCVD error.\r\n")));
            status = SD_API_STATUS_RESPONSE_TIMEOUT;
            }
        }
...
SD_API_STATUS CSDIOControllerBase::SDHCDeinitializeImpl()
{
    ...
    if (NULL != m_htCardDetectIST)
    {
		...
		if(m_dwSlot == MMCSLOT_2)
		{
			RETAILMSG(1, (TEXT("CSDIOControllerBase::SDHCDeinitializeImpl - Remove Card\r\n")));
    ...
    if (NULL != m_hCardPresent)
    {
		RETAILMSG(1, (TEXT("CSDIOControllerBase::SDHCDeinitializeImpl - Close resence\r\n")));
...
SD_API_STATUS CSDIOControllerBase::SDHCInitializeImpl()
{
	...
	wsprintf(ev, L"MMC Card Detect %d", m_dwSlot);

    // allocate the interrupt event for card detection
    m_hCardDetectEvent = CreateEvent(0, 0, 0, ev);
	...
	wsprintf(ev, L"MMC Card Present %d", m_dwSlot);

    // allocate the interrupt event for card detection
    m_hCardPresent = CreateEvent(0, 1, 0, ev);

	// ##### Michael. workaround for WLAN/BT power down mode
	m_hModemOnOffEventSoc = CreateEvent(0, 1, 0, L"ModemOnOffEvent");
	...
	if (m_dwSlot == MMCSLOT_2)
	{
		RETAILMSG(0, (TEXT("CSDIOControllerBase::SDHCInitializeImpl - Card Presence\r\n")));
		ResetEvent(m_hCardPresent);
	}
...

    {
       ...
       while (!(Read_MMC_STAT() & MMCHS_STAT_CC)) {
           if (retries > SDIO_MAX_LOOP) {
                ...
                {
                    status = SD_API_STATUS_RESPONSE_TIMEOUT;
                    RETAILMSG(0, (TEXT("SDHCBusRequestHandler() - MMCHS_STAT_CTO\r\n")));
                }
    ...
    if( MMC_PSTAT & MMCHS_PSTAT_DATI )
    {
        if( pRequest->CommandResponse.ResponseType == ResponseR1b )
        {
            ...
            do 
            {
                ...
                if (GetTickCount() - dwCountStart > m_dwMaxTimeout)
                {
                    RETAILMSG(0, (TEXT("SDHControllerIstThread() - Card BUSY timeout!\r\n")));
                    status = SD_API_STATUS_RESPONSE_TIMEOUT;
                    goto TRANSFER_DONE;
                }
    ...
    switch (Option)
    {
        ...
        case SDHCDGetSlotInfo:
           ...
           SDHCDSetDesiredSlotVoltage(pSlotInfo, SD_VDD_WINDOW_2_9_TO_3_0);
            ...
            // set power up delay
            SDHCDSetPowerUpDelay(pSlotInfo, 250);
...
BOOL CSDIOControllerBase::SetPower(CEDEVICE_POWER_STATE dx)
{
    ...
    RETAILMSG(0, (L"CSDIOControllerBase::SetPower %d D%d -> D%d\r\n", m_dwSlot, m_ActualPowerState, dx));
...
{
    RETAILMSG(1, (L"CSDIOControllerBase::PowerUp %d\r\n", m_dwSlot));
}
...
//
BOOL CSDIOControllerBase::ContextRestore()
{
    SD_API_STATUS      status;              // SD status
