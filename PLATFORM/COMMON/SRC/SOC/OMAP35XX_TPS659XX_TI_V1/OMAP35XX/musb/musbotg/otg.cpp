...
#define SUSPEND_RESUME_DEBUG_ENABLE	0
...
static const DEVICE_REGISTRY_PARAM s_deviceRegParams[] = 
{
    ...
    }, 
    {
        L"DisableHighSpeed", PARAM_DWORD, FALSE, offset(MUsbOTGRegCfg_t, DisableHighSpeed),
            fieldsize(MUsbOTGRegCfg_t, DisableHighSpeed), (void *) 0
    }, 
    {
...
DWORD OMAPMHSUSBIdGnd::ThreadRun()
{
    ...
    CeSetThreadPriority(GetCurrentThread(), IDGND_IST_PRIORITY);
    ...
    { 
        ...
        if (rc != WAIT_OBJECT_0)
        {
            DEBUGMSG(ZONE_OTG_ERROR, (TEXT("Invalid IdGndIntrEvent\r\n")));
            m_fTerminated = TRUE;
        }
        else
        {                 
			RETAILMSG(1,(L"%S: USB_PRES interrupt\r\n", __FUNCTION__)); 
...
{    
    CeSetThreadPriority(GetCurrentThread(), DMA_IST_PRIORITY);
            ...
            while (i < MAX_DMA_CHANNEL)
            {
                if (reg & INTR_CH(i))
                {
                    if (m_pUsbOtg->GetMode() == HOST_MODE)
                        pOTG->pFuncs[HOST_MODE-1]->ProcessDMA(pOTG, i);
                    else if (m_pUsbOtg->GetMode() == DEVICE_MODE)
                        pOTG->pFuncs[DEVICE_MODE-1]->ProcessDMA(pOTG, i);
                    else
                    {
...
BOOL OMAPMHSUSBOTG::Init()
{
	...
	RETAILMSG(0, (TEXT("OMAPMHSUSBOTG::Init:Power = 0x%x\r\n"), INREG8(&m_pOTG->pUsbGenRegs->Power)));
...
BOOL OMAPMHSUSBOTG::PostInit()
{
    ...
    UpdateDevicePower(m_hParent, D3, NULL);
    ThreadStart();
        ...
        case IOCTL_POWER_SET:
            ...
            if (pBufOut && dwLenOut >= sizeof(CEDEVICE_POWER_STATE))
            {
                __try
                {
                    ...
                    RETAILMSG(1, (TEXT("OTG IOCTL_POWER_SET D%d\r\n"), ReqDx));
...
void OMAPMHSUSBOTG::PowerDownDisconnect()
{
	...
	RETAILMSG(1, (TEXT("Disconnect detected: Reset Endpoints \r\n")));
    ResetEndPoints();
    // If we have done the disconnect, don't process anymore.
    if ((m_pOTG->connect_status & CONN_DC) == 0)
	{
		RETAILMSG(1, (TEXT("Disconnect detected: Wait for Host disconnect \r\n")));
	...
	RETAILMSG(1, (TEXT("Disconnect detected: disconnect function\r\n")));
    if (m_pOTG->pFuncs[DEVICE_MODE-1] != NULL)
        m_pOTG->pFuncs[DEVICE_MODE-1]->Disconnect((void *)m_pOTG);

	RETAILMSG(1, (TEXT("Disconnect detected: reset controller\r\n")));
...
{    
    ...
    if ((((m_pOTG->intr_usb & INTRUSB_DISCONN) == INTRUSB_DISCONN)) || m_bSessionDisable)
    {
        DWORD dwPrevState;
        DWORD dwCurMode = m_pOTG->operateMode;              
        RETAILMSG(0, (TEXT("Disconnect detected(%d)\r\n"), m_bSessionDisable));        
        ...
        if ((m_dwStatus & STATUS_HNP_SESSION_MASK) == 0x00)
        {
            ...
            RETAILMSG(0, (TEXT("operateMode = IDLE (d%X)\r\n"), m_dwStatus));
            m_pOTG->operateMode = IDLE_MODE;
        ...
        RETAILMSG(0, (TEXT("Disconnect with Sess = 0x%x\r\n"),INREG8(&m_pOTG->pUsbGenRegs->DevCtl)));            
        RETAILMSG(0, (TEXT("Disconnect with Pwr = 0x%x\r\n"), INREG8(&m_pOTG->pUsbGenRegs->Power)));
        RETAILMSG(0, (TEXT("Disconnect with connect_status=0x%x\r\n"), INREG8(&m_pOTG->connect_status)));
    ...
    {
        RETAILMSG(0, (TEXT("Suspend detected with DevCtl = 0x%x\r\n"), INREG8(&m_pOTG->pUsbGenRegs->DevCtl)));
        RETAILMSG(0, (TEXT("Suspend detected with Power = 0x%x\r\n"), INREG8(&m_pOTG->pUsbGenRegs->Power)));
        ...
        {
            RETAILMSG(0, (TEXT("################ DEBUG THIS, IT IS DEVICE MODE using A_DEVICE\r\n")));
...
DWORD    OMAPMHSUSBOTG::OTG_ConfigISR_stage1()
{
    ...
    if ((m_pOTG->intr_usb & INTRUSB_RESUME) == INTRUSB_RESUME)
    {
        ...
        RETAILMSG(0, (TEXT("Clear the Resume interrupt\r\n")));
    ...
    {
        RETAILMSG(0, (TEXT("Session Request\r\n")));
        SessionRequest(TRUE, TRUE);
    }

    // VBUS error interrupt only valid on A-Device
/*
	do
	{
	#if 1
        // Removed this section, seems to cause problems with USB host mode...
        // B-Device field only valid when session is in progress
		if((INREG8(&m_pOTG->pUsbGenRegs->DevCtl) & DEVCTL_SESSION) != DEVCTL_SESSION)
		{
            RETAILMSG(0, (TEXT("No SESSION\r\n")));
			break;
		}
        if((INREG8(&m_pOTG->pUsbGenRegs->DevCtl) & DEVCTL_B_DEVICE) != 0)
			break;
	#endif
*/            
		...
		if((m_pOTG->intr_usb & INTRUSB_VBUSERR) == INTRUSB_VBUSERR)
		{
			...
			RETAILMSG(1, (TEXT("VBus Error - OTG_HandleVBusError to be implemented (%X)\r\n"), m_pOTG->intr_usb));
/*
			m_pTransceiver->SetVBusSource(TRUE);
            Sleep(1000);
            if(m_pOTG->pFuncs[DEVICE_MODE-1])
				m_pOTG->pFuncs[DEVICE_MODE-1]->Disconnect((void *)m_pOTG);                  
			ResetEndPoints();

            // Send the message to Host to do Host_Connect
            // Not sure why we need to change the power state, seems odd...
            UpdateDevicePower(m_hParent, D4, 0);
            // Take critical section to reduce chance of transceiver 
            // driver accessing ULPI interface while clocks are off.
            EnterCriticalSection(&m_csUSBClock);        
            Sleep(3000);
            LeaveCriticalSection(&m_csUSBClock);        
            UpdateDevicePower(m_hParent, D0, 0);
...
*/
        ...
        ((INREG8(&m_pOTG->pUsbGenRegs->DevCtl) & DEVCTL_HOSTMODE) == DEVCTL_HOSTMODE))
    {
        ...
        else
        {
            ...
            if(m_pOTG->dwPwrMgmt !=  MODE_SYSTEM_SUSPEND)
            {
				...
				RETAILMSG(1, (TEXT("Connect: DeviceType [%s]\r\n"), 
    ...
    {
        RETAILMSG(1, (TEXT("Invalid Connection Detect with DevCtl = 0x%x\r\n"), INREG8(&m_pOTG->pUsbGenRegs->DevCtl)));
    }
    ...
    if ((m_pOTG->intr_usb & INTRUSB_RESET) == INTRUSB_RESET)
    {
            ...
            && ((m_dwStatus & STATUS_HNP_SESSION_MASK) == 0x00))// Device Mode
        {
            ...
            RETAILMSG(0, (TEXT("USB Reset detected, it would be connection!!\r\n")));
            RETAILMSG(0, (TEXT("INTR_USB = 0x%x\r\n"), m_pOTG->intr_usb));
            RETAILMSG(0, (TEXT("DEVCTL = 0x%x\r\n"), INREG8(&m_pOTG->pUsbGenRegs->DevCtl)));
            DEBUGMSG(ZONE_OTG_INFO, (TEXT("INTRRX = 0x%x, INTRTX = 0x%x\r\n"), m_pOTG->intr_rx, m_pOTG->intr_tx));
            RETAILMSG(0, (TEXT("Power Mode = 0x%x\r\n"), INREG8(&m_pOTG->pUsbGenRegs->Power)));
            DEBUGMSG(ZONE_OTG_INFO, (TEXT("DMA INTR = 0x%x\r\n"), INREG32(&m_pOTG->pUsbDmaRegs->Intr)));

            m_pOTG->deviceType = ((INREG8(&m_pOTG->pUsbGenRegs->DevCtl) & DEVCTL_B_DEVICE)? B_DEVICE: A_DEVICE);
            RETAILMSG(0, (TEXT("Reset: DeviceType [%s]\r\n"), 
                ((m_pOTG->deviceType & B_DEVICE)?(TEXT("B_DEVICE")):(TEXT("A_DEVICE"))) ));
            ...
            {                
                ...
                ResetEndPoints();
                RETAILMSG(0, (TEXT("operateMode = IDLE (HNP host to device)\r\n")));
                m_pOTG->operateMode = IDLE_MODE;
        ...
        {
            if ((m_dwStatus & STATUS_HNP_SESSION_MASK) == 0x00)
                RETAILMSG(0, (TEXT("USB Babble:DevCtl;0x%x\r\n"),INREG8(&m_pOTG->pUsbGenRegs->DevCtl)));
            else
                RETAILMSG(0, (TEXT("Wait for disconnect complete\r\n")));
        }
...
BOOL OMAPMHSUSBOTG::StopUSBClock()
{
    ...
    pOtg = m_pOTG->pUsbOtgRegs;        
        ...
        DEBUGMSG(1, (TEXT("USB Clock force standby, force idle\r\n")));
    ...
    switch (reqDx)
    {
        ...
        case D2:
        {
            ...
            if (m_pTransceiver->SupportsTransceiverWakeWithoutClock())
			{
                ...
                if (m_disconnected)
                {
					RETAILMSG(1, (L" OTG SetPowerState - disconnected\r\n"));
                    if(a_device_present)
                    {
						RETAILMSG(1, (L" OTG SetPowerState - A device\r\n"));
...
//						m_pTransceiver->Resume();
                    ...
                    {
						RETAILMSG(1, (L" OTG SetPowerState - B device\r\n"));
                        OUTREG8(&pGen->IntrUSBE, INTRUSB_ALL&~INTRUSB_SOF);
                        OUTREG16(&pGen->IntrTxE, 0xffff);
                        OUTREG16(&pGen->IntrRxE, 0xfffe);
                    ...
                    {
						RETAILMSG(1, (L" OTG SetPowerState - NO devices\r\n"));
                        StopUSBClock();
                ...
                {
					RETAILMSG(1, (L" OTG SetPowerState - connected\r\n"));
                    StopUSBClock();
...
//				SetInterruptEvent(m_dwSysIntr);
        ...
        case D4:
        {
            ...
            if (!m_disconnected)
            {
                ...
                RETAILMSG(1, (TEXT("OTG Power Down\r\n")));
                ...
                {
                    RETAILMSG(1, (TEXT("OTG Power Down (D3)\r\n")));
                    ...
                    m_pTransceiver->SetVBusSource(FALSE);
                    m_pTransceiver->EnableVBusDischarge(TRUE);
                    Sleep(m_OTGRegCfg.vbusDischargeTime);
                    m_pTransceiver->EnableVBusDischarge(FALSE);
                ...
                {
                    RETAILMSG(1, (TEXT("OTG Power Down (D4)\r\n")));
            ...
            {
                RETAILMSG(1, (TEXT("OTG Power Down disconnected\r\n")));
                if (m_pOTG->bClockStatus == TRUE)
                {
	                m_pOTG->dwPwrMgmt = MODE_SYSTEM_SUSPEND;
					// clock control moved to PowerDown
            ...
            m_pTransceiver->EnableWakeupInterrupt(0);
        }
        break;
    }
cleanUp:
...
DWORD OMAPMHSUSBOTG::ThreadRun()
{
    ...
    CeSetThreadPriority(GetCurrentThread(), OTG_IST_PRIORITY);
	...
	/*do{
		RETAILMSG(1, (TEXT("DEVCTL_SESSION false, setting it again 0x%x\r\n"), INREG8(&m_pOTG->pUsbGenRegs->DevCtl)));    
	    SETREG8(&m_pOTG->pUsbGenRegs->DevCtl, DEVCTL_SESSION);
		Sleep(100);
	}while(!(INREG8(&m_pOTG->pUsbGenRegs->DevCtl) & DEVCTL_SESSION));
*/
    m_bOTGReady = 1;
    ...
    {                        
        ...
        if (m_pTransceiver->SupportsTransceiverWakeWithoutClock())
		{
            ...
            if (m_fPowerRequest)
            {
                ...
                SetEvent(m_pOTG->hPowerEvent);
				RETAILMSG(1, (TEXT("OTG Power management: operateMode(%d), powerMode(%X) m_dwStatus(%X)\r\n"),
							 m_pOTG->operateMode, m_pOTG->dwPwrMgmt, m_dwStatus));
            }
            if(m_Dx == D4 || m_Dx == D3)
            {
                ResetEvent(m_hIntrEvent);
            ...
            if (rc == WAIT_TIMEOUT)
            {
                ...
                if(m_bSuspendTransceiver)
                {
                    ...
                    {
						RETAILMSG(1, (TEXT("OMAPMHSUSBOTG::ThreadRun: A device connected\r\n")));
                ...
                if (m_pOTG->operateMode == HOST_MODE)
                {
					RETAILMSG(1, (TEXT("OMAPMHSUSBOTG::ThreadRun: operate as HOST\r\n")));
                    //EnableSuspend(TRUE);
                }
                else if (m_pOTG->operateMode == DEVICE_MODE)
                {
					RETAILMSG(1, (TEXT("OMAPMHSUSBOTG::ThreadRun: operate as DEVICE\r\n")));
                }
                else
                {
                    // If you are here, it can be either IDLE_MODE or SUSPEND_MODE
                    RETAILMSG(1, (TEXT("StopUSBClock with operateMode=%d, m_dwStatus=0x%x\r\n"),
                    ...
                    if ((m_dwStatus & STATUS_HNP_SESSION_MASK) == 0)
                    {
                        ...
                        {
                            RETAILMSG(1, (TEXT("+(m_dwStatus & STATUS_WAIT_HOST_DISCONN_COMPLETE) == 0x00\r\n")));
            ...
            else
            {
                if ((m_dwStatus & STATUS_SESSION_RESTART)
                    ...
                    && (m_pOTG->operateMode == IDLE_MODE))
                {
					RETAILMSG(1, (TEXT("OMAPMHSUSBOTG::ThreadRun: Session restart\r\n")));
                    if (m_pOTG->pFuncs[DEVICE_MODE-1] != NULL)
                        m_pOTG->pFuncs[DEVICE_MODE-1]->Disconnect((void *)m_pOTG);
            ...
            if (m_pOTG->dwPwrMgmt == MODE_SYSTEM_RESUME)
            {
                if (m_pOTG->operateMode == HOST_MODE)
                {
                    ...
                    RETAILMSG(1, (TEXT("Resume on Host Mode\r\n")));
                ...
                else
                {
					...
					RETAILMSG(1, (TEXT("OMAPMHSUSBOTG::ThreadRun: Resume Device %X\r\n"), devCtl));
                    ...
                    if (m_pTransceiver->IsBDeviceConnected() ||
                        ...
                        {
                        // No Device attached
                        RETAILMSG(1, (TEXT("NO Device Connected\n")));
            ...
            if((!m_pOTG->intr_usb) && (!m_pOTG->intr_tx) && (!m_pOTG->intr_rx))
            {
                ...
                RETAILMSG(0,(TEXT("Interrupt Variable INTRUSB No interrupt at all\n")));
            }
            ...
            {
                RETAILMSG(1, (TEXT("Status Retention with DevCtl Session = 0x%x\r\n"),
                    INREG8(&m_pOTG->pUsbGenRegs->DevCtl)));
            ...
            {
                RETAILMSG(1, (TEXT("We can process role switch loading now\r\n")));
                if ((INREG8(&m_pOTG->pUsbGenRegs->DevCtl) & DEVCTL_HOSTMODE) == 0x00)
            ...
            if (m_pOTG->intr_usb)
            {
                RETAILMSG(0, (TEXT("OTG Config\r\n")));
                OTG_ConfigISR_stage1();
            }

            if ((m_dwStatus & STATUS_HNP_SESSION_MASK) == 0x00)
            {
                RETAILMSG(0, (TEXT("OTG Status process EPs\r\n")));
            ...
            if (m_pOTG->intr_usb)
            {
                RETAILMSG(0, (TEXT("OTG Config 2\r\n")));
                OTG_ConfigISR_stage2();
            }

            if(m_handleVBUSError)
            {
                RETAILMSG(1, (TEXT("OTG Status VBUS error\r\n")));
                SessionRequest(TRUE, TRUE);
			    m_timeout = m_OTGRegCfg.startupTimeout;
                m_handleVBUSError = FALSE;
            }
            ...
            if ((m_dwStatus & STATUS_HNP_SESSION_MASK) == 0x00)
            {
                RETAILMSG(0, (TEXT("OTG Status No HNP session\r\n")));
                m_pOTG->intr_rx = 0;
                m_pOTG->intr_tx = 0;
            }
            m_pOTG->intr_usb = 0;
            ...
            {
                RETAILMSG(0, (TEXT("OTG Status connect\r\n")));
                m_dwStatus &= ~STATUS_CONNECT;
                if (m_pOTG->operateMode == HOST_MODE)
                    m_timeout = DO_USBHOST_TIMEOUT;
                else
            ...
            if ((m_dwStatus & STATUS_DISCONN_COMPLETE) && (m_pOTG->connect_status & CONN_DC))
            {
                ...
                {
                    m_bRequestSession = FALSE;
                    RETAILMSG(1, (TEXT("Request Session Again\r\n")));
                    SessionRequest(TRUE, TRUE);
                }
                else
                {
                    ...
                    RETAILMSG(1, (TEXT("Disconnect Complete and Set IOCTRL again\r\n")));
                }
                ...
                if (m_dwStatus & STATUS_HNP_SESSION_INIT)
                {
                    ...
                    m_dwStatus |= STATUS_HNP_SESSION_PROCESS;
                    m_dwStatus &= ~STATUS_HNP_SESSION_INIT;
                    RETAILMSG(1, (TEXT("m_bHNPSession set interrupt\r\n")));
                    m_timeout = INFINITE;
                }
            }
...
extern "C" BOOL OTGUSBClock(BOOL fStart)
{
    ...
    DEBUGMSG(0, (TEXT("OTGUSBClock(%d) called\r\n"), fStart));
    if (gpHsUsbOtg)
    {
        if (fStart)
