...
BOOL ReadFIFO(MUsbFnPdd_t *pPdd, UCHAR endpoint, void *pData, DWORD size)
{
    ...
    volatile ULONG *pReg = &pGenRegs->fifo[endpoint];
...
BOOL WriteFIFO(MUsbFnPdd_t *pPdd, UCHAR endpoint, void *pData, DWORD size)
{
    ...
    // Critical section would be handled outside
    DEBUGMSG(ZONE_INFO, (TEXT("WriteFIFO: total (0x%x), remain (0x%x), size(0x%x)\r\n"), total, remain, size));
...
DWORD Device_ResetIRQ(PVOID pHSMUSBContext)
{
    ...
    RETAILMSG(1, (TEXT("+Device_ResetIRQ\r\n")));
...
DWORD Device_Disconnect(PVOID pHSMUSBContext)
{
    ...
    {
		RETAILMSG(1, (TEXT("MUSBFN: Wait while active\r\n")));
    ...
    {
        RETAILMSG(1, (TEXT("MUSBFN: Client Mode Complete Disconnect\r\n")));
        pOTG->connect_status |= CONN_DC;
        SetEvent(pOTG->hSysIntrEvent);
    }
    
    RETAILMSG(1, (TEXT("MUSBFN: Disconnected\r\n")));
    return ERROR_SUCCESS;
}
//------------------------------------------------------------------------------
...
{
    MUsbFnPdd_t *pPdd = pPddContext;
    RETAILMSG(1, (L"UfnPdd_DeregisterDevice\r\n"));
...
)
{
    ...
    PCSP_MUSB_CSR_REGS pCsrRegs = pPdd->pUSBContext->pUsbCsrRegs;
    PHSMUSB_T pOTG = pPdd->pUSBContext;
    RETAILMSG(1, (L"UfnPdd_DeinitEndpoint: endpoint %d\r\n", endpoint));
...
)
{
    ...
    RETAILMSG(1, (L"UfnPdd_InitEndpoint: endpoint %d  speed: %d  MaxPacketSize: %d\r\n",
...
)
{
    ...
    RETAILMSG(1, (L"UsbFnPdd_RegisterDevice\r\n"));
...
)
{
    ...
    RETAILMSG(1, (TEXT("UfnPdd_Init called\r\n")));
    // Allocate PDD object
