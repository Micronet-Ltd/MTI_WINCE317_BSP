...
typedef enum OMAP_DSS_PIXELFORMAT {
    ...
    OMAP_DSS_PIXELFORMAT_RGB24      = DISPC_PIXELFORMAT_RGB24,
...
public:
    ...
    //
    BOOL    InitController(BOOL bEnableGammaCorr, BOOL bEnableWaitForVerticalBlank);
    BOOL    InitLCD();
    ...
    BOOL    SetPowerLevel(DWORD dwPowerLevel, int hld_bkl=0);
...
private:
    ...
    // DSS clock manager registers
    OMAP_PRCM_DSS_CM_REGS *m_pPrcmDssCMRegs;

	// PRCM CLK control
	OMAP_PRCM_CLOCK_CONTROL_CM_REGS  *m_pPrcmClkCM;
	OMAP_PRCM_CLOCK_CONTROL_PRM_REGS *m_pPrcmClkPRM;
