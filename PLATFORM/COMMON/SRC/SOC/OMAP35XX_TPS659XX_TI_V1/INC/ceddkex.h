...
//------------------------------------------------------------------------------
#define IOCTL_FMD_QUERY_FORMAT          IOCTL_DISK_USER(IOCTL_DISK_USER_END)
...
//------------------------------------------------------------------------------
#define IOCTL_HAL_SHUTDOWN  CTL_CODE(FILE_DEVICE_HAL, 2202, METHOD_BUFFERED, FILE_ANY_ACCESS)

BOOL OALIoCtlHalShutdown( UINT32 code,  VOID *pInpBuffer, UINT32 inpSize, VOID *pOutBuffer, UINT32 outSize,  UINT32 *pOutSize  );

//------------------------------------------------------------------------------
#define IOCTL_HAL_GET_HW_BOOT_STAT CTL_CODE(FILE_DEVICE_HAL, 2102, METHOD_BUFFERED, FILE_ANY_ACCESS)

BOOL HalQueryHwBootStat(VOID *pInBuffer, UINT32 inSize, VOID *pOutBuffer, UINT32 outSize, UINT32 *pOutSize);

//------------------------------------------------------------------------------
#define IOCTL_HAL_GET_HW_BOOT_STAT_EX CTL_CODE(FILE_DEVICE_HAL, 2103, METHOD_BUFFERED, FILE_ANY_ACCESS) //Includes Thermal shutdown status

//------------------------------------------------------------------------------
// Function name	: RegCopyValues
// Description	    : Copies registry values from source to destination
//                    
// Return type		: If the function succeeds, ERROR_SUCCESS is returned; otherwise, error code is returned 
// Arguments        : Source - source registry key
//                  : Destination - destination registry key
// !Note!			: Value name len (incl.0) <= REG_VALUE_NAME_LEN
//					: Data len <= REG_VALUE_LEN
//------------------------------------------------------------------------------
LONG RegCopyValues(TCHAR *Source, TCHAR *Destination);
...
//
#define IOCTL_DVFS_OPPNOTIFY  \
        CTL_CODE(FILE_DEVICE_DVFS, 0x0201, METHOD_BUFFERED, FILE_ANY_ACCESS)
...
//
#define IOCTL_TIPMX_CONTEXTPATH  \
        CTL_CODE(FILE_DEVICE_TIPMX, 0x0101, METHOD_BUFFERED, FILE_ANY_ACCESS)
...
#define IOCTL_CONTEXT_RESTORE  \
        CTL_CODE(FILE_DEVICE_LIST, 0x2050, METHOD_BUFFERED, FILE_ANY_ACCESS)
