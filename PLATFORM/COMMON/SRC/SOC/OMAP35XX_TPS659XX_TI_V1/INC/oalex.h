...
#define OAL_ARGS_QUERY_HIVECLEAN        (BSP_ARGS_QUERY + 4)          // Query hive clean flag.
#define OAL_ARGS_QUERY_FORMATPART       (BSP_ARGS_QUERY + 5)          // Query format partition flag.
#define OAL_ARGS_QUERY_SW_UPDATE		(BSP_ARGS_QUERY + 6)          // Query sw update flags
#define OAL_ARGS_QUERY_MUNUFACT			(BSP_ARGS_QUERY + 7)          // Query manufacture block
#define OAL_ARGS_QUERY_EBOOT_VERS		(BSP_ARGS_QUERY + 8)          // Query eboot version
#define OAL_ARGS_QUERY_XLDR_VERS		(BSP_ARGS_QUERY + 9)          // Query xloader verrsion 
#define OAL_ARGS_QUERY_MAIN_CFG			(BSP_ARGS_QUERY + 10)         // Query Main board config and version 
#define OAL_ARGS_QUERY_LCD_CFG			(BSP_ARGS_QUERY + 11)         // Query Display board config and version 
#define OAL_ARGS_QUERY_WLAN_CFG			(BSP_ARGS_QUERY + 12)         // Query WLAN board config and version 
#define OAL_ARGS_QUERY_CAN_CFG			(BSP_ARGS_QUERY + 13)         // Query CAN board config and version 
#define OAL_ARGS_QUERY_OALFLAGS			(BSP_ARGS_QUERY + 14)         //
#define OAL_ARGS_QUARY_HW_BOOT_STAT		(BSP_ARGS_QUERY + 15)         //
#define OAL_ARGS_QUERY_GPS_CFG			(BSP_ARGS_QUERY + 16)         // Query GPS board config and version 
#define OAL_ARGS_QUERY_MODEM_CFG		(BSP_ARGS_QUERY + 17)         // Query MODEM board config and version 
#define OAL_ARGS_QUERY_ETHMAC			(BSP_ARGS_QUERY + 18)         // Query Ethernet MAC Address 
#define OAL_ARGS_QUERY_EXTAUDIO_CFG		(BSP_ARGS_QUERY + 19)         // Query External Audio board config and version
#define OAL_ARGS_QUERY_WIFIMAC			(BSP_ARGS_QUERY + 20)         // Query WiFi MAC Address 
#define OAL_ARGS_QUERY_CDSPARAMS		(BSP_ARGS_QUERY + 21)         // Query CDS (CPU Dynamic Speed) parameters. 
#define OAL_ARGS_QUERY_THERMALSHUTDOWN	(BSP_ARGS_QUERY + 22)         // Query Cause of last shutdown to be MPU Thermal Shutdown
...
#define CPU_FAMILY_35XX     0
#define CPU_FAMILY_37XX     1
...
//#define CPU_FAMILY_35XX_REVISION_ES_3_0     3
//#define CPU_FAMILY_35XX_REVISION_ES_3_1     4

//#define CPU_REVISION_UNKNOWN                (-1)
...
// pInpBuffer points to DWORD with value to select registers to dump
...
//-----------------------------------------------------------------------------
//
//  Define: IOCTL_HAL_QUERY_CARD_CFG
//
//  This code is used to get the configured speed of the IVA2 (DSP) subsystem
//
#define IOCTL_HAL_QUERY_CARD_CFG CTL_CODE(FILE_DEVICE_HAL, 3110, METHOD_BUFFERED, FILE_ANY_ACCESS)

BOOL OALIoCtlHalQueryCardConfig(UINT32 code, VOID *pInBuffer,UINT32 inSize, VOID *pOutBuffer, UINT32 outSize, UINT32 *pOutSize);

//-----------------------------------------------------------------------------
//
//  Define: IOCTL_HAL_RTC_UPDATE
//
//  This code is used to call from TWL(Triton) chip driver to synchronize
//  RTC value. This synchronization usually happen each minute or hour
//  (depending on device power state).
//
#define IOCTL_HAL_RTC_UPDATE    \
    CTL_CODE(FILE_DEVICE_HAL, 1024, METHOD_BUFFERED, FILE_ANY_ACCESS)

typedef struct {
    UINT32      offset;         // Secure offset from registry
    UCHAR       time[6];        // RTC date/time from Tritron2
} IOCTL_HAL_RTC_UPDATE_IN;
...
//
//  This code is used to call from TWL(Triton) chip driver on each interrupt.
//  It will give OAL RTC code to get current time or alarm set.
//
#define IOCTL_HAL_RTC_QUERY     \
    CTL_CODE(FILE_DEVICE_HAL, 1026, METHOD_BUFFERED, FILE_ANY_ACCESS)

#define IOCTL_HAL_RTC_QUERY_SET_TIME      (1 << 0)
#define IOCTL_HAL_RTC_QUERY_SET_ALARM     (1 << 1)

typedef struct {
    UINT32      flags;          // Update type
    UCHAR       time[6];        // Updated time 
    UCHAR       alarm[6];       // Updated alarm 
} IOCTL_HAL_RTC_QUERY_OUT;
...
//  Define: IOCTL_HAL_PRCMDEVICEENABLECLOCKS
//
//  Allow call PrcmDeviceEnableClocks from driver
//

#define IOCTL_HAL_PRCMDEVICEENABLECLOCKS  \
    CTL_CODE(FILE_DEVICE_HAL, 1040, METHOD_BUFFERED, FILE_ANY_ACCESS)

typedef struct {
    UINT devId;
    BOOL bEnable;
} IOCTL_HAL_PRCMDEVICEENABLECLOCKS_IN;
...
//  Define: IOCTL_HAL_OALPATOVA
//
//  Allow call OALPAtoVA from driver
//

#define IOCTL_HAL_OALPATOVA  \
    CTL_CODE(FILE_DEVICE_HAL, 1041, METHOD_BUFFERED, FILE_ANY_ACCESS)

typedef struct {
    UINT32 pa;
    BOOL cached;
} IOCTL_HAL_OALPATOVA_IN;
...
//  Define: IOCTL_HAL_OALGETVIIRQ
//
//  Allow call OALgetVIIRQ from driver
//

#define IOCTL_HAL_OALGETVIIRQ  \
    CTL_CODE(FILE_DEVICE_HAL, 1042, METHOD_BUFFERED, FILE_ANY_ACCESS)
...
#define IOCTL_HAL_GET_ETH_MACADDR  \
    CTL_CODE(FILE_DEVICE_HAL, 3111, METHOD_BUFFERED, FILE_ANY_ACCESS)
...
#define IOCTL_HAL_GET_WIFI_MACADDR  \
    CTL_CODE(FILE_DEVICE_HAL, 3112, METHOD_BUFFERED, FILE_ANY_ACCESS)
//-----------------------------------------------------------------------------
//
//  Function:  OALIoCtlHalGetWiFiMacAddr
//
BOOL
OALIoCtlHalGetWiFiMacAddr(
    UINT32 code, 
    VOID *pInBuffer,
    UINT32 inSize, 
    VOID *pOutBuffer, 
...
#define IOCTL_HAL_GET_CDS_PARAMS  \
    CTL_CODE(FILE_DEVICE_HAL, 3113, METHOD_BUFFERED, FILE_ANY_ACCESS)
...
#define IOCTL_HAL_SET_MPU_SPEED  \
    CTL_CODE(FILE_DEVICE_HAL, 3114, METHOD_BUFFERED, FILE_ANY_ACCESS)
