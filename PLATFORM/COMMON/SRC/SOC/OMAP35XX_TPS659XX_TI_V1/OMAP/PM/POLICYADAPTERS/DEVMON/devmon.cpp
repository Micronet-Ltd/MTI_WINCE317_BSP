    ...
    )
{
    ...
    if (pDeviceMap->powerDomain != POWERDOMAIN_NULL)
        ...
        if (opm != s_DeviceMonitorInfo.currentOpm)
            {
            //RETAILMSG(1, (L"DEVMON: CONSTRAINT_MSG_DVFS_REQUEST OPM%d\r\n", opm));
    ...
    )
{
    ...
    // set DEVMON DVFS constraint to undefined (no contraint)
    ...
    // set DEVMON CORE domain constraint to undefined (no contraint)
