...
{
    //RETAILMSG(1, (L"SYSTEMSTATE: CONSTRAINT_MSG_DVFS_REQUEST OPM%d\r\n", dwOpm));
    PmxUpdateConstraint(
...
//-----------------------------------------------------------------------------
// 
//  Function:  SystemStateThreadFn
//
//  Contains the Power state transition listener
//
DWORD WINAPI
SystemStateThreadFn(LPVOID pvParam)
{
    union
    {
    ...
    msgOptions.cbMaxMessage = sizeof(POWER_BROADCAST) + (MAX_PATH * sizeof(TCHAR));
	    ...
	    if (s_SystenStatePolicyInfo.hPowerNotificationsHandle)
	    ...
	    else
	        {
   	        RETAILMSG(ZONE_ERROR,(L"System State Policy:"
	            L" ERROR: PM is not ready, retry:%d\r\n", 11-retry));
			retry--;
			Sleep(5000);
			}
		}
    ...
    if (retry == 0)
        {
        RETAILMSG(ZONE_ERROR,(L"System State Policy:"
            L" ERROR: Power Notification request fail (0x%x)\r\n", GetLastError()));
        goto cleanUp;
        }
