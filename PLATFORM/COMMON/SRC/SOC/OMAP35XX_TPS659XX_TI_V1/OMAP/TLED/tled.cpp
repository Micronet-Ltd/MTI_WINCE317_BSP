...
static const DEVICE_REGISTRY_PARAM s_deviceRegParams[] = {
    {
        ...
        L"MinValue", PARAM_DWORD, FALSE, offset(Device_t, minChannelValue),
        fieldsize(Device_t, minChannelValue), (VOID*)1
    }
};
    ...
    )
{
	...
	//if(offTick <= pDevice->minChannelValue)
	//	offTick = pDevice->minChannelValue + 1;
	if(offTick < pDevice->minChannelValue)
		offTick = pDevice->minChannelValue;
...
//  channel averaging.
//
BOOL InitializeDevice(Device_t *pDevice)
{
    BOOL rc = FALSE;
    UINT8 val;

    DEBUGMSG(ZONE_FUNCTION, (L"TLD:  +InitializeDevice(0x%08x)\r\n", pDevice));

	// disable vibra
	if(!TWLReadRegs(pDevice->hTWL, TWL_VIBRA_CTL, &val, 1))
	{
        DEBUGMSG(ZONE_ERROR, (L"TLD: !ERROR - Failed to initialize TWL_VIBRA_CTL register\r\n"));
        goto cleanUp;
	}

	val &= 0x7E;
	if(!TWLWriteRegs(pDevice->hTWL, TWL_VIBRA_CTL, &val, 1))
	{
        DEBUGMSG(ZONE_ERROR, (L"TLD: !ERROR - Failed to initialize TWL_VIBRA_CTL register\r\n"));
        goto cleanUp;
	}
    ...
    // disable all leds
	// Vladimir: Enable leds - The Marketing requirements are backlight will on during startup
	val = 0x11;
...
#if 0
	// PWM MUX
	if(!TWLReadRegs(pDevice->hTWL, TWL_PMBR1, &val, 1))
	{
        DEBUGMSG(ZONE_ERROR, (L"TLD: !ERROR - Failed to initialize PMBR register\r\n"));
        goto cleanUp;
	}

	val |= ((3 << 4) | (1 << 2));
	if(!TWLWriteRegs(pDevice->hTWL, TWL_PMBR1, &val, 1))
	{
        DEBUGMSG(ZONE_ERROR, (L"TLD: !ERROR - Failed to initialize PMBR register\r\n"));
        goto cleanUp;
	}
	...
	// PWM enable
	if(!TWLReadRegs(pDevice->hTWL, TWL_GPBR1, &val, 1))
	{
        DEBUGMSG(ZONE_ERROR, (L"TLD: !ERROR - Failed to initialize GPBR register\r\n"));
        goto cleanUp;
	}

	val |= (PWM0_ENABLE | PWM1_ENABLE | PWM0_CLK_ENABLE | PWM1_CLK_ENABLE);
	if(!TWLWriteRegs(pDevice->hTWL, TWL_GPBR1, &val, 1))
	{
        DEBUGMSG(ZONE_ERROR, (L"TLD: !ERROR - InitializeDevice Failed to initialize GPBR register\r\n"));
        goto cleanUp;
	}
#endif
	...
	// Vladimir: Enable leds - The Marketing requirements are backlight will on during startup
    val = 0;	//(UINT8)pDevice->minChannelValue;
    if(TWLWriteRegs(pDevice->hTWL, TWL_PWMAON, &val, 1) == FALSE)
	{
        RETAILMSG(ZONE_ERROR, (L"TLD:  !ERROR - InitializeDevice Failed to initialize TWL_PWMAON\r\n"));
        goto cleanUp;
	}
