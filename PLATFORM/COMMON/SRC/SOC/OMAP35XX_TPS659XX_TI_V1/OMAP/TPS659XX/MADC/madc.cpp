...
static const DEVICE_REGISTRY_PARAM s_deviceRegParams[] = {
    {
        ...
        fieldsize(Device_t, GPChAvgEnable), (VOID*)0x4
    },{
...
{
    UINT8 val;
    DEBUGMSG(ZONE_FUNCTION, (L"ADC: +InitializeDevice(0x%08x)\r\n", pDevice));

    BOOL rc = FALSE;

	if(!TWLReadRegs(pDevice->hTWL, TWL_GPBR1, &val, 1))
	{
        DEBUGMSG(ZONE_ERROR, (L"ADC: !ERROR - InitializeDevice Failed to initialize GPBR register\r\n"));
        goto cleanUp;
	}
	val |= (MADC_HFCLK_EN | DEFAULT_MADC_CLK_EN);
    ...
    // ##### Michael, 4-Nov-2009
	// Add initialization of 'TWL_CARKIT_ANA_CTRL' register
	// Allow using ADCIN3-ADCIN6 as external A2Ds
	val = DEFAULT_MADC_SEL_MCPC;
	if(!TWLWriteRegs(pDevice->hTWL, TWL_CARKIT_ANA_CTRL_SET, &val, 1))
        {
	...
	if(!TWLWriteRegs(pDevice->hTWL, TWL_SW1AVERAGE_LSB, &(pDevice->GPChAvgEnable), 2))
        {
        DEBUGMSG(ZONE_ERROR, (L"ADC: !ERROR - InitializeDevice "
            L"Failed to initialize GP average registers\r\n"
            ));
    ...
    )
{
	...
	pDevice->GPChAvgEnable = DEFAULT_MADC_GPCH_MASK;
