...
{    
    ...
    RETAILMSG(0, (TEXT("ProcessSecondaryInterrupts USB_INT (0x%x), OTHER_INT (0x%x)\r\n"), data[0], data[1]));
...
//  generic routine which processes the n.
//
BOOL ProcessSecondaryInterrupts(Device_t *pDevice, HANDLE *rgEvents, SIHEntry_t const *pSIHEntry)
{
    UINT8 status;
    UINT8 clearMask;    
    int interruptCount;
    ...
    )
{
    ...
    pDevice->bIntrInitialized = GPIOInterruptInitialize(pDevice->hGpio, pDevice->gpio, pDevice->hIntrEvent);
    if (pDevice->bIntrInitialized == FALSE)
        {
        DEBUGMSG (ZONE_ERROR, (L"ERROR: TWL_Init: "
    ...
    )
{
    ...
    // Disable interrupt
    if (pDevice->bIntrInitialized == TRUE)
        {
        GPIOInterruptDisable(pDevice->hGpio, pDevice->gpio);
        GPIOInterruptRelease(pDevice->hGpio, pDevice->gpio);
        }
...
DWORD TWL_InterruptThread(VOID *pContext)
{
	...
	while(!pDevice->intrThreadExit)
	{
        ...
        GPIOInterruptDone(pDevice->hGpio, pDevice->gpio);
	}
    ...
    )
{
    ...
    if (!!((pStatusRegister->ffWakeupEnable) & (1 << TWL_MASKBIT(intrId))) != !bEnable)
        ...
        if (pDevice->wakeupCount == 1)
            {
            if (!GPIOWakeEnable(pDevice->hGpio, pDevice->gpio))
                {
            ...
            {
            if (!GPIOWakeDisable(pDevice->hGpio, pDevice->gpio))
                {
                DEBUGMSG(ZONE_ERROR, (L"WARN: TWL_EnableWakeup: "
                    L"Failed enable keyboard as wakeup source\r\n"
                    ));
    ...
    )
{
    ...
    if (pInstance == NULL || pInstance->cookie != TWL_INSTANCE_COOKIE)
        {
        RETAILMSG(1, (L"ERROR: TWL_IOControl: "
            L"Incorrect context instance\r\n"
            ));
        goto cleanUp;
        }
    ...
    switch (code)
        ...
        case IOCTL_TWL_INTRMASK:
            if (pInBuffer != NULL && inSize == sizeof(IOCTL_TWL_INTRMASK_IN))
                ...
                rc = TWL_InterruptMask(context, pInParam->intrId, pInParam->bEnable);
                }
            break;
