...
typedef struct {
	void (*ecc_init)(OMAP_GPMC_REGS *pGpmcRegs, UINT configMask, UINT xfer_mode);
	void (*ecc_calculate)(OMAP_GPMC_REGS *pGpmcRegs, BYTE *pEcc,int size);	
	void (*ecc_reset)(OMAP_GPMC_REGS *pGpmcRegs);
	BOOL (*ecc_correct_data)(OMAP_GPMC_REGS *pGpmcRegs,
...
VOID ECC_Init(OMAP_GPMC_REGS *pGpmcRegs, UINT configMask, EccType_e ECC_mode, UINT xfer_mode);
VOID ECC_Result(OMAP_GPMC_REGS *pGpmcRegs, BYTE *pEcc, int size);
VOID ECC_Reset(OMAP_GPMC_REGS *pGpmcRegs);
BOOL ECC_CorrectData(OMAP_GPMC_REGS *pGpmcRegs,
					 BYTE *pData,                // Data buffer
					 int sizeData,               // Count of bytes in data buffer
					 BYTE const *pEccOld,        // Pointer to the ECC on flash
					 BYTE const *pEccNew         // Pointer to the ECC the caller calculated
