...
//-----------------------------------------------------------------------------
static VOID BCH4_ECC_Calculate(OMAP_GPMC_REGS *pGpmcRegs, BYTE *pEcc, int size);
static VOID BCH8_ECC_Calculate(OMAP_GPMC_REGS *pGpmcRegs, BYTE *pEcc, int size);
//-----------------------------------------------------------------------------
...
//
static __inline UCHAR CountNumberOfOnes(DWORD num)
{
    UCHAR count = 0;
    while(num)
        {
...
//-----------------------------------------------------------------------------
static VOID Hamming_ECC_Init(OMAP_GPMC_REGS *pGpmcRegs, UINT configMask, UINT xfer_mode)
{
...
//-----------------------------------------------------------------------------
static VOID Hamming_ECC_Calculate(OMAP_GPMC_REGS *pGpmcRegs, BYTE *pEcc, int size)
{
    UINT    regIndex = 0;
    UINT8   eccIndex;
    UINT32  regVal;
    ...
    for (i = 0; i < blockCnt; i++) 
    {
        ...
        for (j = 0; (j < eccsize) && (eccCheck == 0); j++)
		{
            if(calc_ecc[j] != 0)
			{
				/*
				int iii;
				RETAILMSG(1, (L"BCH_correct_data: BCH4 result(block %d)\r\n", i));
				for(iii = 0; iii < eccsize; iii++)
				{
					RETAILMSG(1, (L" %x", calc_ecc[iii]));
				}
				RETAILMSG(1, (L"\r\n"));
                */
				eccCheck = 1;
        ...
        if (eccCheck == 1) 
        {
            ...
            for(j = 0; (j < eccsize) && (eccCheck == 0); j++)
			{
                if(read_ecc[j] != 0xFF)
				{
					/*
					int iii;
					RETAILMSG(1, (L"BCH_correct_data: stored BCH4\r\n"));
					for(iii = 0; iii < eccsize; iii++)
					{
						RETAILMSG(1, (L" %x", read_ecc[iii]));
					}
					RETAILMSG(1, (L"\r\n"));
					*/
                    eccCheck = 1;
        ...
        if (eccCheck == 1) 
        {
            ...
            RETAILMSG(1, (L"...bch correct(%d 512 byte) done, count=%d\r\n", i+1, count));
