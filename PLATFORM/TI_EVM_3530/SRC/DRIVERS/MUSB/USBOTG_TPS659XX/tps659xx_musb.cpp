...
void HSUSBOTGTransceiverTps659xx::Configure()
{
    ...
    WriteOmapTriton2(TWL_FUNC_CTRL_SET, FUNC_CTRL_RESET);
	do
	{
		ReadOmapTriton2(TWL_FUNC_CTRL, &bModeBits);
	}while(bModeBits & FUNC_CTRL_RESET);

	ReadOmapTriton2(TWL_ID_STATUS, &bModeBits);
	RETAILMSG(1, (TEXT("ID pin STATUS %x\r\n"), bModeBits));
...
//    WriteOmapTriton2(TWL_VBUS_DEBOUNCE,0xF); // This change affects CETK tests.
	...
	// Set DM/DP PULLDOWN bits
    WriteOmapTriton2(TWL_OTG_CTRL_SET, (OTG_CTRL_DM_PULLDOWN | OTG_CTRL_DP_PULLDOWN));
    // Setting to normal operation mode
    WriteOmapTriton2(TWL_FUNC_CTRL_CLR, FUNC_CTRL_OPMODE_MASK | FUNC_CTRL_XCVRSELECT_MASK | FUNC_CTRL_TERMSELECT);
    WriteOmapTriton2(TWL_FUNC_CTRL_SET, FUNC_CTRL_TERMSELECT);
...
void HSUSBOTGTransceiverTps659xx::Resume()
{
}
...
BOOL HSUSBOTGTransceiverTps659xx::IsADeviceConnected()
{
	...
	{
        // A device detected
		RETAILMSG(1, (TEXT("%S: VBUS suplied (VBUS_PRES)\r\n"), __FUNCTION__));
        return 1;
	}
...
BOOL HSUSBOTGTransceiverTps659xx::IsBDeviceConnected()
{
	...
	{
        // B device detected
		RETAILMSG(1, (TEXT("%S: USB connector plugged (ID_PRES)\r\n"), __FUNCTION__));
        return 1;
	}
...
void HSUSBOTGTransceiverTps659xx::AconnNotifHandle(HANDLE hAconnEvent)
{
    m_hAconnEvent = hAconnEvent;
    DEBUGMSG(ZONE_OTG_FUNCTION, (TEXT("HSUSBOTGTransceiverTriton2::AconnNotifHandle (0x%x)\r\n"), hAconnEvent));
}

void HSUSBOTGTransceiverTps659xx::EnableWakeupInterrupt(BOOL fEnable)
{    
    DEBUGMSG(ZONE_OTG_FUNCTION, (TEXT("HSUSBOTGTransceiverTriton2::EnableWakeupInterrupt (%d)\r\n"), fEnable));

	TWLInterruptMask(m_hTriton2Handle, TWL_INTR_USB_PRES, !fEnable);
    TWLWakeEnable(m_hTriton2Handle, TWL_INTR_USB_PRES, fEnable);
}


DWORD HSUSBOTGTransceiverTps659xx::StateChangeThread(void* pvData)
{
    ...
    for(;;)
    {
		...
		BYTE val = 0;
		// Check if a host/device is connected
		me->ReadOmapTriton2(TWL_STS_HW_CONDITIONS, &val);

        // usb presence interrupt
        //
		RETAILMSG(1, (L"########### USB Presence interrupt VBUS_PRES/A_DEVICE(%x) ID_PRES/B_DEVICE(%x)\r\n", (val & TWL_STS_VBUS)>>7, (val & TWL_STS_USB)>>2));                                    
...
HSUSBOTGTransceiverTps659xx::HSUSBOTGTransceiverTps659xx(PHSMUSB_T pOTG)
{
    ...
    {
        DEBUGMSG(ZONE_OTG_ERROR, (L"HSUSBOTGTransceiver::HSUSBOTGTransceiver() "
            L"Failed to unmask TWL_INTR_USB_PRES interrupt\r\n"
            ));
    }
...
BOOL HSUSBOTGTransceiverTps659xx::UpdateUSBVBusSleepOffState(BOOL fActive)
{
    ...
    if(fActive)
    {
        ...
        WriteOmapTriton2(TWL_CLKEN_REMAP, state);
        ...
        WriteOmapTriton2(TWL_HFCLKOUT_REMAP, state);
...
/*BOOL HSUSBOTGTransceiverTps659xx::LeaveLowPowerMode()
{
	SetT2USBXvcrPowerOff(0);
	EnableFClock(1);
	UpdateUSBVBusSleepOffState(1);
	ResetPHY();
	SetVBusSource(1);
	Configure();
    RETAILMSG(1, (L"TPS659XX Leave low power mode\r\n"));
...
{
//    RETAILMSG(1, (L"TPS659XX Enter low power mode\r\n"));
//	UpdateUSBVBusSleepOffState(0);
//	EnableFClock(0);
//	SetT2USBXvcrPowerOff(1);
