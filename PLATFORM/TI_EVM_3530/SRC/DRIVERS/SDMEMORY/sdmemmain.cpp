...
extern "C" BOOL WINAPI SMC_Deinit(DWORD hDeviceContext)
{
    ...
    RETAILMSG(1, (TEXT("SDMemory: Deinit\n")));
...
extern "C" DWORD WINAPI SMC_Init(DWORD dwContext)
{
    ...
    RETAILMSG(1, (TEXT("SDMemory: +SMC_Init\r\n")));

    pDevice = (PSD_MEMCARD_INFO)SDAllocateMemoryWithTag(
        sizeof(SD_MEMCARD_INFO),
        SD_MEMORY_TAG);
    if (pDevice == NULL) {
        RETAILMSG(1, (TEXT("SDMemory: Failed to allocate device info\r\n")));
        RETAILMSG(1, (TEXT("SDMemory: -SMC_Init\r\n")));
        return 0;
    }
    ...
    if (NULL == hClientHandle) {
        RETAILMSG(1, (TEXT("SDMemory: Failed to get client handle\r\n")));
        CleanUpDevice(pDevice);
        RETAILMSG(1, (TEXT("SDMemory: -SMC_Init\r\n")));
        return 0;
    }
    ...
    pDevice->pSterileIoRequest = (PSG_REQ)LocalAlloc(LPTR, (sizeof(SG_REQ) + ((MAX_SG_BUF - 1) * sizeof(SG_BUF))));
    if (NULL == pDevice->pSterileIoRequest) {
        RETAILMSG(1, (TEXT("SDMemory: Failed to allocate sterile I/O request\r\n")));
        CleanUpDevice(pDevice);
        RETAILMSG(1, (TEXT("SDMemory: -SMC_Init\r\n")));
        return 0;
    }
    ...
    if (!SD_API_SUCCESS(Status)) {
        RETAILMSG(1, (TEXT("SDMemory: Failed to register client : 0x%08X\r\n"),Status));
        CleanUpDevice(pDevice);
        RETAILMSG(1, (TEXT("SDMemory: -SMC_Init\r\n")));
        return 0;
    }
    ...
    if( SDMemCardConfig( pDevice ) != ERROR_SUCCESS ) {
        CleanUpDevice(pDevice);
        RETAILMSG( 1, (TEXT("SDMemory: Error initializing MemCard structure and card\r\n")));
        return 0;
    }
    ...
    if (RegOpenKeyEx(
        ...
        if (pDevice->BlockTransferSize != DEFAULT_BLOCK_TRANSFER_SIZE) {
            RETAILMSG(1, (TEXT("SDMemory: Initialize: Using block transfer size of %d blocks\r\n"),
        ...
        if (RegQueryValueEx(
            ...
            RETAILMSG(1, (TEXT("SDMemory: Initialize: Using single block write commands only\r\n")));
        }
        ...
        if (RegQueryValueEx(
            ...
            // key is present
            RETAILMSG(1, (TEXT("SDMemory: Initialize: Disabled power management\r\n")));
        ...
        if (RegQueryValueEx(
            ...
            // key is present
            RETAILMSG(1, (TEXT("SDMemory: Initialize: Using idle timeout of %u milliseconds\r\n"),
                pDevice->IdleTimeout));
        }
    ...
    if (pDevice->hBufferList == NULL) {
        RETAILMSG(1, (TEXT("SDMemory: Initialize: Failed to allocate buffer list\r\n")));
        CleanUpDevice(pDevice);
        return 0;
    }
    ...
    switch(IoctlCode) {
        ...
        case DISK_IOCTL_READ:
            ...
            if (0 == CeSafeCopyMemory((LPVOID)pHandle->pSterileIoRequest, (LPVOID)pSG, InBufSize)) {
                Status = ERROR_INVALID_PARAMETER;
				RETAILMSG(1, (L"SDMEM::READ: Invalid input\r\n"));
                break;
            }
            ...
            __except(EXCEPTION_EXECUTE_HANDLER) {
				RETAILMSG(1, (L"SDMEM::READ: Exception\r\n"));
                Status = ERROR_INVALID_PARAMETER;
            }
			RETAILMSG(ERROR_SUCCESS != Status, (L"SDMEM::READ: Error %X\r\n", Status));
            break;
        ...
        case IOCTL_POWER_SET:
        {
            ...
            if (0 == CeSafeCopyMemory((LPVOID)&SafeCeDevicePowerState, (LPVOID)pOutBuf, sizeof(CEDEVICE_POWER_STATE))) {
                Status = ERROR_INVALID_PARAMETER;
				RETAILMSG(1, (L"SDMEM::SETPOWER: Invalid input\r\n"));
                break;
            }
			RETAILMSG(1, (L"SDMEM::SETPOWER: D%d\r\n", SafeCeDevicePowerState));
            Status = ERROR_SUCCESS;
            ...
            if (0 == CeSafeCopyMemory((LPVOID)pOutBuf, (LPVOID)&SafeCeDevicePowerState, sizeof(CEDEVICE_POWER_STATE))) {
				RETAILMSG(1, (L"SDMEM::SETPOWER: Invalid output\r\n"));
                Status = ERROR_INVALID_PARAMETER;
                break;
            }
            if (pBytesReturned) {
