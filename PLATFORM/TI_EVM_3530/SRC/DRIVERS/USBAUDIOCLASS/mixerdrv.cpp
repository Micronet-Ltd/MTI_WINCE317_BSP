...
#define DEVICE_NAME     L"USB Audio Mixer"
#define DRIVER_VERSION  0x100

#define MAXMIXERCONTROLS  8
...
COMPTYPE(DWORD dwValue)
{
    ...
    for (Index = 0; Index < _countof(ctype_table); Index++)
    {
...
g_mixerline[] =
{
    ...
    MXLED(LINE_OUT, 0,
          ...
          2, // stereo
          3, // controls: volume, mute, SPDIF, SPDIF-WmaPro, eq preset
    ...
    MXLED(PCM_IN, 1,
          ...
          g_PCM_IN_sources, _countof(g_PCM_IN_sources)
         ),
    ...
    MXSLE(MXLINEID(PCM_IN, MIC), 1, 1,
          ...
          1, // mono
          4, // controls: volume, mute, gain, AGC
          "Microphone","Microphone",
          MIXERLINE_TARGETTYPE_WAVEIN
         ),

};
const int nlines = _countof(g_mixerline);
#undef _MXLE
#undef MXLED
#undef MXSLE
...
{
public:
    MixerControl(HardwareContext* pHWContext) : m_dwControlID(-1) { m_pHWContext = pHWContext; }
    ...
    {
        ZeroMemory(pamxctrl, sizeof(*pamxctrl));
        pamxctrl->cbStruct = sizeof(MIXERCONTROL);
        pamxctrl->dwControlID = GetControlID();
        pamxctrl->dwControlType = GetControlType();
    }

    virtual DWORD GetControlDetails (PMIXERCONTROLDETAILS pDetail, DWORD dwFlags) {return MMSYSERR_NOTSUPPORTED;}
    virtual DWORD SetControlDetails (PMIXERCONTROLDETAILS pDetail, DWORD dwFlags) {return MMSYSERR_NOTSUPPORTED;}
...
{
public:
    MixerControlVolume(HardwareContext* pHWContext) : MixerControl(pHWContext) {}
...
{
public:
    MixerControlVolumeMaster(HardwareContext* pHWContext) : MixerControlVolume(pHWContext) {}

    void CopyMixerControl(PMIXERCONTROL pamxctrl)
    {
        MixerControlVolume::CopyMixerControl(pamxctrl);
        StringCbCopy(pamxctrl->szName, sizeof(pamxctrl->szName), TEXT("Master Volume"));
        StringCbCopy(pamxctrl->szShortName, sizeof(pamxctrl->szShortName), TEXT("Master Volume"));
    }
    USHORT GetLineID() {return MXLINEID(LINE_OUT,NOLINE);}

    void SetGain(DWORD dwSetting)
    {
        m_pHWContext->SetOutputGain(dwSetting);
    }

    DWORD GetGain()
    {
        return m_pHWContext->GetOutputGain();
    }
};

// Final class for master input volume
class MixerControlVolumeMic: public MixerControlVolume
{
public:
    MixerControlVolumeMic(HardwareContext* pHWContext) : MixerControlVolume(pHWContext) {}

    void CopyMixerControl(PMIXERCONTROL pamxctrl)
    {
        MixerControlVolume::CopyMixerControl(pamxctrl);
        StringCbCopy(pamxctrl->szName, sizeof(pamxctrl->szName), TEXT("Input Volume"));
        StringCbCopy(pamxctrl->szShortName, sizeof(pamxctrl->szShortName), TEXT("Input Volume"));
    }

    USHORT GetLineID() {return MXLINEID(PCM_IN,MIC);}

    void SetGain(DWORD dwSetting)
    {
        m_pHWContext->SetInputGain(dwSetting);
    }

    DWORD GetGain()
    {
        return m_pHWContext->GetInputGain();
    }
};

class MixerControlMicGain: public MixerControlVolume
{
public:
    MixerControlMicGain(HardwareContext* pHWContext) : MixerControlVolume(pHWContext) {}

    void CopyMixerControl(PMIXERCONTROL pamxctrl)
    {
        MixerControlVolume::CopyMixerControl(pamxctrl);
        StringCbCopy(pamxctrl->szName, sizeof(pamxctrl->szName), TEXT("Microphone Gain"));
        StringCbCopy(pamxctrl->szShortName, sizeof(pamxctrl->szShortName), TEXT("Mic Gain"));
		pamxctrl->Bounds.dwMinimum = m_pHWContext->GetMicGainMin();
        pamxctrl->Bounds.dwMaximum = m_pHWContext->GetMicGainMax();
        pamxctrl->Metrics.cSteps =  m_pHWContext->GetMicGainRes();
    }
    ...
    void SetGain(DWORD dwSetting)
    {
        m_pHWContext->SetMicGain(dwSetting);
    }

    DWORD GetGain()
    {
        return m_pHWContext->GetMicGain();
    }
};
...
{
public:
    MixerControlBoolean(HardwareContext* pHWContext) : MixerControl(pHWContext) {}
...
{
public:
    MixerControlMuteMaster(HardwareContext* pHWContext) : MixerControlBoolean(pHWContext) {}

    void CopyMixerControl(PMIXERCONTROL pamxctrl)
    {
        MixerControlBoolean::CopyMixerControl(pamxctrl);
        StringCbCopy(pamxctrl->szName, sizeof(pamxctrl->szName), TEXT("Master Mute"));
        StringCbCopy(pamxctrl->szShortName, sizeof(pamxctrl->szShortName), TEXT("Master Mute"));
    }
    DWORD GetControlType() {return MIXERCONTROL_CONTROLTYPE_MUTE;}
    USHORT GetLineID() { return MXLINEID(LINE_OUT,NOLINE); }
    BOOL GetValue() { return m_pHWContext->GetOutputMute(); }
    void SetValue(BOOL bMute) { m_pHWContext->SetOutputMute(bMute); }
};

// Final class for master input mute
class MixerControlMuteMic: public MixerControlBoolean
{
public:
    MixerControlMuteMic(HardwareContext* pHWContext) : MixerControlBoolean(pHWContext) {}

    void CopyMixerControl(PMIXERCONTROL pamxctrl)
    {
        MixerControlBoolean::CopyMixerControl(pamxctrl);
        StringCbCopy(pamxctrl->szName, sizeof(pamxctrl->szName), TEXT("Microphone Mute"));
        StringCbCopy(pamxctrl->szShortName, sizeof(pamxctrl->szShortName), TEXT("Mic Mute"));
    }

    USHORT GetLineID() { return MXLINEID(PCM_IN,MIC); }
    BOOL GetValue() { return m_pHWContext->GetInputMute(); }
    void SetValue(BOOL bMute) { m_pHWContext->SetInputMute(bMute); }
};

class MixerControlMicAGC: public MixerControlBoolean
{
public:
    MixerControlMicAGC(HardwareContext* pHWContext) : MixerControlBoolean(pHWContext) {}

    void CopyMixerControl(PMIXERCONTROL pamxctrl)
    {
        MixerControlBoolean::CopyMixerControl(pamxctrl);
        StringCbCopy(pamxctrl->szName, sizeof(pamxctrl->szName), TEXT("Microphone AGC"));
        StringCbCopy(pamxctrl->szShortName, sizeof(pamxctrl->szShortName), TEXT("Mic AGC"));
    }

    USHORT GetLineID() { return MXLINEID(PCM_IN,MIC); }
    BOOL GetValue() { return m_pHWContext->GetMicAGC(); }
    void SetValue(BOOL bEnable) { m_pHWContext->SetMicAGC(bEnable); }

	DWORD GetControlDetails (PMIXERCONTROLDETAILS pDetail, DWORD dwFlags)
    {
		if (!m_pHWContext->IsMicAGCSupported())
			return MMSYSERR_NOTSUPPORTED;

		return MixerControlBoolean::GetControlDetails(pDetail, dwFlags);
    }
    ...
    DWORD SetControlDetails (PMIXERCONTROLDETAILS pDetail, DWORD dwFlags)
    {
        ...
        return MixerControlBoolean::SetControlDetails(pDetail, dwFlags);
    }
};
...
{
public:
    MixerControlSingleMultiSelect(HardwareContext* pHWContext) : MixerControl(pHWContext) {}
...
// Final class to implement output equalizer preset.
class MixerControlEqPreset: public MixerControlSingleMultiSelect
{
public:
    MixerControlEqPreset(HardwareContext* pHWContext) : m_CurrSelection(0), MixerControlSingleMultiSelect(pHWContext) {}

    void CopyMixerControl(PMIXERCONTROL pamxctrl)
    {
        MixerControlSingleMultiSelect::CopyMixerControl(pamxctrl);
        StringCbCopy(pamxctrl->szName, sizeof(pamxctrl->szName), TEXT("Eq Preset"));
        StringCbCopy(pamxctrl->szShortName, sizeof(pamxctrl->szShortName), TEXT("Eq Preset"));
        pamxctrl->fdwControl = MIXERCONTROL_CONTROLF_UNIFORM|MIXERCONTROL_CONTROLF_MULTIPLE;
    }

    USHORT GetLineID() { return MXLINEID(LINE_OUT,NOLINE); }
    DWORD GetControlType() {return MIXERCONTROL_CONTROLTYPE_EQPRESET;}

    DWORD GetNumSelections() { return 3; }
    ...
    void GetListText(MIXERCONTROLDETAILS_LISTTEXT * pValue, DWORD Index)
    {
        ...
        pValue[Index].dwParam1 = 0;
        pValue[Index].dwParam2 = 0;
        StringCbCopy(pValue[Index].szName, sizeof(pValue[Index].szName),szNames[Index]);
    }
...
{
public:
    MixerControlInputMux(HardwareContext* pHWContext) : MixerControlSingleMultiSelect(pHWContext) {}

    void CopyMixerControl(PMIXERCONTROL pamxctrl)
    {
        MixerControlSingleMultiSelect::CopyMixerControl(pamxctrl);
        StringCbCopy(pamxctrl->szName, sizeof(pamxctrl->szName), TEXT("Input Mux"));
        StringCbCopy(pamxctrl->szShortName, sizeof(pamxctrl->szShortName), TEXT("Input Mux"));
        pamxctrl->fdwControl = MIXERCONTROL_CONTROLF_UNIFORM|MIXERCONTROL_CONTROLF_MULTIPLE;
    }
...
{
public:
    MixerControlList(HardwareContext* pHWContext)
        :   m_MixerControlVolumeMaster(pHWContext),
            m_MixerControlMuteMaster(pHWContext),
            m_MixerControlEqPreset(pHWContext),
            m_MixerControlVolumeMic(pHWContext),
            m_MixerControlMuteMic(pHWContext),
            m_MixerControlInputMux(pHWContext),
			m_MixerControlMicGain(pHWContext),
			m_MixerControlMicAGC(pHWContext)
    {
        m_MixerControlList[0] = &m_MixerControlVolumeMaster;
        m_MixerControlList[1] = &m_MixerControlMuteMaster;
        m_MixerControlList[2] = &m_MixerControlEqPreset;
        m_MixerControlList[3] = &m_MixerControlVolumeMic;
        m_MixerControlList[4] = &m_MixerControlMuteMic;
		m_MixerControlList[5] = &m_MixerControlInputMux;
		m_MixerControlList[6] = &m_MixerControlMicGain;
		m_MixerControlList[7] = &m_MixerControlMicAGC;
    ...
    MMRESULT SetMasterVolume(HardwareContext* pHWContext, DWORD dwVolume);
    DWORD GetMasterVolume(HardwareContext* pHWContext);

private:
    static const DWORD m_cControls = MAXMIXERCONTROLS;

    MixerControlVolumeMaster        m_MixerControlVolumeMaster;
    MixerControlMuteMaster          m_MixerControlMuteMaster;
    MixerControlEqPreset            m_MixerControlEqPreset;
    MixerControlVolumeMic           m_MixerControlVolumeMic;
    MixerControlMuteMic             m_MixerControlMuteMic;
    MixerControlInputMux            m_MixerControlInputMux;
	MixerControlMicGain             m_MixerControlMicGain;
	MixerControlMicAGC              m_MixerControlMicAGC;

    MixerControl *m_MixerControlList[m_cControls];

    friend MMRESULT SetMasterVolume(HardwareContext* pHWContext, DWORD dwVolume);
    friend DWORD GetMasterVolume(HardwareContext* pHWContext);
};
...
wdev_MXDM_GETDEVCAPS (PMIXERCAPS pCaps, DWORD dwSize)
{
    ...
    pCaps->cDestinations = _countof(g_dst_lines);
    pCaps->fdwSupport = 0;
...
wdev_MXDM_OPEN (PDWORD phMixer, PMIXEROPENDESC pMOD, DWORD dwFlags)
{
    ...
    if (pNew == NULL)
    {
        DEBUGMSG(ZONE_ERROR, (L"wdev_MXDM_OPEN: out of memory"));
...
{ int Index;
    ...
    case MIXER_GETLINEINFOF_DESTINATION:
        ...
        {
            if (pDetail->dwDestination >= _countof(g_dst_lines))
            {
    ...
    if (pFound != NULL)
    {
        ...
        StringCbCopy(pDetail->szName, sizeof(pDetail->szName), pFound->szName);
        StringCbCopy(pDetail->szShortName, sizeof(pDetail->szShortName), pFound->szShortName);
        StringCbCopy(pDetail->Target.szPname, sizeof(pDetail->Target.szPname), DEVICE_NAME);
...
DWORD
wdev_MXDM_GETLINECONTROLS (HardwareContext* pHWContext, PMIXERLINECONTROLS pDetail, DWORD dwFlags)
{
    MixerControlList *mixerControlList = (MixerControlList *)pHWContext->GetMixerControlList();
    ...
    case MIXER_GETLINECONTROLSF_ALL:
        ...
        // retrieve all controls for the line pDetail->dwLineID
        {
            ...
            {
                MixerControl *pMixerControl = mixerControlList->LookupControlByIndex(Index);
                if (!pMixerControl)
                {
    ...
    case MIXER_GETLINECONTROLSF_ONEBYID:
        {
            ...
            MixerControl *pMixerControl = mixerControlList->LookupControlByIndex(pDetail->dwControlID);
            if (!pMixerControl)
            {
    ...
    case MIXER_GETLINECONTROLSF_ONEBYTYPE:
        // retrieve the control specified by pDetail->dwLineID and pDetail->dwControlType
        {
            ...
            for (Index = 0;;Index++)
            {
                pMixerControl = mixerControlList->LookupControlByIndex(Index);
                if (!pMixerControl)
                {
                    break;
                }
...
DWORD
wdev_MXDM_GETCONTROLDETAILS (HardwareContext* pHWContext, PMIXERCONTROLDETAILS pDetail, DWORD dwFlags)
{
    ...
    MixerControl *pMixerControl = ((MixerControlList *)pHWContext->GetMixerControlList())->LookupControlByIndex(pDetail->dwControlID);
    if (!pMixerControl)
    {
        return MIXERR_INVALCONTROL;
    }
...
DWORD
wdev_MXDM_SETCONTROLDETAILS (HardwareContext* pHWContext, PMIXERCONTROLDETAILS pDetail, DWORD dwFlags)
{
    MIXERCONTROLDETAILS_UNSIGNED * pValue = (MIXERCONTROLDETAILS_UNSIGNED * ) pDetail->paDetails;
    ...
    MMRESULT mmRet = pMixerControl->SetControlDetails(pDetail, dwFlags);
...
BOOL InitMixerControls(HardwareContext* pHWContext)
{
    MixerControlList *mixerControlList = new MixerControlList(pHWContext);
    if (mixerControlList == NULL) {
        return FALSE;
    }

    pHWContext->SetMixerControlList((void *)mixerControlList);
    return TRUE;
}

BOOL HandleMixerMessage(HardwareContext* pHWContext, PMMDRV_MESSAGE_PARAMS pParams, DWORD *pdwResult)
{
    MMRESULT dwRet;
    ...
    _try
    {
        ...
        case MXDM_GETNUMDEVS:
            DEBUGMSG(ZONE_WODM, (TEXT("MXDM_GETNUMDEVS\r\n")));
            dwRet = 1;
            break;

        case MXDM_GETDEVCAPS:
            DEBUGMSG(ZONE_WODM, (TEXT("MXDM_GETDEVCAPS\r\n")));
            dwRet = wdev_MXDM_GETDEVCAPS((PMIXERCAPS) pParams->dwParam1, pParams->dwParam2);
            break;

        case MXDM_OPEN:
            DEBUGMSG(ZONE_WODM, (TEXT("MXDM_OPEN\r\n")));
            dwRet = wdev_MXDM_OPEN((PDWORD) pParams->dwUser, (PMIXEROPENDESC) pParams->dwParam1, pParams->dwParam2);
            break;

        case MXDM_CLOSE:
            DEBUGMSG(ZONE_WODM, (TEXT("MXDM_CLOSE\r\n")));
            dwRet = wdev_MXDM_CLOSE(pParams->dwUser);
            break;

        case MXDM_GETLINEINFO:
            DEBUGMSG(ZONE_WODM, (TEXT("MXDM_GETLINEINFO\r\n")));
            dwRet = wdev_MXDM_GETLINEINFO((PMIXERLINE) pParams->dwParam1, pParams->dwParam2);
            break;

        case MXDM_GETLINECONTROLS:
            DEBUGMSG(ZONE_WODM, (TEXT("MXDM_GETLINECONTROLS\r\n")));
            dwRet = wdev_MXDM_GETLINECONTROLS(pHWContext, (PMIXERLINECONTROLS) pParams->dwParam1, pParams->dwParam2);
            break;

        case MXDM_GETCONTROLDETAILS:
            DEBUGMSG(ZONE_WODM, (TEXT("MXDM_GETCONTROLDETAILS\r\n")));
            dwRet = wdev_MXDM_GETCONTROLDETAILS(pHWContext, (PMIXERCONTROLDETAILS) pParams->dwParam1, pParams->dwParam2);
            break;

        case MXDM_SETCONTROLDETAILS:
            DEBUGMSG(ZONE_WODM, (TEXT("MXDM_SETCONTROLDETAILS\r\n")));
            dwRet = wdev_MXDM_SETCONTROLDETAILS(pHWContext, (PMIXERCONTROLDETAILS) pParams->dwParam1, pParams->dwParam2);
            break;

        default:
            DEBUGMSG(ZONE_ERROR, (L"Unsupported mixer message"));
...
MMRESULT SetMasterVolume(HardwareContext* pHWContext, DWORD dwVolume)
{
    MixerControlList *pMixerControlList = (MixerControlList *)pHWContext->GetMixerControlList();
    pMixerControlList->m_MixerControlVolumeMaster.SetGain(dwVolume);
    PerformMixerCallbacks (MM_MIXM_CONTROL_CHANGE, pMixerControlList->m_MixerControlVolumeMaster.GetControlID());
    return MMSYSERR_NOERROR;
}

DWORD GetMasterVolume(HardwareContext* pHWContext)
{
    return ((MixerControlList *)pHWContext->GetMixerControlList())->m_MixerControlVolumeMaster.GetGain();
}
