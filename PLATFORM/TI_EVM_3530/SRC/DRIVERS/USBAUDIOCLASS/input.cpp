...
PBYTE InputStreamContext::Render2(PBYTE pBuffer, PBYTE pBufferEnd, PBYTE pBufferLast, TRANSFER_STATUS *pTransferStatus)
{
    if (m_dwChannels == 2) // stereo
    {   
        return Render2FromStereo(pBuffer, pBufferEnd, pBufferLast);
    } 
    else // mono
    {   
        return Render2FromMono(pBuffer, pBufferEnd, pBufferLast);
    }
}

PBYTE InputStreamContext::Render2FromStereo(PBYTE pBuffer, PBYTE pBufferEnd, PBYTE pBufferLast)
{
...
PBYTE InputStreamContext::Render2FromMono(PBYTE pBuffer, PBYTE pBufferEnd, PBYTE pBufferLast)
{
    LONG CurrPos = m_CurrPos;
    DWORD ClientRate = m_ClientRate;
    DWORD ClientRateInv = m_ClientRateInv;
