...
DWORD OutputStreamContext::Open(DeviceContext *pDeviceContext, LPWAVEOPENDESC lpWOD, DWORD dwFlags)
{
    ...
    m_dwChannels = m_pDeviceContext->GetChannels();
...
PBYTE OutputStreamContextS8::Render2ToStereo(PBYTE pBuffer, PBYTE pBufferEnd, PBYTE pBufferLast, TRANSFER_STATUS *pTransferStatus)
{
    LONG CurrPos = m_CurrPos;
    DWORD ClientRate = m_ClientRate;
    DWORD BaseRate = m_pDeviceContext->GetBaseSampleRate();
...
PBYTE OutputStreamContextS16::Render2ToStereo(PBYTE pBuffer, PBYTE pBufferEnd, PBYTE pBufferLast, TRANSFER_STATUS *pTransferStatus)
{
    LONG CurrPos = m_CurrPos;
    DWORD ClientRate = m_ClientRate;
    DWORD BaseRate = m_pDeviceContext->GetBaseSampleRate();
...
PBYTE OutputStreamContextS8::Render2ToMono(PBYTE pBuffer, PBYTE pBufferEnd, PBYTE pBufferLast, TRANSFER_STATUS *pTransferStatus)
{
    LONG CurrPos = m_CurrPos;
    DWORD ClientRate = m_ClientRate;
    DWORD BaseRate = m_pDeviceContext->GetBaseSampleRate();
...
PBYTE OutputStreamContextS16::Render2ToMono(PBYTE pBuffer, PBYTE pBufferEnd, PBYTE pBufferLast, TRANSFER_STATUS *pTransferStatus)
{
    LONG CurrPos = m_CurrPos;
    DWORD ClientRate = m_ClientRate;
    DWORD BaseRate = m_pDeviceContext->GetBaseSampleRate();
...
PBYTE OutputStreamContextMC::Render2(PBYTE pBuffer, PBYTE pBufferEnd, PBYTE pBufferLast, TRANSFER_STATUS *pTransferStatus)
{
    ...
    while (pBuffer < pBufferEnd)
    {
        ...
        for (i=0;i<2;i++)
        {
            ...
            // DEBUGMSG(ZONE_FUNCTION, (TEXT("PrevSamp0=0x%x, CurrSamp0=0x%x, CurrT=0x%x, OutSamp0=0x%x\r\n"), PrevSamp0,CurrSamp0,CurrT,OutSamp0));
...
PBYTE OutputStreamContextS8::Render2(PBYTE pBuffer, PBYTE pBufferEnd, PBYTE pBufferLast, TRANSFER_STATUS *pTransferStatus)
{
       if (m_dwChannels == 2)           // stereo
       {
           return Render2ToStereo(pBuffer, pBufferEnd, pBufferLast, pTransferStatus);
       } 
       else                             // Mono
       {
           return Render2ToMono(pBuffer, pBufferEnd, pBufferLast, pTransferStatus);
       }    
}
