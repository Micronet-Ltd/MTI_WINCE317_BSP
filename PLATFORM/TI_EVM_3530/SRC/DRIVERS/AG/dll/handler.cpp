...
DWORD CAGEngine::Init(CATParser* pParser)
{
    ...
    if (ERROR_SUCCESS != dwErr) {
		...
		RETAILMSG(1, (L"BTAGSVC:%S, Error initializing network module: %d.\r\n",__FUNCTION__, dwErr));
    }
    else {
    ...
    if (ERROR_SUCCESS != dwErr) {
		...
		RETAILMSG(1, (L"BTAGSVC:%S, Error initializing PhoneExt module: %d.\r\n",__FUNCTION__, dwErr));
    }
    else {
    ...
    if (ERROR_SUCCESS != BthAGSetServiceCallback(PhoneExtServiceCallback)) {
        ...
        RETAILMSG(1, (L"BTAGSVC:%S, Error setting service callback in PhoneExt.\r\n",__FUNCTION__));
    }
...
void CAGEngine::Deinit(void)
{
	...
	RETAILMSG(1, (L"BTAGSVC:+%S, AG Engine is being de-initialized.\r\n",__FUNCTION__));

    Lock();

    if (m_pParser) {
		RETAILMSG(1, (L"BTAGSVC:%S, CloseAudioChannel:\r\n",__FUNCTION__));
        CloseAudioChannel();
		RETAILMSG(1, (L"BTAGSVC:%S, CloseControlChannel:\r\n",__FUNCTION__));
        CloseControlChannel();

        if (m_fPhoneExtInited) {
            RETAILMSG(1, (L"BTAGSVC:%S, BthAGPhoneExtDeinit:\r\n",__FUNCTION__)); 
            BthAGPhoneExtDeinit();
        }

        if (m_fNetworkInited) {
			RETAILMSG(1, (L"BTAGSVC:%S, BthAGNetworkDeinit:\r\n",__FUNCTION__)); 
            BthAGNetworkDeinit();
        }

        if (m_hHangupEvent) {
			RETAILMSG(1, (L"BTAGSVC:%S, CloseHandle(m_hHangupEvent = 0x%X):\r\n",__FUNCTION__, m_hHangupEvent)); 
            CloseHandle(m_hHangupEvent);
            m_hHangupEvent = NULL;
        }

        if (m_pTP) {
			RETAILMSG(1, (L"BTAGSVC:%S, delete m_pTP:\r\n",__FUNCTION__)); 
            delete m_pTP;
            m_pTP = NULL;
        }
		...
		RETAILMSG(1, (L"BTAGSVC:%S, Sleeping while AGEngine ref count is still active:\r\n",__FUNCTION__)); 
        while (GetRefCount() > 1) {
            DEBUGMSG(ZONE_HANDLER, (L"BTAGSVC: Sleeping while AGEngine ref count is still active.\n"));
			RETAILMSG(1, (L"BTAGSVC:%S, ref count = %d\r\n",__FUNCTION__, GetRefCount())); 
            Unlock();
            Sleep(500);
            Lock();
        }
	...
	RETAILMSG(1, (L"BTAGSVC:-%S, AG Engine de-initialization completed.\r\n",__FUNCTION__));
}
...
DWORD GetBTAddrList(AG_DEVICE* pDevices, const USHORT usNumDevices)
{
    ...
    RETAILMSG(1, (L"BTAGSVC:+%S, max num of devices = %d\r\n",__FUNCTION__, usNumDevices));
    ...
    if (ERROR_SUCCESS != dwRetVal) {
		...
		RETAILMSG(1, (L"BTAGSVC:%S, Could not open registry key for BT Addr: %d.\r\n",__FUNCTION__, dwRetVal));
        goto exit;
    }
...
exit:
	...
	RETAILMSG(1, (L"BTAGSVC:-%S, found devices = %d\r\n",__FUNCTION__, dwIdx));
    return dwRetVal;
}
...
DWORD CAGEngine::SetBTAddrList(BT_ADDR btAddrDefault, BOOL fHfSupport)
{
    ...
    if (i > 0) {
		...
		RETAILMSG(1, (L"BTAGSVC:%S, btaddr = %04x%08x, service = \r\n",__FUNCTION__, GET_NAP(btAddrDefault), GET_SAP(btAddrDefault)));
		RETAILMSG(1, (L"  BTAGSVC:%S, service id = ",__FUNCTION__));
		for( int i = 0; i < sizeof(GUID); i++)
		  RETAILMSG(1, (L"[%02X]", *((unsigned char *)&service +i) ));
    ...
    if (ERROR_SUCCESS == dwRetVal) {
        ...
        while ((dwIdx < ARRAY_SIZE(m_AGProps.DeviceList)) && m_AGProps.DeviceList[dwIdx].bta != 0) {
			...
			RETAILMSG(1, (L"BTAGSVC:%S, Saving to registry btAddr %04x%08x at index %d.\r\n",__FUNCTION__, GET_NAP(m_AGProps.DeviceList[dwIdx].bta), GET_SAP(m_AGProps.DeviceList[dwIdx].bta), dwIdx+1 ));
			RETAILMSG(1, (L"  BTAGSVC:%S, Saving to registry service id = ",__FUNCTION__));
			for( int i = 0; i < sizeof(GUID); i++)
			  RETAILMSG(1, (L"[%02X]", *((unsigned char *)&m_AGProps.DeviceList[dwIdx].service +i) ));
...
DWORD CAGEngine::NotifyConnect(SOCKET sockClient, BOOL fHFSupport)
{
    ...
    if (ERROR_SUCCESS != dwRetVal) {
		...
		RETAILMSG(1, (L"BTAGSVC:%S, Error starting parser module: %d\r\n",__FUNCTION__, dwRetVal));
        goto exit;
    }
...
exit:
    ...
    Unlock();
	RETAILMSG(1, (L"BTAGSVC:-%S, net state = %d, ret = %d, ref Count = %d\r\n",__FUNCTION__, m_AGProps.state, dwRetVal, GetRefCount()));
    return dwRetVal;
}
...
DWORD CAGEngine::NotifySCOConnect(unsigned short handle)
{
    ...
    if (m_AGProps.state >= AG_STATE_AUDIO_UP) {
		...
		RETAILMSG(1, (L"BTAGSVC:%S, NotifySCOConnect - Audio connection already opened.\r\n",__FUNCTION__));
    ...
    if (m_AGProps.state < AG_STATE_CONNECTED) {
		...
		RETAILMSG(1, (L"BTAGSVC:%S, NotifySCOConnect - Cannot open audio connection until service-level connection has been made.\r\n",__FUNCTION__));
        dwRetVal = ERROR_NOT_READY;
        goto exit;        
    }
    ...
    for (UINT i = 0; i < numDevs; i++) {
        waveOutMessage((HWAVEOUT)i, WODM_BT_SCO_AUDIO_CONTROL, 0, 2/*TRUE*/);
		RETAILMSG(1, (L"BTAGSVC:%S, WODM_BT_SCO_AUDIO_CONTROL #2, dev # %d\r\n",__FUNCTION__, i));
    }
...
void CAGEngine::ClearSCO(void)
{
	...
	RETAILMSG(1, (L"BTAGSVC:+%S, net state = %d, ref Count = %d\r\n",__FUNCTION__, m_AGProps.state, GetRefCount()));

    ASSERT(IsLocked());
    
    if (AG_STATE_AUDIO_UP == m_AGProps.state) {
        DEBUGMSG(ZONE_HANDLER, (L"BTAGSVC: SCO connection was closed.\n"));
		RETAILMSG(1, (L"BTAGSVC:%S, SCO connection was closed\r\n",__FUNCTION__));
        m_AGProps.state = AG_STATE_CONNECTED;
        fClearSCO = TRUE;
    }
    ...
    if (fClearSCO) {
        UINT numDevs = waveOutGetNumDevs();
        for (UINT i = 0; i < numDevs; i++) {
			RETAILMSG(1, (L"BTAGSVC:%S, WODM_BT_SCO_AUDIO_CONTROL #13\r\n",__FUNCTION__));
...
DWORD CAGEngine::OpenAGConnection(BOOL fOpenAudio, BOOL fFirstOnly)
{
	...
	RETAILMSG(1, (L"BTAGSVC:+%S, fOpenAudio = %d, fFirstOnly = %d\r\n",__FUNCTION__, fOpenAudio, fFirstOnly));
...
DWORD CAGEngine::OpenControlChannel(BOOL fFirstOnly)
{
	...
	RETAILMSG(1, (L"BTAGSVC:+%S, fFirstOnly = %d, net state = %d, ref Count = %d\r\n",__FUNCTION__, fFirstOnly, m_AGProps.state, GetRefCount()));
    ...
    if (m_AGProps.state >= AG_STATE_CONNECTING) {
        ...
        if (m_fAudioConnect && (m_AGProps.state >= AG_STATE_CONNECTED)) {
			...
			RETAILMSG(1, (L"BTAGSVC:%S, Already connected, just opening audio connection.\r\n",__FUNCTION__));
            m_fAudioConnect = FALSE;
			...
			RETAILMSG(1, (L"BTAGSVC:%S, OpenControlConnection returning ERROR_ALREADY_INITIALIZED\r\n",__FUNCTION__));
            dwRetVal = ERROR_ALREADY_INITIALIZED;
        }
    ...
    if (m_AGProps.DeviceList[0].bta == 0) {
		...
		RETAILMSG(1, (L"BTAGSVC:%S, Tried to open a connection but no device has been paired yet.\r\n",__FUNCTION__));
        dwRetVal = ERROR_NOT_READY;
        goto exit;
    }
    ...
    while (dwDeviceIdx < ARRAY_SIZE(m_AGProps.DeviceList)) {
        if (m_AGProps.DeviceList[dwDeviceIdx].bta == 0) {
			...
			RETAILMSG(1, (L"BTAGSVC:%S, Device with index %d does not exist, failed to connect.\r\n",__FUNCTION__, dwDeviceIdx));
            dwRetVal = ERROR_NOT_READY;
            goto exit;
        }
		...
		RETAILMSG(1, (L"BTAGSVC:%S, Befroe SOCKET connect, dwDeviceIdx = %d\r\n",__FUNCTION__, dwDeviceIdx));
		RETAILMSG(1, (L"BTAGSVC:%S, headset = %d, btaddr = %04x%08x, service = \r\n",__FUNCTION__, fHS, GET_NAP(sa.btAddr), GET_SAP(sa.btAddr)));
		for( int i = 0; i < sizeof(GUID); i++)
          RETAILMSG(1, (L"[%02X]", *((char *)&m_AGProps.DeviceList[dwDeviceIdx].service +i) ));

		RETAILMSG(1, (L"\r\n")); 

		// For test only, because service is not set, set hands-free anyway
		//fHS = 0;
        ...
        if (fHS) {
			...
			RETAILMSG(1, (L"BTAGSVC:%S, - Attempting to connect to headset device at index %d.\r\n",__FUNCTION__, dwDeviceIdx));
			...
			RETAILMSG(1, (L"BTAGSVC:%S, - Attempting to connect to HF device at index %d.\r\n",__FUNCTION__, dwDeviceIdx));
        ...
        if (SOCKET_ERROR == connect(m_sockClient, (SOCKADDR *)&sa, sizeof(sa))) {
            if (fFirstOnly) {
				...
				RETAILMSG(1, (L"BTAGSVC:%S, - SOCKET_ERROR = %d.\r\n",__FUNCTION__, dwRetVal));
                goto exit;
            }
            else {
                dwDeviceIdx++;
    ...
    if (dwDeviceIdx == ARRAY_SIZE(m_AGProps.DeviceList)) {
		...
		RETAILMSG(1, (L"BTAGSVC:%S, - Error: Could not connect to any devices.\r\n",__FUNCTION__));
        dwRetVal = ERROR_NOT_CONNECTED;
        goto exit;
    }
    ...
    BthAGPhoneExtEvent(AG_PHONE_EVENT_BT_CTRL, 2, NULL);
...
void CAGEngine::CloseAGConnection(BOOL fCloseControl)
{
	...
	RETAILMSG(1, (L"BTAGSVC:+%S, CloseControl = %d, handfree = %d\r\n",__FUNCTION__, fCloseControl, m_AGProps.fHandsfree));
...
void CAGEngine::CloseControlChannel(void) 
{
    ...
    if (m_pParser) {
		RETAILMSG(1, (L"BTAGSVC:%S, Stop parser, Tread Id = 0x%X\r\n",__FUNCTION__, GetCurrentThreadId()));
        m_pParser->Stop();
    }
...
void CAGEngine::ServiceConnectionUp()
{
	...
	RETAILMSG(1, (L"BTAGSVC:-%S, net state = %d\r\n",__FUNCTION__, m_AGProps.state));
}
...
DWORD CAGEngine::OpenAudioChannel(void)
{
    ...
    if (m_AGProps.state >= AG_STATE_AUDIO_UP) {
		...
		RETAILMSG(1, (L"BTAGSVC:%S, Audio connection already opened.\r\n",__FUNCTION__));
        m_AGProps.state = AG_STATE_AUDIO_UP;
        dwRetVal = ERROR_ALREADY_INITIALIZED;
        goto exit;
    }
    ...
    if (m_AGProps.state < AG_STATE_CONNECTED) {
		...
		RETAILMSG(1, (L"BTAGSVC:%S, Cannot open audio connection until service-level connection has been made.\r\n",__FUNCTION__));
        dwRetVal = ERROR_NOT_READY;
        goto exit;        
    }
    ...
    if (m_AGProps.fPCMMode) {
        ...
        if (ERROR_SUCCESS != dwRetVal) {
			...
			RETAILMSG(1, (L"BTAGSVC:%S, Error creating SCO connection: %d\r\n",__FUNCTION__, dwRetVal));
    ...
    for (UINT i = 0; i < numDevs; i++) {
		...
		RETAILMSG(1, (L"BTAGSVC:%S, WODM_BT_SCO_AUDIO_CONTROL # 3 , dev # %d\r\n",__FUNCTION__, i));
    }
...
DWORD CAGEngine::CloseAudioChannel(void)
{
    ...
    if (m_AGProps.state >= AG_STATE_AUDIO_UP) {
		...
		RETAILMSG(1, (L"BTAGSVC:%S, CloseAudioChannel - audio is up, closing connection\r\n",__FUNCTION__));

        EnsureActiveBaseband();

        UINT numDevs = waveOutGetNumDevs();
        for (UINT i = 0; i < numDevs; i++) {
			RETAILMSG(1, (L"BTAGSVC:%S, WODM_BT_SCO_AUDIO_CONTROL #14\r\n",__FUNCTION__));
            waveOutMessage((HWAVEOUT)i, WODM_BT_SCO_AUDIO_CONTROL, 0, FALSE);
        }
        ...
        if (m_AGProps.fPCMMode) {
			...
			RETAILMSG(1, (L"BTAGSVC:%S, CloseAudioChannel - Need to close SCO connection in PCM mode\r\n",__FUNCTION__));
            
            dwRetVal = BthCloseConnection (m_hSco);
            if (dwRetVal != ERROR_SUCCESS) {
                DEBUGMSG(ZONE_WARN, (_T("BTAGSVC: Error closing SCO connection: %d\n"), dwRetVal));
				RETAILMSG(1, (L"BTAGSVC:%S, Error closing SCO connection: %d\r\n",__FUNCTION__, dwRetVal));
            }
...
void CAGEngine::SetSpeakerVolume(unsigned short usVolume)
{
	...
	RETAILMSG(1, (L"BTAGSVC:+%S = %d\r\n",__FUNCTION__, usVolume));
...
void CAGEngine::OnHeadsetButton(LPSTR pszParams, int cchParam)
{
	...
	RETAILMSG(1, (L"BTAGSVC:+%S, sparam = %S, cchParam = %d\r\n",__FUNCTION__, pszParams, cchParam));
    ...
    if ((AG_STATE_RINGING == m_AGProps.state) || (AG_STATE_RINGING_AUDIO_UP == m_AGProps.state)) {
		...
		RETAILMSG(1, (L"BTAGSVC:%S, Calling AGEngine::OnHeadsetButton - In ringing state, answer call.\r\n",__FUNCTION__));
        ...
        if (ERROR_SUCCESS != dwErr) {
			...
			RETAILMSG(1, (L"BTAGSVC:%S, Error answering call: %d\r\n",__FUNCTION__, dwErr));
		...
		RETAILMSG(1, (L"BTAGSVC:%S, Not expecting headset button, drop/swap call.\r\n",__FUNCTION__));
        ...
        if (ERROR_SUCCESS != dwErr) {
			...
			RETAILMSG(1, (L"BTAGSVC:%S, Error swapping call: %d\r\n",__FUNCTION__, dwErr));
            m_AGProps.fUseHFAudio = FALSE;
        }
    }
    else {
		...
		RETAILMSG(1, (L"BTAGSVC:%S, Expecting headset button after connect, do nothing.\r\n",__FUNCTION__));
        m_fExpectHeadsetButton = FALSE;
    }

	// In Headset Profile, this event just need pass to Application
...
void CAGEngine::OnSpeakerVol(LPSTR pszParams, int cchParam)
{
    ...
    if (pTmp != (pszParams + strlen(pszParams))) {
		...
		RETAILMSG(1, (L"BTAGSVC:%S, Speaker volume AT Command was poorly formatted.\r\n",__FUNCTION__));
		...
		RETAILMSG(1, (L"BTAGSVC:%S, Received speaker volume indication of %d - returning ERROR (out of range).\r\n",__FUNCTION__, usVolume));
        SendATCommand(AT_ERROR, sizeof(AT_ERROR)-1);
    }
    else {
        m_AGProps.usSpeakerVolume = usVolume;
...
void CAGEngine::OnMicVol(LPSTR pszParams, int cchParam)
{
    ...
    if (pTmp != (pszParams + strlen(pszParams))) {
		...
		RETAILMSG(1, (L"BTAGSVC:%S, Mic volume AT Command was poorly formatted.\r\n",__FUNCTION__));
        SendATCommand(AT_ERROR, sizeof(AT_ERROR)-1);
    }
    else if (usVolume > 15) {
        DEBUGMSG(ZONE_HANDLER, (L"BTAGSVC: Received microphone volume indication of %d - returning ERROR (out of range).\n", usVolume));
		RETAILMSG(1, (L"BTAGSVC:%S, Received microphone volume indication of %d - returning ERROR (out of range).\r\n",__FUNCTION__, usVolume));
        SendATCommand(AT_ERROR, sizeof(AT_ERROR)-1);
    }
    else {
        m_AGProps.usMicVolume = usVolume;
...
void CAGEngine::SetUseHFAudio(BOOL fUseHFAudio)
{
	...
	RETAILMSG(1, (L"BTAGSVC:+%S, Setting use handsfree audio mode to %s\r\n",__FUNCTION__, fUseHFAudio ? L"TRUE" : L"FALSE"));
...
void CAGEngine::SetUseInbandRing(BOOL fUseInbandRing)
{
	...
	RETAILMSG(1, (L"BTAGSVC:+%S, Setting use inband ring to %s\r\n",__FUNCTION__, fUseInbandRing ? L"TRUE" : L"FALSE"));
...
void CAGEngine::OnDial(LPSTR pszParams, int cchParam)
{
    ...
    if (ERROR_SUCCESS != dwErr) {
		...
		RETAILMSG(1, (L"BTAGSVC:%S, Error dialing number: %d\r\n",__FUNCTION__, dwErr));
...
void CAGEngine::OnDialLast(LPSTR pszParams, int cchParam)
{
	...
	RETAILMSG(1, (L"BTAGSVC:%S, Last Dialed number: %s\r\n",__FUNCTION__, wszNumber));
...
void CAGEngine::OnDialMemory(LPSTR pszParams, int cchParam)
{
    ...
    if (pTmp != (pszParams + strlen(pszParams))) {
		...
		RETAILMSG(1, (L"BTAGSVC:%S, ATD> Command was poorly formatted\r\n",__FUNCTION__));
        SendATCommand(AT_ERROR, sizeof(AT_ERROR)-1);
    }
    else {
        WCHAR wszNumber[MAX_PHONE_NUMBER];
...
void CAGEngine::OnHangupCall(LPSTR pszParam, int cchParam)
{
        ...
        (!(m_AGProps.usHFCapability & AG_CAP_REJECT_CALL)))
    {
		...
		RETAILMSG(1, (L"BTAGSVC:%S, Do not support rejecting incoming calls\r\n",__FUNCTION__));
        SendATCommand(AT_ERROR, sizeof(AT_ERROR)-1);
    }
    else {       
        DWORD dwErr = ERROR_SUCCESS;
        ...
        if (ERROR_SUCCESS == dwErr) {    
            ...
            if (dwDropFlags) {
                ...
                RETAILMSG(1, (L"BTAGSVC:%S, Wait for hang-up event when a call was dropped\r\n",__FUNCTION__)); 
                DWORD dwRet = WaitForSingleObject(m_hHangupEvent, AG_COMMAND_COMPLETE_TO);
				switch(dwRet)
				{
				 case  WAIT_OBJECT_0: RETAILMSG(1, (L"BTAGSVC:%S, hang-up event has come\r\n",__FUNCTION__));  break;
				 case  WAIT_TIMEOUT: RETAILMSG(1, (L"BTAGSVC:%S, hang-up event timeout\r\n",__FUNCTION__));  break;
				 case  WAIT_FAILED:  RETAILMSG(1, (L"BTAGSVC:%S, hang-up event failure\r\n",__FUNCTION__));  break;
				 default:  RETAILMSG(1, (L"BTAGSVC:%S, hang-up event error\r\n",__FUNCTION__));  break;
				}
			...
			RETAILMSG(1, (L"BTAGSVC:%S, Error dropping call: %d\r\n",__FUNCTION__, dwErr));
            SendATCommand(AT_ERROR, sizeof(AT_ERROR)-1);            
        }
    }
...
void CAGEngine::OnEnableCallWaiting(LPSTR pszParams, int cchParam)
{
	...
	RETAILMSG(1, (L"BTAGSVC:+%S, sparam = %S, cparam = %d\r\n",__FUNCTION__, pszParams, cchParam));
...
{
	RETAILMSG(1, (L"BTAGSVC:+%S, dwEvent = %d\r\n",__FUNCTION__, dwEvent));
    ...
    switch (dwEvent) {
        ...
        case NETWORK_EVENT_CALL_DISCONNECT:
            if (sizeof(DWORD) > cbParam) {
				...
				RETAILMSG(1, (L"BTAGSVC:%S, Received error event from network component with invalid param size.\r\n",__FUNCTION__));
                ASSERT(0);                    
            }
            else {
        ...
        default:
			...
			RETAILMSG(1, (L"BTAGSVC:%S, Unknown network event: %d\r\n",__FUNCTION__, dwEvent));
            ASSERT(0);
    }
}
    ...
    if ((m_AGProps.state >= AG_STATE_CONNECTED) && (m_AGProps.fUseHFAudio || m_AGProps.fUseHFAudioAlways)) {
        if (m_AGProps.usCallType == AG_CALL_TYPE_IN) {
            ...
            if ((ERROR_SUCCESS != dwErr) && (ERROR_ALREADY_INITIALIZED != dwErr)) {
				...
				RETAILMSG(1, (L"BTAGSVC:%S, Error opening audio connection: %d\r\n",__FUNCTION__, dwErr));
            }
        }
    }
    ...
    if (m_AGProps.fInCall) {
        ...
        if ((ERROR_SUCCESS != dwErr) && (ERROR_ALREADY_INITIALIZED != dwErr)) {
            ...
            if (m_hUIThread) {
				...
				RETAILMSG(1, (L"BTAGSVC:%S, Out of resources.\r\n",__FUNCTION__));
            }
        }
    }
...
void CAGEngine::OnNetworkCallOut(void)
{
    ...
    if ((ERROR_SUCCESS != dwErr) && (ERROR_ALREADY_INITIALIZED != dwErr)) {
		...
		RETAILMSG(1, (L"BTAGSVC:%S, Error opened AG Connection: %d.\r\n",__FUNCTION__, dwErr));
    }
    else {
        if (! m_hUIThread) {
            m_hUIThread = CreateThread(NULL, 0, AGUIThread, this, 0, NULL);
...
void CAGEngine::OnNetworkInfo(LPSTR pszNumber)
{
	...
	RETAILMSG(1, (L"BTAGSVC:+%S, num = [%S]\r\n",__FUNCTION__, pszNumber));
...
void CAGEngine::OnNetworkRing(void)
{
    ...
    if (m_AGProps.state >= AG_STATE_CONNECTED) {
		...
		RETAILMSG(1, (L"BTAGSVC:%S, net state = %d, fMuteRings = %d\r\n",__FUNCTION__,  m_AGProps.state, m_AGProps.fMuteRings));
...
void CAGEngine::OnNetworkCallFailed(USHORT usCallType, DWORD dwStatus)
{
	...
	RETAILMSG(1, (L"BTAGSVC:+%S, error # %d\r\n",__FUNCTION__, dwStatus));
    ...
    switch (usCallType) {
        case CALL_TYPE_ANSWER:
        {
			...
			RETAILMSG(1, (L"BTAGSVC:+%S, Error answering call: 0x%X\r\n",__FUNCTION__, dwStatus));
            m_AGProps.fUseHFAudio = FALSE;
        ...
        case CALL_TYPE_DROP:
        {
			...
			RETAILMSG(1, (L"BTAGSVC:+%S, Error dopping call: 0x%X\r\n",__FUNCTION__, dwStatus));
        }
        break;
        ...
        case CALL_TYPE_HOLD:
        {
			...
			RETAILMSG(1, (L"BTAGSVC:+%S, Error putting call on hold: 0x%X\r\n",__FUNCTION__, dwStatus));
        }
        break;
        ...
        case CALL_TYPE_SWAP:
        {
			...
			RETAILMSG(1, (L"BTAGSVC:+%S, Error swapping call: 0x%X\r\n",__FUNCTION__, dwStatus));
            m_AGProps.fUseHFAudio = FALSE;
        }
        break;
        ...
        case CALL_TYPE_UNHOLD:
        {
			...
			RETAILMSG(1, (L"BTAGSVC:+%S, Error taking call off hold: 0x%X\r\n",__FUNCTION__, dwStatus));
            m_AGProps.fUseHFAudio = FALSE;
        }
        break;
    }
...
DWORD CAGEngine::SendATCommand(LPCSTR pszCommand, unsigned int cbCommand)
{
	...
	DbgPrintATCmd(1, pszCommand, cbCommand);
    ...
    if (SOCKET_ERROR == cdwBytesWritten) {
		...
		RETAILMSG(1, (L"BTAGSVC:%S, Error writing AT command to the device: %d\r\n",__FUNCTION__, dwRetVal));
    }
		...
		RETAILMSG(1, (L"BTAGSVC:%S, Warning --> Send only wrote %d of %d total bytes.\r\n",__FUNCTION__, cdwBytesWritten, cbCommand));
    }
	...
	RETAILMSG(1, (L"BTAGSVC:-%S, net state = %d, ret = %d\r\n",__FUNCTION__, m_AGProps.state, dwRetVal));
    return dwRetVal;
}
...
void CAGEngine::AGUIThread_Int(void)
{
    ...
    if (! fCallAG) {
        RETAILMSG(1, (L"BTAGSVC:%S, Close Channel\r\n",__FUNCTION__)); 
        CloseAudioChannel();
        if (m_AGProps.fPowerSave || !m_AGProps.fHandsfree) {
            CloseControlChannel();
        }
...
{
	RETAILMSG(1, (L"BTAGSVC:+%S, Cookie = 0x%X\r\n",__FUNCTION__, m_SniffCookie));
    ...
    if (m_SniffCookie) {        
		...
		RETAILMSG(1, (L"BTAGSVC:%S, m_pTP = 0x%X\r\n",__FUNCTION__,  &m_pTP));
    ...
    if ((m_AGProps.state >= AG_STATE_CONNECTED) && (m_AGProps.state < AG_STATE_AUDIO_UP)) {
        ...
        if (ERROR_SUCCESS == BthGetCurrentMode(&m_AGProps.btAddrClient, &mode)) {
            if (mode < 0x02) { // check we are not already in deep enough sleep
				...
				RETAILMSG(1, (L"BTAGSVC:%S,  Going into SNIFF.\r\n",__FUNCTION__)); 
                ...
                if (ERROR_SUCCESS != BthEnterSniffMode(&m_AGProps.btAddrClient, 
					...
					RETAILMSG(1, (L"BTAGSVC:%S, !! Warning - failed to enter sniff mode.\r\n",__FUNCTION__)); 
                }
            }
        }
    }
