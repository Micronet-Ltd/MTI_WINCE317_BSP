...
{
	RETAILMSG(AG_SVC_DBG, (L"BTAGSVC:+%S, hInstDll = 0x%X, fdwReason = %d\r\n",__FUNCTION__, hInstDll, fdwReason));
	...
	RETAILMSG(AG_SVC_DBG, (L"BTAGSVC:-%S\r\n",__FUNCTION__));
    return TRUE;
}
...
extern "C" DWORD BAG_Init (DWORD dwData)
{
    ...
    if (! g_pAG) {
		...
		RETAILMSG(AG_SVC_DBG, (L"BTAGSVC:%S, Init pass Ok\r\n",__FUNCTION__));
		...
		RETAILMSG(AG_SVC_DBG, (L"BTAGSVC:%S, [%s] key read Ok\r\n",__FUNCTION__, RK_AG_SERVICE));
        ...
        if (dwStartService) {
			...
			RETAILMSG(AG_SVC_DBG, (L"BTAGSVC:%S, Service started Ok\r\n",__FUNCTION__));
        }
    }
    else {
        fRetVal = FALSE;
...
exit:
	RETAILMSG(AG_SVC_DBG, (L"BTAGSVC:-%S, err = %d\r\n",__FUNCTION__, fRetVal));
    if ((FALSE == fRetVal) && g_pAG) {
        g_pAG->Deinit();
        delete g_pAG;
        g_pAG = NULL;
        g_dwState = SERVICE_STATE_UNINITIALIZED;
		RETAILMSG(AG_SVC_DBG, (L"BTAGSVC:%S, Service UNINITIALIZED\r\n",__FUNCTION__));
    }
