...
void CATParser::ATParserThread_Int(void)
{
	...
	RETAILMSG(1, (L"BTAGSVC:+%S, ATParserThread started\r\n",__FUNCTION__));
    ...
    while (1) {
		...
		RETAILMSG(1, (L"BTAGSVC:%S, Parser thread is calling recv again.\r\n",__FUNCTION__));
        ...
        if (m_fShutdown || (0 == cbCommand)) {
			...
			RETAILMSG(1, (L"BTAGSVC:%S, Signalled to break from ATParserThread_Int.\r\n",__FUNCTION__));
			RETAILMSG(1, (L"BTAGSVC:%S, ParserThreadId = 0x%X\r\n",__FUNCTION__, GetCurrentThreadId()));
            
            Unlock();
            m_pHandler->CloseAGConnection(TRUE);
            Lock();

			RETAILMSG(1, (L"BTAGSVC:%S, 'CloseAGConnection passed'\r\n",__FUNCTION__));
            break;
        }
        ...
        DEBUGMSG(ZONE_PARSER, (L"\n"));
#endif // DEBUG
		DbgPrintATCmd(1, pszBuf, cbCommand);
        ...
        while ((! fHandled) && (! m_fShutdown)) {
            if (ATCmdTable[i].pszCommand[0] == 0) {
                if (! _stricmp(pszBuf, "\r\nOK\r\n")) {
					...
					RETAILMSG(1, (L"BTAGSVC:%S, AT Command is not handled\r\n",__FUNCTION__));
				...
				RETAILMSG(1, (L"BTAGSVC:%S, AT Command is being handled.\r\n",__FUNCTION__));
                ...
                Unlock();
				// Parser calls command
				RETAILMSG(1, (L"BTAGSVC:%S call [%S] command, params = [%S] , %d\r\n",__FUNCTION__, ATCmdTable[i].pszCommand, pszParam, cchParam));
	...
	RETAILMSG(1, (L"BTAGSVC:-%S, ATParserThread exiting.\r\n",__FUNCTION__));
}
...
DWORD CATParser::Start(CAGEngine* pHandler, SOCKET sockClient)
{
	...
	RETAILMSG(1, (L"BTAGSVC:+%S, AT Command Parser is being started.\r\n",__FUNCTION__));
...
#if 0 // Testing change, need for show Thread Id, not obnly thread handler
...
#else
    m_hThread = CreateThread(NULL, 0, ATParserThread, this, 0, &m_hThreadId);
    if (! m_hThread) {
        dwRetVal = GetLastError();
        goto exit;
    }
...
exit:
	...
	RETAILMSG(1, (L"BTAGSVC:-%S, Parser Thread handle = 0x%X, Thread Id = 0x%X\r\n",__FUNCTION__, m_hThread, m_hThreadId));
    return dwRetVal;
}
...
void CATParser::Stop(void)
{
	...
	RETAILMSG(1, (L"BTAGSVC:+%S, Stopping AT Command Parser.\r\n",__FUNCTION__));
    
    Lock();

    HANDLE h = m_hThread;
	DWORD Id = m_hThreadId;
    m_fShutdown = TRUE;
    m_hThread = NULL;
	m_hThreadId = 0;
    ...
    if (m_sockClient != INVALID_SOCKET) {
		...
		RETAILMSG(1, (L"BTAGSVC:%S, Closing client socket %d.\r\n",__FUNCTION__, m_sockClient));
	...
	RETAILMSG(1, (L"BTAGSVC:%S, Id = 0x%X, current Thread Id = 0x%X\r\n",__FUNCTION__, Id, GetCurrentThreadId()));

    if (Id) {
        if (Id != GetCurrentThreadId()) {
            DEBUGMSG(ZONE_PARSER, (L"BTAGSVC: Waiting for ATParserThread (id=%d) to exit.\n", h));
			RETAILMSG(1, (L"BTAGSVC:%S, Waiting for ATParserThread (id=0x%X) to exit.\r\n",__FUNCTION__, GetCurrentThreadId()));
	...
	RETAILMSG(1, (L"BTAGSVC:-%S, AT Command Parser is stopped.\r\n",__FUNCTION__));
}
