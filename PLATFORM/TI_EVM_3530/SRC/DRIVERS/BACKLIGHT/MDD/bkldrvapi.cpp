...
extern DWORD GetTimeout(BKL_MDD_INFO *pBKLinfo);

GPIO_Backlight s_Backlight;
...
//MYS     pBKLinfo->dwPddContext = BacklightInit(pContext, lpvBusContext, &(pBKLinfo->dwCurState));
    if (s_Backlight.Initialize(pContext) == FALSE)
        {
        DEBUGMSG(ZONE_ERROR, (L"ERROR: BKL_Init: "
        ...
        case IOCTL_POWER_CAPABILITIES:  
            ...
            if (lpOutBuf && nOutBufSize >= sizeof (POWER_CAPABILITIES) && lpBytesReturned) 
            {
                __try 
                {
                    ...
                    PowerCaps->DeviceDx = s_Backlight.BacklightGetSupportedStates();
        ...
        case IOCTL_POWER_SET: // requests a change from one device power state to another
                ...
                if (lpOutBuf && nOutBufSize >= sizeof(CEDEVICE_POWER_STATE)) 
                {
                    __try 
                    {
                        ...
                        if(!VALID_DX(ReqDx))
                        {
							RETAILMSG(1, (TEXT("BKL: Invalid state request D%u\r\n"), ReqDx));
							break;
                        }
						if(ReqDx == pBKLinfo->dwCurState) 
                        {
                            *(PCEDEVICE_POWER_STATE)lpOutBuf = ReqDx;
                            *lpBytesReturned = sizeof(CEDEVICE_POWER_STATE);

                            dwErr = ERROR_SUCCESS;
							RETAILMSG(1, (TEXT("BKL: already set D%u\r\n"), ReqDx));
							break;
						}
                        ...
                        {       
                            if( (D1 == SupportedDx || D2 == SupportedDx)	&&
								(GetTimeout(pBKLinfo) == INFINITE)				)
								{
									pBKLinfo->dwCurState = D0;
							}
							else
							...
							if(D0 == pBKLinfo->dwCurState)
							{
                                ...
                                if(pszDevice != NULL)
                                {
                                    ...
                                    {
										RETAILMSG(1, (L"Backlight::PowerRequirement(On)\r\n"));
                            ...
                            {
								RETAILMSG(0, (L"Backlight::ReleasePowerRequirement()\r\n"));
...
//MYS                                pBKLinfo->dwCurState=SupportedDx;
                        ...
                        {
                            RETAILMSG(1, (TEXT("Unsupported state request D%u\r\n"), ReqDx));
                        }
                    }
                    __except(EXCEPTION_EXECUTE_HANDLER) 
                    {
                        RETAILMSG(1, (TEXT("Exception in ioctl\r\n")));
                    }
                }
                break;
