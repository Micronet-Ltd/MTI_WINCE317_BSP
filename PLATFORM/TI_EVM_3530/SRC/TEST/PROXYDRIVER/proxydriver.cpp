    ...
    )
{
    ...
    switch (code)
		...
		case IOCTL_HAL_SW_UPDATE_FLAG:
		case IOCTL_HAL_PARAM_BLOCK:
		case IOCTL_HAL_GET_HW_BOOT_STAT:
		case IOCTL_HAL_GET_HW_BOOT_STAT_EX:
		case IOCTL_HAL_QUERY_CARD_CFG:

            rc = KernelIoControl(code, pInBuffer, inSize, pOutBuffer, outSize, pOutSize);
            break;
		case IOCTL_HAL_GET_POOL_PARAMETERS:
			rc = KernelLibIoControl((HANDLE)KMOD_CORE, IOCTL_KLIB_GET_POOL_STATE, pInBuffer, inSize, pOutBuffer, outSize, pOutSize);
			RETAILMSG(1,(L"IOCTL_HAL_GET_POOL_PARAMETERS: ret %d, err %d\r\n", rc, GetLastError()));
			break;
    }
