...
{
    DWORD code;
    OMAP_IDCORE_REGS *pDieId = OALPAtoUA(OMAP_IDCODE_REGS_PA);

    // Create unique part of name from SoC ID
  //  code  = INREG32(pDieId);
	code  = INREG32(&pDieId->DIE_ID_0);
...
//------------------------------------------------------------------------------
VOID GetRndisMac(UINT8* MacAddr )
{
	BSP_ARGS *pArgs	= (BSP_ARGS *)(IMAGE_SHARE_ARGS_PA);
	memcpy(MacAddr, pArgs->RndisMacAddress, sizeof(pArgs->RndisMacAddress));
}
