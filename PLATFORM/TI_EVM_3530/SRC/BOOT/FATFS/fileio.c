...
// TODO: Add support for total_sectors == 0 (indicating that number_of_sectors_huge should be used)
...
pfnFMD_ReadSectorNotify  g_fnFat_ReadSectorNotify = 0;
UINT32 NotifCount = 100;

// --------------------------------------------------------------------
void SetFatNotify(pfnFMD_ReadSectorNotify fnFat_ReadSectorNotify)
{
 RETAILMSG (0, (TEXT("SetFatNotify\r\n")));
	g_fnFat_ReadSectorNotify = fnFat_ReadSectorNotify;
}
 ...
 *
 */
static int FileIoReadNextSector(S_FILEIO_OPERATIONS_PTR pfileio_ops, PFILEHANDLE pFile, void * pSector) 
{
	UINT32 SectorNumber;
	...
	if (pFile->current_sector_in_cluster == CURRENT_SECTOR_EOF_VALUE)
	{
			...
			OALMSG(OAL_INFO, (L"BOOTLOADER: FileIoReadNextSector EOF\r\n"));
		#endif
	...
	switch (FilesysInfo.FatType)
	{
		...
		case FAT_TYPE_FAT32:
			if (pFile->current_cluster < FAT32_CLUSTER_NEXT_START || (pFile->current_cluster > FAT32_CLUSTER_NEXT_END && pFile->current_cluster < FAT32_CLUSTER_LAST_START))
			{
					...
					OALMSG(OAL_INFO, (L"BOOTLOADER: FileIoReadNextSector failed, invalide current_cluster 0x%x\r\n", pFile->current_cluster));
				#endif
				return FILEIO_STATUS_READ_FAILED;
			}
			break;
	...
	// ??? The reserved fat entry stuff is poorly documented, this code may be wrong
		...
		if (pFile->current_cluster < 2)
			OALMSG(OAL_INFO, (L"BOOTLOADER: FileIoReadNextSector() called with current_cluster < 2!\r\n"));
	#endif
		...
		switch (FilesysInfo.FatType)
		{
			...
			case FAT_TYPE_FAT12:
				...
				else
				{
					...
					FatOffsetInByte = pFile->current_cluster & 1;
					FatOffsetInSector = ((pFile->current_cluster * 3) / 2) % SECTOR_SIZE;
					FatSectorNumber = ((pFile->current_cluster * 3) / 2) / SECTOR_SIZE;
						...
						if (pfileio_ops->read_sector(pfileio_ops->drive_info, (FilesysInfo.partition_start + FatSectorNumber + FilesysInfo.BiosParameterBlock.reserved_sectors) + ExtendedPartionBaseSector, pSector) != ATA_STATUS_OK)
						{
							#if BOOTLOADER_DEBUG_DISPLAY_ERRORS
						...
						// get data from first byte of containing current FAT entry
						if (FatOffsetInByte)
							pFile->current_cluster = (*(((UINT8 *)pSector) + FatOffsetInSector) >> 4) & 0xf;	// lower 4 bits of 12 from upper half of byte
						else
							pFile->current_cluster = *(((UINT8 *)pSector) + FatOffsetInSector);	// lower 8 bits of 12 from entire byte
					#endif
					...
					if (FatOffsetInSector == 511)
					{
						...
						#else
							if (pfileio_ops->read_sector(pfileio_ops->drive_info, (FilesysInfo.partition_start + FatSectorNumber + FilesysInfo.BiosParameterBlock.reserved_sectors + 1) + ExtendedPartionBaseSector, pSector) != ATA_STATUS_OK)
							{
								#if BOOTLOADER_DEBUG_DISPLAY_ERRORS
					...
					#else
						if (FatOffsetInByte)
							pFile->current_cluster |= *(((UINT8 *)pSector) + FatOffsetInSector) << 4;			// upper 8 of 12 from entire byte
						else
							pFile->current_cluster |= (*(((UINT8 *)pSector) + FatOffsetInSector) & 0xf) << 8;	// upper 4 of 12 from lower half of byte
					#endif
			...
			case FAT_TYPE_FAT16:
				...
				{			
					FatOffsetInSector = (pFile->current_cluster * 2) % SECTOR_SIZE;
					FatSectorNumber = (pFile->current_cluster * 2) / SECTOR_SIZE;
						...
						//  update current fat index
						pFile->current_cluster = *(((UINT16 *)pSector) + (FatOffsetInSector/2));
					#endif
					//  reset current_sector_in_cluster
					pFile->current_sector_in_cluster = 0;
			...
			case FAT_TYPE_FAT32:
				...
				{			
					FatOffsetInSector = (pFile->current_cluster * 4) % SECTOR_SIZE;
					FatSectorNumber = (pFile->current_cluster * 4) / SECTOR_SIZE;
						...
						if (FatBufferSectorNumber != FilesysInfo.partition_start + FatSectorNumber + FilesysInfo.BiosParameterBlock.reserved_sectors)
						{
							...
							{
								#if BOOTLOADER_DEBUG_DISPLAY_ERRORS
									OALMSG(OAL_INFO, (L"BOOTLOADER: FileIoReadNextSector failed reading FAT: LBA 0x%x (cluster 0x%x, sector 0x%x)\r\n", FatBufferSectorNumber + ExtendedPartionBaseSector, pFile->current_cluster, pFile->current_sector_in_cluster));
								#endif
								FatBufferSectorNumber = 0;
								return FILEIO_STATUS_READ_FAILED;
							}
	...
	if (pfileio_ops->read_sector(pfileio_ops->drive_info, (FilesysInfo.partition_start + SectorNumber) + ExtendedPartionBaseSector, pSector) != ATA_STATUS_OK)
	{
		#if BOOTLOADER_DEBUG_DISPLAY_ERRORS
			OALMSG(OAL_INFO, (L"BOOTLOADER: FileIoReadNextSector failed reading file or dir: LBA 0x%x, cluster 0x%x, sector in cluster 0x%x\r\n", (FilesysInfo.partition_start + SectorNumber) + ExtendedPartionBaseSector, pFile->current_cluster - 2, pFile->current_sector_in_cluster));
		#endif
		return FILEIO_STATUS_READ_FAILED;
	}
...
void FileNameToDirEntry(LPCWSTR pFileName, PCHAR pName, PCHAR pExtension)
{
	...
	for (i = 0; i < 8; i++)
	{
		...
		{
			pName[i] = toupper((CHAR)(pFileName[i]));
	...
	if (pFileName[i] == L'.')
	{
		...
		for (j = 0; j < 3; j++)
		{
			if (pFileName[i])
			{
				pExtension[j] = toupper((BYTE)(pFileName[i++]));
...
NoPartitionTable:
	...
	switch (FilesysInfo.FatType)
	{
		...
		case FAT_TYPE_FAT16:
			...
			FilesysInfo.root_directory_start = FilesysInfo.BiosParameterBlock.reserved_sectors + (FilesysInfo.BiosParameterBlock.number_of_fats * FilesysInfo.BiosParameterBlock.sectors_per_fat);

			// compute file data area starting sector
			FilesysInfo.file_data_area_start = FilesysInfo.root_directory_start + (FilesysInfo.BiosParameterBlock.number_of_root_directory_entries/(SECTOR_SIZE/sizeof(DIRECTORY_ENTRY)));
		...
		case FAT_TYPE_FAT32:
			...
			FilesysInfo.file_data_area_start = FilesysInfo.BiosParameterBlock.reserved_sectors + (FilesysInfo.BiosParameterBlock.number_of_fats * FilesysInfo.BiosParameterBlock.big_sectors_per_fat);
...
int FileIoOpen(S_FILEIO_OPERATIONS_PTR pfileio_ops, PFILEHANDLE pFile) 
{
	...
	switch (FilesysInfo.FatType)
	{
		...
		case FAT_TYPE_FAT32:
			...
			if ((status = FileIoReadNextSector(pfileio_ops, pFile, pSector)) != FILEIO_STATUS_OK)
			{
				#if BOOTLOADER_DEBUG_DISPLAY_ERRORS
					OALMSG(1, (L"BOOTLOADER: read_sector %u (DIR) failed\r\n", (FilesysInfo.partition_start + sector_number) + ExtendedPartionBaseSector));
				#endif
	...
	for ( ; ; )
	{
		if( g_fnFat_ReadSectorNotify )
		{
			if( ++NotifCount > 99 )
			{
				NotifCount = 0;
				g_fnFat_ReadSectorNotify();
			}
		}
		...
		#endif		
		{
			...
			UINT8 * pExtension = (((DIRECTORY_ENTRY *)pSector) + entry)->extension;
			...
			if (pName[0] == 0x05)
				...
				OALMSG(1, (L"  Directory Entry #%d: <%c%c%c%c%c%c%c%c %c%c%c> ", DirEntryCount, pName[0], pName[1], pName[2], pName[3], pName[4], pName[5], pName[6], pName[7], pExtension[0], pExtension[1], pExtension[2]));
			#endif
			...
			if (i == 8)
			{
				...
				if (i == 3) 
				{
					...
					pFile->bytes_in_buffer = 0;
					#if BOOTLOADER_DEBUG_DISPLAY_DIRECTORY_ENTRIES
						OALMSG(1, (L"Found file, starting cluster = %x, file size = %X\r\n", pFile->current_cluster, pFile->file_size));
					#endif
					return FILEIO_STATUS_OK;
				}
			}
		...
		switch (FilesysInfo.FatType)
		{
			...
			case FAT_TYPE_FAT16:
				...
				#if BOOTLOADER_DEBUG_DISPLAY_DIRECTORY_ENTRIES
					OALMSG(1, (L"Next directory sector %d, file data area start %d\r\n", sector_number, FilesysInfo.file_data_area_start));
				#endif
				...
				{
					#if BOOTLOADER_DEBUG_DISPLAY_ERRORS
						OALMSG(1, (L"BOOTLOADER: read_sector %u (next DIR) failed\r\n", (FilesysInfo.partition_start + sector_number) + ExtendedPartionBaseSector));
					#endif
					return FILEIO_STATUS_OPEN_FAILED;
				}
...
int FileIoRead(S_FILEIO_OPERATIONS_PTR pfileio_ops, PFILEHANDLE pFile, void * pDest, DWORD Count)
{
	int status;
	UINT8 * s;
	RETAILMSG(0,(L"FileIoRead: count = %d\r\n", Count));
	while (Count)
	{
		...
		{
			if ((status = FileIoReadNextSector(pfileio_ops, pFile, (UINT8 *)pDest)) != FILEIO_STATUS_OK)
				return status;

			(UINT8 *)pDest += SECTOR_SIZE;
			Count -= SECTOR_SIZE;
		...
		if (Count)
		{
			if ((status = FileIoReadNextSector(pfileio_ops, pFile, pFile->buffer)) != FILEIO_STATUS_OK)
				return status;
