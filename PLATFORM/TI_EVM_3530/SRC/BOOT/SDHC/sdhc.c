...
#define MMC1_GPIO_CARDDET              192     // Triton GPIO 0 
#define MMC2_GPIO_CARDDET              193     // Triton GPIO 1

#define MMC1_GPIO_CARDDET_ENABLE_MASK  0x01    // 
#define MMC2_GPIO_CARDDET_ENABLE_MASK  0x02    // 
#define T2_GPIO_MODULE_ENABLE_MASK     0x04    // 

#define MMC1_GPIO_CARDDET_PU_PD_MASK   0xFC    // disable PU / PD
#define MMC1_GPIO_CARDDET_PU           0x02    // enable PU
#define MMC2_GPIO_CARDDET_PU_PD_MASK   0xF3    // disable PU / PD
#define MMC2_GPIO_CARDDET_PU           0x08    // enable PU

#define MMC1_GPIO_CARDDET_DEB          0x01    // enable debounce
#define MMC2_GPIO_CARDDET_DEB          0x02    // enable debounce
...
static BOOL InitializeT2GPIO()
{
    ...
    m_hTWL = OALTritonOpen();
    if (m_hTWL == NULL)
    {
        OALMSG(OAL_ERROR, (L"open TWL failed\r\n"));
        return FALSE;
    }
    ...
    switch (m_dwSlot)
    {
        case MMCSLOT_1:
            ...
            regval &= MMC1_GPIO_CARDDET_PU_PD_MASK;
            regval |= MMC1_GPIO_CARDDET_PU;
...
static BOOL InitializeHardware(DWORD Slot)
{
    ...
    m_dwMaxClockRate = 24000000;

    if (Slot == MMCSLOT_1)
        m_pbRegisters = OALPAtoUA(OMAP_MMCHS1_REGS_PA);
    else
        m_pbRegisters = OALPAtoUA(OMAP_MMCHS2_REGS_PA);
...
#ifdef MMCHS1_VDDS_WORKAROUND
    m_pSyscGeneralRegs = (OMAP_SYSC_GENERAL_REGS *) OALPAtoUA(OMAP_SYSC_GENERAL_REGS_PA);
#endif
...
static void DeinitializeHardware()
{
    ...
    if (m_hTWL != NULL)
    {
        switch (m_dwSlot)
        {
          case MMCSLOT_1:
            OALTritonRead(m_hTWL, TWL_GPIO_CTRL, &regval);
            regval &= ~MMC1_GPIO_CARDDET_ENABLE_MASK;
            OALTritonWrite(m_hTWL, TWL_GPIO_CTRL, regval);
            break;
...
static void SdhcControllerInit()
{
    ...
    // turn power and system clocks on
    #ifdef MMCHS1_VDDS_WORKAROUND
        {
            ...
            {
                // Note: MMCSLOT_2 not supported on EVM
                CLRREG32(&m_pSyscGeneralRegs->CONTROL_DEVCONF1, 1 << 6);
            }
            ...
            if (m_dwSlot == MMCSLOT_1)
            {
                ...
                //m_pSyscGeneralRegs->CONTROL_DEVCONF1 |= 1 << 24;  // reserved bit
            }
            else if (m_dwSlot == MMCSLOT_2)
            {
                // Note: MMCSLOT_2 not supported on EVM
...
VOID SdhcSetInterface(DWORD mode)
{
    ...
    {
#ifndef SHIP_BUILD
        RETAILMSG(1, (TEXT("SDHC: 4 bit mode\r\n")));
#endif
    ...
    {
#ifndef SHIP_BUILD
        RETAILMSG(1, (TEXT("SDHC: 1 bit mode\r\n")));
#endif
...
static BOOL SDIPollingReceive(PBYTE pBuff, DWORD dwLen)
{
    ...
    DWORD dwCountStart;
    BOOL fTimeoutOverflow = FALSE;
    BOOL bAlignedBuffer = (DWORD)pBuff & 0x3 ? FALSE : TRUE;
    ...
    if (dwLen % MMC_BLOCK_SIZE || m_dwSDIOCard)
    {
        ...
        if (dwLen % sizeof(DWORD)) 
		    fifoSizeW++;
        OALMSGX(OAL_IO, (TEXT("SDIPollingReceive reading data 1\r\n")));
