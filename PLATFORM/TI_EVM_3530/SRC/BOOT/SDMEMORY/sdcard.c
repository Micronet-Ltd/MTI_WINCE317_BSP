...
Detect4BitCardDone:
    ...
    if (pDisk->d_Supports4Bit)
    {
        ...
        {
            RETAILMSG(1, (TEXT("Using 4 bit mode\r\n")));
...
//
UINT32 BLSDCardDownload(BOOT_CFG *pConfig, OAL_KITL_DEVICE *pBootDevices)
{
	...
	if(Disk.d_DiskCardState != STATE_OPENED)
	{
    ...
    FileNameToDirEntry(pConfig->filename, pFile->name, pFile->extension);
    ...
    if( !memcmp( pFile->extension, "BMP", 3 ) )
		return BL_DOWNLOAD_BMP;
	else
		return BL_DOWNLOAD;
}
...
{
    OALMSGX(OAL_FUNC, (L"BLSDCardConfig\r\n"));
}
//read the whole file - return file size
UINT32 BLSDCardReadFileData(ULONG MaxSize, UCHAR *pData)
{
    // called to read data from MMC/SD card as stream data, not as block data
	UINT32 size = 0;

    OALMSGX(OAL_FUNC, (L"BLSDCardReadFileData: address 0x%x, %d bytes\r\n", pData, size));

    if( pFile && (MaxSize >= pFile->file_size) )
	{	
		if (FileIoRead(&fileio_ops, pFile, (PVOID)pData, pFile->file_size) == FILEIO_STATUS_OK)
			size = pFile->file_size;
	}
    return size;
}