...
//BOOL  (*pfnEndPlayCallBack)( DWORD playerRet );
typedef BOOL (*ENDPLAYCALLBACK)(DWORD);
...
public:
 ...
 ENDPLAYCALLBACK pfnEndPlayCallBack;

 HWND   m_hDlg;
 DWORD  m_PlayProgressBar, m_PlayProgressText;

 DWORD  Init(ENDPLAYCALLBACK pfnCallBack )
{
  if( pfnCallBack == NULL )
   return(-1);

  pfnEndPlayCallBack = pfnCallBack;
...
//	WAVEHDR whdr;
    ...
    if (format->wFormatTag != WAVE_FORMAT_PCM)
	 ...
	 err = waveOutOpen( &m_hwo, 0, format, (unsigned long)m_hDoneEvent, 0, CALLBACK_EVENT | WAVE_FORMAT_DIRECT);

	// Check if handle to waveout exists
	if( m_hwo == 0 )
      return -1;  
...
DWORD PrepAction()
{
  TCHAR      myTest[32];
	...
	unsigned int	min, sec, curPlayTime;
	unsigned int	totalMin, totalSec;
...
//	unsigned unsigned int old_vol;
	...
	unsigned int		totalPlayTime = m_WaveFile.get_PlayTime();

	totalMin = totalPlayTime/60;
	totalSec = totalPlayTime%60;
	
	unsigned int		GlobalLen(0), DataLen;

    float		        fGlobalLenCalc(0.0);
	float		        fFileLenCalc(0);
...
//			break;
		...
		// m_hDlg
        fFileLenCalc = (float)FileLen;
		fGlobalLenCalc = (float)GlobalLen;

		 barPosition = (unsigned int) ((fGlobalLenCalc * 100.0)/fFileLenCalc);
		 SendDlgItemMessage(m_hDlg, m_PlayProgressBar, PBM_SETPOS, barPosition, 0);

		 curPlayTime = (unsigned int) ((fGlobalLenCalc * (float)totalPlayTime)/fFileLenCalc); 
		 min = curPlayTime / 60;
		 sec = curPlayTime % 60;

		wsprintf( myTest, _T("%02d:%02d/%02d:%02d"), min, sec, totalMin, totalSec );
		SetDlgItemText( m_hDlg, m_PlayProgressText, myTest );
	}


	SendDlgItemMessage(m_hDlg, m_PlayProgressBar, PBM_SETPOS, 0, 0);
	wsprintf( myTest, _T("--:--/--:--"));
	SetDlgItemText( m_hDlg, m_PlayProgressText, myTest );
...
private:
	...
	static DWORD WINAPI  WaitForActionThreadProc(LPVOID lpParameter)
	{
		...
		This->pfnEndPlayCallBack(Ret);
		return Ret; // Return from playing here
 	}
