...
static BOOL WaitIncomingApp(HANDLE hCom);
...
extern "C" HANDLE WINAPI __OpenComPortTest(LPCTSTR pszPortName)
{
 ...
 static BOOL WaitIncomingApp(HANDLE hCom)
{
  RETAILMSG(1, (L"GSM API:+%S, hCom = 0x%X\r\n",__FUNCTION__, hCom));
      ...
      while(hCom != INVALID_HANDLE_VALUE)
      {
			...
			RETAILMSG(1, (L"GSM API:-%S Ok\r\n",__FUNCTION__));
            return TRUE;
      }
	  RETAILMSG(1, (L"GSM API:-%S Fail\r\n",__FUNCTION__));
      return FALSE;
}
...
BOOL sendATCmdApp(HANDLE hCom, const char *cmdStr, char *respStr)
{
 ...
 RETAILMSG(1, (L"GSM API:+%S, cmdStr = %S\r\n",__FUNCTION__, cmdStr));
 ...
 if( strnlen(cmdStr, MAX_SIZE_TO_READ) >= MAX_SIZE_TO_READ )
 {
  ...
  return(FALSE);
	 ...
	 {
       RETAILMSG(1, (L"GSM API:%S, cmdStr sent %d bytes\r\n",__FUNCTION__, NumBytesWritten));
	 }
	 else
	 {
  ...
  RETAILMSG(1, (L"GSM API:%S, ret = %d, timeout = %d ms, char cnt = %d\r\n",__FUNCTION__, ret, (timeout), charCnt));
#if 1
  DWORD i;

  RETAILMSG(1, (L"GSM API:%S, respond string:\r\n",__FUNCTION__));
  for( i = 0; i < charCnt; i++ )
  {
    if( respStr[i] == '\r' )
     RETAILMSG(1, (L"[\\r]"));
     ...
     RETAILMSG(1, (L"[\\n]"));
      ...
      RETAILMSG(1, (L"%c",respStr[i])); 
  }
  RETAILMSG(1, (L"\r\n\r\n"));
#endif

  //RETAILMSG(1, (L"GSM API:-%S, ret = %d\r\n",__FUNCTION__, ret));
  return(ret);
