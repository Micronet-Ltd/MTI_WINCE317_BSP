...
//
/*
    ipaddr.cpp - TCP/IP Address custom control
...
extern "C" void RegisterIPClass(HINSTANCE hInst)
{
...
extern "C" void UnregisterIPClass(HINSTANCE hInst)
{
...
LONG CALLBACK IPAddressWndFn(HWND hWnd, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch( wMsg )
    {
        ...
        case WM_CREATE : /* create pallette window */
        {
            ...
            pControl = (CONTROL *) LocalAlloc(LPTR, sizeof(CONTROL));
            if (pControl)
            {
#define LPCS	((CREATESTRUCT FAR *)lParam)
                ...
                for (i = 0; i < NUM_FIELDS; ++i)
                {
					...
					//IME DISABLE
					ImmAssociateContext(pControl->Children[i].hWnd, (HIMC)NULL);
    ...
    switch (wMsg)
    {
        ...
        case WM_CHAR:
            ...
            if (wParam >= '0' && wParam <= '9')
            {
				...
				BOOL ifExit = ExitField(pControl, dwChildID);
