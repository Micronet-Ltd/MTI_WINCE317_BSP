...
void _BFSFGetDisplayInfo(PBFSF pbfsf, TV_DISPINFO *lpnm)
{
    ...
    if (lpnm->item.mask & TVIF_TEXT)
    {
        ...
        {
            wcscpy(lpnm->item.pszText, str.pOleStr);
            ti.mask |= TVIF_TEXT;
...
/*
    Utility functions ported from desktop.  If these are needed by other functions, 
    these can be moved to another module.
*/
