...
namespace pvrengine
{
		...
		case eUsLIGHTPOSMODEL:
			{
				...
				PVRTVec4 vLightPosModel = ((LightPoint*)m_pLightManager->getLight(sUniform.getIdx()))->getPositionPVRTVec4() *m_mWorld ;
		...
		case eUsLIGHTDIRMODEL:
			{
				...
				PVRTVec4 vLightDirectionModel,
					vLightDirection =((LightDirectional*)m_pLightManager->getLight(sUniform.getIdx()))->getDirectionPVRTVec4();
			...
			break;
		case eUsEYEPOSMODEL:
			{	//TODO: Fix this to work with where the camera actually is and ignore that POD stuff
				//PVRTVec3 vFrom, vTo;
				//m_psScene->GetCameraPos(vFrom,vTo,0);	// unsure about 3rd param
				//PVRTVECTOR4	vPos;
				//PVRTTransformBack(&vPos, (PVRTVECTOR4*)&vFrom, &m_mWorld);	// cast is a bit dodgy
				//glExtensions->glUniform3fARB(sUniform.getLocation(), vPos.x, vPos.y, vPos.z);
			}
			break;
		default:
	...
	void UniformHandler::CalculateFrameUniform(const Uniform& sUniform)
	{
		...
		case eUsLIGHTPOSWORLD:
			{
				m_vLightPosWorld[index] = ((LightPoint*)m_pLightManager->getLight(index))->getPositionPVRTVec4();
			}
			break;
		case eUsLIGHTPOSEYE:
			{
		...
		case eUsLIGHTDIRWORLD:
			{
				...
				m_vLightDirWorld[index] = ((LightDirectional*)m_pLightManager->getLight(index))->getDirectionPVRTVec4();
		...
		case eUsEYEPOSWORLD:
			{
				//TODO: FIx me!!!
				//PVRTVec3 vTo;
				//m_psScene->GetCameraPos(m_vEyePositionWorld,vTo,0);	// unsure about 3rd param ; camera num???
			}
			break;
		case eUsANIMATION:
			{
