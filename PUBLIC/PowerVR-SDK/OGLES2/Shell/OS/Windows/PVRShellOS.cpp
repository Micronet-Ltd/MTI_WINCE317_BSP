...
#ifndef NO_GDI
{
	...
	*/
	...
	{
		m_hWnd = CreateWindow(WINDOW_CLASS, appName, WS_VISIBLE,
		   	0, 0, m_pShell->m_pShellData->nShellDimX, m_pShell->m_pShellData->nShellDimY, NULL, NULL, m_hInstance, this);
	}
	else
	{
		int x, y;
...
void PVRShellInit::OsDisplayDebugString(char const * const str)
{
	if(str)
	{
		...
		do{
			strc[i] = (wchar_t)str[i];
		}while ((str[i] != '\0') && (i++ < (sizeof(strc) / sizeof(*strc))));
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		{
			...
			SetWindowLong(hWnd, GWL_USERDATA, (LONG)pCreate->lpCreateParams); // Not ideal, but WinCE doesn't have SetWindowLongPtr
	...
	case WM_LBUTTONDOWN:
		{
			if(pData->m_pShell->PVRShellGet(prefFullScreen))
			{
				pData->gShellDone = true;
			}
			break;
	...
	case WM_HOTKEY:
		{
			...
			{
			case 0xC1:
				pData->nLastKeyPressed = PVRShellKeyNameScreenshot;
				break;
			case 0xC2:
				pData->nLastKeyPressed = PVRShellKeyNameACTION1;
				break;
			case 0xC3:
				pData->nLastKeyPressed = PVRShellKeyNameACTION2;
				break;
			case 0xC4:
				pData->nLastKeyPressed = PVRShellKeyNameQUIT;
				break;
			}
			break;
		}
	...
	case WM_KEYDOWN:
	{
		...
		case VK_UP:
		case 0x35:
			pData->nLastKeyPressed = PVRShellKeyNameUP;
			break;
		case VK_DOWN:
		case 0x30:
			pData->nLastKeyPressed = PVRShellKeyNameDOWN;
			break;
		case VK_LEFT:
		case 0x37:
			pData->nLastKeyPressed = PVRShellKeyNameLEFT;
			break;
		case VK_RIGHT:
		case 0x39:
			pData->nLastKeyPressed = PVRShellKeyNameRIGHT;
			break;
		case VK_F23:	// X50v select button
		case VK_SPACE:
		case 0x38:
			pData->nLastKeyPressed = PVRShellKeyNameSELECT;
			break;
		case '1':
		case 0x34:
			pData->nLastKeyPressed = PVRShellKeyNameACTION1;
