...
const char c_szSceneFile[] = "Head_fixed.pod";
#else
const char c_szSceneFile[] = "Head_float.pod";
#endif
...
bool OGLESPolybump::InitView()
{
	...
	m_mProjection = PVRTMat4::PerspectiveFovRH(f2vt(30.0f*(3.14f/180.0f)), f2vt((float)PVRShellGet(prefWidth)/(float)PVRShellGet(prefHeight)), f2vt(10.0f), f2vt(8000.0f), PVRTMat4::OGL, bRotate);
...
bool OGLESPolybump::RenderScene()
{
	...
	LightVector.x = PVRTSIN(VERTTYPEDIV(f2vt(m_i32Frame), f2vt(40.0f)));
	LightVector.y = f2vt(0.0f);
	LightVector.z = -PVRTABS(PVRTCOS(VERTTYPEDIV(f2vt(m_i32Frame), f2vt(40.0f))));
	LightVector.w = f2vt(0.0f);
	...
	else
	{
		...
		myglLightv(GL_LIGHT0, GL_POSITION, &LightVector.x);
	}
	...
	mModelView = m_mView * PVRTMat4::RotationY(f2vt(sin(m_i32Frame * 0.003f) - PVRT_PI_OVER_TWOf));
...
void OGLESPolybump::CalculateDot3LightDirection(PVRTVec4 Dot3LightPos)
{
	 ...
	 */
	myglColor4(Dot3LightPos.y, Dot3LightPos.z, Dot3LightPos.x, f2vt(1.0f));
}
