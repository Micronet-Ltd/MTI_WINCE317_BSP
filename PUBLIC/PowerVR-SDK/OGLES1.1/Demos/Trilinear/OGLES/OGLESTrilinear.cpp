...
const char c_szSceneFile[]	= "o_model_fixed.pod";
#else
const char c_szSceneFile[]	= "o_model_float.pod";
#endif
...
bool OGLESTrilinear::InitView()
{
	...
	m_mProjection = PVRTMat4::PerspectiveFovRH(fFOV, f2vt((float)PVRShellGet(prefWidth)/(float)PVRShellGet(prefHeight)), f2vt(g_CameraNear), f2vt(g_CameraFar), PVRTMat4::OGL, bRotate);
...
void OGLESTrilinear::DrawSphere(eTapePosition ePosition)
{
	...
	*/
	...
	mScale     = PVRTMat4::Scale(f2vt(0.9f), f2vt(0.9f), f2vt(0.9f));
	mTranslate = PVRTMat4::Translation(f2vt(0.0f), fSpherePosY, fSpherePosZ);
	mRotateX   = PVRTMat4::RotationX(f2vt(m_fFrame/50.0f));
	mRotateY   = PVRTMat4::RotationY(f2vt(m_fFrame/50.0f));
...
void OGLESTrilinear::DrawBanner(eTapePosition ePosition)
{
	...
	*/
	...
	// Calculates the sphere specific transformation matrices
	mSpecificModel = PVRTMat4::Translation(f2vt(0.0f), fSpherePosY, fSpherePosZ);
...
void OGLESTrilinear::GetModelMatrixFromPosition(PVRTMat4 &mModel, eTapePosition ePosition)
{
	...
	{
		case eLeft:
			mModel = PVRTMat4::Translation(f2vt(-110),f2vt(0),f2vt(0));
			break;
		case eMiddle:
...
void OGLESTrilinear::ComputeTapeVertices(eTapePosition ePosition)
{
	...
	unsigned char* pfY = pMesh->pInterleaved + (size_t)pMesh->sVertex.pData + sizeof(VERTTYPE) * 1;
	unsigned char* pfZ = pMesh->pInterleaved + (size_t)pMesh->sVertex.pData + sizeof(VERTTYPE) * 2;
