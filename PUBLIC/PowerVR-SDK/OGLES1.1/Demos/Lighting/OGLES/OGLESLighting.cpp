...
const char c_szSceneFile[] = "LightingScene_fixed.pod";
#else
const char c_szSceneFile[] = "LightingScene_float.pod";
#endif
...
bool OGLESLighting::InitView()
{
	...
	myglMaterialv(GL_FRONT_AND_BACK, GL_AMBIENT, Ambient);
	myglMaterialv(GL_FRONT_AND_BACK, GL_DIFFUSE, Diffuse);
	myglMaterialv(GL_FRONT_AND_BACK, GL_SPECULAR, Specular);
	myglMaterial(GL_FRONT_AND_BACK, GL_SHININESS, f2vt(10.0f));	// Nice and shiny so we don't get aliasing from the 1/2 angle
	...
	mProjection = PVRTMat4::PerspectiveFovRH(f2vt(20.0f*(PVRT_PIf/180.0f)), f2vt((float)PVRShellGet(prefWidth)/(float)PVRShellGet(prefHeight)), f2vt(10.0f), f2vt(1200.0f), PVRTMat4::OGL, bRotate);
...
bool OGLESLighting::RenderScene()
{
	...
	RotationMatrix = PVRTMat4::RotationY(f2vt((-m_i32FrameNo * 0.1f) * PVRT_PIf / 180.0f));
	...
	// Display info text
	m_Print3D.DisplayDefaultTitle("OGLESLighting", "8 point lights.", ePVRTPrint3DSDKLogo);
	m_Print3D.Flush();
...
void OGLESLighting::InitLight(SLightVars &Light)
{
	...
	Light.Ambient.x = f2vt(0.0);
	Light.Ambient.y = f2vt(0.0);
	Light.Ambient.z = f2vt(0.0);
	Light.Ambient.w = f2vt(1.0);

	// Light Diffuse colour
	double difFac = 0.4;
	Light.Diffuse.x = f2vt((float)( difFac * (rand()/(double)RAND_MAX) ) * 2.0f); //1.0;
	Light.Diffuse.y = f2vt((float)( difFac * (rand()/(double)RAND_MAX) ) * 2.0f); //1.0;
	Light.Diffuse.z = f2vt((float)( difFac * (rand()/(double)RAND_MAX) ) * 2.0f); //1.0;
	Light.Diffuse.w = f2vt((float)( 1.0 ));

	// Light Specular colour
	double specFac = 0.1;
	Light.Specular.x = f2vt((float)( specFac * (rand()/(double)RAND_MAX) ) * 2.0f); //1.0;
	Light.Specular.y = f2vt((float)( specFac * (rand()/(double)RAND_MAX) ) * 2.0f); //1.0;
	Light.Specular.z = f2vt((float)( specFac * (rand()/(double)RAND_MAX) ) * 2.0f); //1.0;
	Light.Specular.w = f2vt((float)( 1.0 ));

	// Randomize some of the other parameters
	float lightDist = 80.0f;
	Light.vPosition.x = f2vt((float)((rand()/(double)RAND_MAX) * lightDist/2.0f ) + lightDist/2.0f);
	Light.vPosition.y = f2vt((float)((rand()/(double)RAND_MAX) * lightDist/2.0f ) + lightDist/2.0f);
	Light.vPosition.z = f2vt((float)((rand()/(double)RAND_MAX) * lightDist/2.0f ) + lightDist/2.0f);

	float rStep = 2;
	Light.vRotationStep.x = f2vt((float)( rStep/2.0 - (rand()/(double)RAND_MAX) * rStep ));
	Light.vRotationStep.y = f2vt((float)( rStep/2.0 - (rand()/(double)RAND_MAX) * rStep ));
	Light.vRotationStep.z = f2vt((float)( rStep/2.0 - (rand()/(double)RAND_MAX) * rStep ));

	Light.vRotation.x = f2vt(0.0f);
	Light.vRotation.y = f2vt(0.0f);
	Light.vRotation.z = f2vt(0.0f);

	Light.vRotationCentre.x = f2vt(0.0f);
	Light.vRotationCentre.y = f2vt(0.0f);
	Light.vRotationCentre.z = f2vt(0.0f);
}
...
void OGLESLighting::StepLight(SLightVars &Light)
{
	...
	while(Light.vRotation.x > f2vt(360.0f)) Light.vRotation.x -= f2vt(360.0f);
	while(Light.vRotation.y > f2vt(360.0f)) Light.vRotation.y -= f2vt(360.0f);
	while(Light.vRotation.z > f2vt(360.0f)) Light.vRotation.z -= f2vt(360.0f);

	// Create three rotations from rotation angles
	RotationMatrixX = PVRTMat4::RotationX(VERTTYPEMUL(Light.vRotation.x, f2vt(PVRT_PIf / 180.0f)));
	RotationMatrixY = PVRTMat4::RotationY(VERTTYPEMUL(Light.vRotation.y, f2vt(PVRT_PIf / 180.0f)));
	RotationMatrixZ = PVRTMat4::RotationZ(VERTTYPEMUL(Light.vRotation.z, f2vt(PVRT_PIf / 180.0f)));
...
void OGLESLighting::DrawLight(SLightVars &Light)
{
	...
	myglColor4(VERTTYPEMUL(Light.Diffuse.x, f2vt(2.0f)), VERTTYPEMUL(Light.Diffuse.y,f2vt(2.0f)), VERTTYPEMUL(Light.Diffuse.z,f2vt(2.0f)), f2vt(1));
