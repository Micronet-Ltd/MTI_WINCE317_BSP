...
const char c_szSceneFile[]	= "Mouse_fixed.pod";
#else
const char c_szSceneFile[]	= "Mouse_float.pod";
#endif
/******************************************************************************
 Defines
******************************************************************************/
...
const char c_szDescriptionNonVGP[] = "Toon Shading";
const char c_szDescriptionVGP[]    = "VGP Toon Shading";
...
class OGLESMouse : public PVRShell
{
	...
	VERTTYPE	*m_pUVBuffer;	// Used for software processing
...
bool OGLESMouse::InitView()
{
	...
	*/
	if(m_bUseVertexProgram)
	{
		...
		if(strstr(pszRendererString, "PowerVR"))
		{
			...
			{
				// VGPLite present
				m_bUseVertexProgram = InitVertexProgram(vgp_CSH_VGPLITE,sizeof(vgp_CSH_VGPLITE));
			}
			else // Assume VGP is present
			{
				m_bUseVertexProgram = InitVertexProgram(vgp_CSH_VGP,sizeof(vgp_CSH_VGP));

				if(!m_bUseVertexProgram)
				{
					// Try loading ARM VP Vertex Program instead
					m_bUseVertexProgram = InitVertexProgram(vgp_CSH_VGPARMVP,sizeof(vgp_CSH_VGPARMVP));
				}
			}
		}
		else
	...
	m_pDescription = m_bUseVertexProgram ? c_szDescriptionVGP : c_szDescriptionNonVGP;
	...
	*/
	...
	PVRTMatrixPerspectiveFovRH(ProjectionMatrix, f2vt(20.0f * (3.14f / 180.0f)), f2vt(fWidth / fHeight), f2vt(800.0f), f2vt(2800.0f));
	myglMultMatrix(ProjectionMatrix.f);
	...
	*/
	...
	/*
		If we're not using the vertex program to calculate the second set of UVs
		then we have to do it in software.
	*/
	...
	if(!m_bUseVertexProgram)
	{
		...
		m_pUVBuffer = new VERTTYPE[i32MaxVertexNo * 2];
...
bool OGLESMouse::ReleaseView()
{
	...
	if(m_bUseVertexProgram)
		m_Extensions.glDeleteProgramsARB(1, &m_ui32VGPProgram);
	else
	{
...
bool OGLESMouse::RenderScene()
{
	...
	*/
	...
	if(m_bUseVertexProgram)
	{
		...
		m_Extensions.glBindProgramARB(GL_VERTEX_PROGRAM_ARB, m_ui32VGPProgram);
	}

	// Set the current frame
	m_Scene.SetFrame(f2vt(m_fFrame));
	...
	// Draw Text
	m_Print3D.DisplayDefaultTitle("Mouse", m_pDescription, ePVRTPrint3DSDKLogo);
	m_Print3D.Flush();
...
******************************************************************************/
bool OGLESMouse::InitVertexProgram(const void *pCSH, int nSzCSH)
{
	// Load VGP code
	m_Extensions.glGenProgramsARB(1, &m_ui32VGPProgram);
	m_Extensions.glBindProgramARB(GL_VERTEX_PROGRAM_ARB, m_ui32VGPProgram);
	m_Extensions.glProgramStringARB(GL_VERTEX_PROGRAM_ARB, GL_PROGRAM_FORMAT_BINARY_IMG, nSzCSH, pCSH);
...
void OGLESMouse::DrawMesh(unsigned int ui32MeshID)
{
	...
	/*
		If texture programs are not available, do software processing of
		the second set of UVs to generate the Toon texture coordinates.
	*/
	if(!m_bUseVertexProgram)
	{
		VERTTYPE *pNormals;
		...
		{
			pNormals = (VERTTYPE*) pData;

            m_pUVBuffer[i * 2 + 0] = VERTTYPEMUL(pNormals[0], m_CameraInverse.x) +
									 VERTTYPEMUL(pNormals[1], m_CameraInverse.y) +
									 VERTTYPEMUL(pNormals[2], m_CameraInverse.z);

			m_pUVBuffer[i * 2 + 1] = VERTTYPEMUL(pNormals[0], m_LightInverse.x) +
									 VERTTYPEMUL(pNormals[1], m_LightInverse.y) +
									 VERTTYPEMUL(pNormals[2], m_LightInverse.z);
		...
		glEnableClientState(GL_TEXTURE_COORD_ARRAY);
		glTexCoordPointer(2,VERTTYPEENUM,0, m_pUVBuffer);
	}
	...
	// Disable second layer texture coords if vertex programs were not used
	if(!m_bUseVertexProgram)
	{
...
void OGLESMouse::SetupCamAndLightInverse(PVRTMat4 mModelView)
{
	...
	CameraPos.x = f2vt(0.0f);
	CameraPos.y = f2vt(0.0f);
	CameraPos.z = f2vt(1000.0f);
	CameraPos.w = f2vt(1.0f);

	m_CameraInverse = mModelViewInverse * CameraPos;

	PVRTMatrixVec3Normalize((PVRTVec3&)m_CameraInverse, (PVRTVec3&)m_CameraInverse);

	if(m_bUseVertexProgram)
		m_Extensions.myglProgramLocalParameter4v(GL_VERTEX_PROGRAM_ARB,0,&m_CameraInverse.x);  // Camera Position

	//	Setup the light position in the VGP code.
	LightPos.x = f2vt(10000.0f);
	LightPos.y = f2vt(0.0f);
	LightPos.z = f2vt(0.0f);
	LightPos.w = f2vt(1.0f);

	m_LightInverse = mModelViewInverse * LightPos;

	PVRTMatrixVec3Normalize((PVRTVec3&)m_LightInverse, (PVRTVec3&)m_LightInverse);

	if(m_bUseVertexProgram)
        m_Extensions.myglProgramLocalParameter4v(GL_VERTEX_PROGRAM_ARB,1,&m_LightInverse.x);  // Light Position
}
