/******************************************************************************
 ...
 @Copyright    Copyright (C) 2006 - 2008 by Imagination Technologies Limited.
...
const char c_szSceneFile[]	= "model_fixed.pod";
#else
const char c_szSceneFile[]	= "model_float.pod";
#endif
...
bool OGLESMatrixPalette::InitView()
{
	...
	PVRTVec4 lightGlobalAmbient = PVRTVec4(f2vt(1.0f), f2vt(1.0f), f2vt(1.0f), f2vt(1.0f));
	myglLightModelv(GL_LIGHT_MODEL_AMBIENT, lightGlobalAmbient.ptr());

	// Setup a directional light source
	PVRTVec4 lightPosition = PVRTVec4(f2vt(-0.7f), f2vt(-1.0f), f2vt(+0.2f), f2vt(0.0f));
    PVRTVec4 lightAmbient  = PVRTVec4(f2vt(1.0f), f2vt(1.0f), f2vt(1.0f), f2vt(1.0f));
    PVRTVec4 lightDiffuse  = PVRTVec4(f2vt(1.0f), f2vt(1.0f), f2vt(1.0f), f2vt(1.0f));
    PVRTVec4 lightSpecular = PVRTVec4(f2vt(0.2f), f2vt(0.2f), f2vt(0.2f), f2vt(1.0f));

    glEnable(GL_LIGHT0);
    myglLightv(GL_LIGHT0, GL_POSITION, lightPosition.ptr());
    myglLightv(GL_LIGHT0, GL_AMBIENT,  lightAmbient.ptr());
    myglLightv(GL_LIGHT0, GL_DIFFUSE,  lightDiffuse.ptr());
    myglLightv(GL_LIGHT0, GL_SPECULAR, lightSpecular.ptr());
...
void OGLESMatrixPalette::LoadMaterial(int i32Index)
{
	...
	PVRTVec4 fMat;
	int i;
	fMat.ptr()[3] = f2vt(1.0f);

	for(i = 0; i < 3; ++i)
		fMat.ptr()[i] = pMaterial->pfMatAmbient[i];

	myglMaterialv(GL_FRONT_AND_BACK, GL_AMBIENT, fMat.ptr());

	for(i = 0; i < 3; ++i)
		fMat.ptr()[i] = pMaterial->pfMatDiffuse[i];

	myglMaterialv(GL_FRONT_AND_BACK, GL_DIFFUSE, fMat.ptr());

	for(i = 0; i < 3; ++i)
		fMat.ptr()[i] = pMaterial->pfMatSpecular[i];

	myglMaterialv(GL_FRONT_AND_BACK, GL_SPECULAR, fMat.ptr());
	myglMaterial(GL_FRONT_AND_BACK, GL_SHININESS, pMaterial->fMatShininess);
}
...
void OGLESMatrixPalette::DrawModel()
{
	...
	for(int iNode = 0; iNode < (int)m_Scene.nNumMeshNode; ++iNode)
	{
		...
		if(bSkinning)
		{
			...
			m_Extensions.glWeightPointerOES(pMesh->sBoneWeight.n, VERTTYPEENUM, pMesh->sBoneWeight.nStride, pMesh->sBoneWeight.pData);
		}
...
void OGLESMatrixPalette::CameraGetMatrix()
{
	...
	if(m_Scene.nNumCamera)
	{
		...
		*/
		fFOV = VERTTYPEMUL(fFOV, f2vt(0.75f));
	}
	else
		fFOV = VERTTYPEMUL(PVRT_PI, f2vt(0.16667f));

	// Set up the view matrix
	m_mView = PVRTMat4::LookAtRH(vFrom, vTo, vUp);

	// Set up the projection matrix
	m_mProjection = PVRTMat4::PerspectiveFovRH(fFOV, f2vt((float) PVRShellGet(prefWidth) / (float) PVRShellGet(prefHeight)), f2vt(g_fCameraNear), f2vt(g_fCameraFar), PVRTMat4::OGL, bRotate);
}
