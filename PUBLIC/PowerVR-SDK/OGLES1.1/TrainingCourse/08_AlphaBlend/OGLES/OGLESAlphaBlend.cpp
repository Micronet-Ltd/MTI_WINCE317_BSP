...
/******************************************************************************
 Const
******************************************************************************/
const VERTTYPE fBlockWidth  = f2vt(0.31f);
const VERTTYPE fBlockHeight = f2vt(0.284f);
...
bool OGLESAlphaBlend::InitView()
{
	...
	VERTTYPE afVertices[] = {-fBlockWidth, -fBlockHeight, 0, // Position
							 0, 0,							 // UV
...
void OGLESAlphaBlend::DrawQuad(float x1, float y1)
{
	...
	*/
	...
	PVRTMat4 mMat;
	bool bRotate = PVRShellGet(prefIsRotated) && PVRShellGet(prefFullScreen);
	mMat = PVRTMat4::RotationZ( bRotate ? f2vt(-90.0f * PVRT_PIf / 180.0f) : 0 );
	
	myglLoadMatrix(mMat.f);
	...
	*/
	...
	glPushMatrix();
	myglTranslate(f2vt(x1), f2vt(y1), 0);
