                                        ...
                                        TI_BOOL bAllVarIes)
{
	...
	/* Get current connected site */
	pParam->paramType = SITE_MGR_GET_PRIMARY_SITE;
	ret = siteMgr_getParam(pScanResultTable->hSiteMgr, pParam);
	if (ret != NOT_CONNECTED){
		os_memoryCopy (pScanResultTable->hOS, 
			(void *)&lSite, 
			(void *)pParam->content.pPrimarySite, 
			sizeof(lSite));
	}
    ...
    for (uSiteIndex = 0; uSiteIndex < pScanResultTable->uCurrentSiteNumber; uSiteIndex++)
    {
		...
		/* Add SSID to hidden entry if connected to this BS */
		if ((ret != NOT_CONNECTED) && 
			(pSiteEntry->bssid[0] == lSite.bssid[0]) && (pSiteEntry->bssid[1] == lSite.bssid[1]) && (pSiteEntry->bssid[2] == lSite.bssid[2]) &&
			(pSiteEntry->bssid[3] == lSite.bssid[3]) && (pSiteEntry->bssid[4] == lSite.bssid[4]) && (pSiteEntry->bssid[5] == lSite.bssid[5]))
		{
			os_memoryCopy (pScanResultTable->hOS, 
				(void *)pBssid->Ssid.Ssid, 
				(void *)lSite.ssid.str,
				lSite.ssid.len);
			pBssid->Ssid.SsidLength = lSite.ssid.len;
			currentBssidAppearsInList = TI_TRUE;
		}
    ...
    *pLength = uLength;
	...
	//Add current BSS to scanned network - Trimble request
	if ((currentBssidAppearsInList == TI_FALSE) && (ret != NOT_CONNECTED))
	{
		pSite = scanResultTbale_AllocateNewEntry (hScanResultTable);
		os_memoryCopy (pScanResultTable->hOS, 
			(void *)pSite, 
