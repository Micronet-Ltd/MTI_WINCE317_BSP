...
#define HANDLE_CALLBACKS_FAILURE_STATUS(hDrvMain, eStatus)      \
            if (eStatus != TI_OK) { 	\
			WLAN_OS_REPORT(("HANDLE_CALLBACKS_FAILURE_STATUS: %d\r\n", eStatus)); \
			drvMain_SmEvent (hDrvMain, SM_EVENT_FAILURE);  return; }
...
typedef enum
{
    ...
    /* 11 */ SM_EVENT_FAILURE,
    /* 12 */ SM_EVENT_SET_ACTIVE_MODE_RX_FILTER,
    /* 13 */ SM_EVENT_SET_SUSPEND_MODE_RX_FILTER,
} ESmEvent;
...
typedef struct
{
    ...
    ESuspendAction    eSuspendAction; /*  The action to perform when suspending*/
...
static TI_STATUS drvMain_SetDefaults (TI_HANDLE hDrvMain, TI_UINT8 *pBuf, TI_UINT32 uLength)
{
    ...
    /* Set Suspend action */
    pDrvMain->eSuspendAction = pInitTable->tDrvMainParams.eSuspendAction;
...
static void drvMain_InitFwCb (TI_HANDLE hDrvMain, TI_STATUS eStatus)
{
	...
	WLAN_OS_REPORT(("drvMain_InitFwCb: SM_EVENT_FW_INIT_COMPLETE\r\n"));
}

static void drvMain_ConfigFwCb (TI_HANDLE hDrvMain, TI_STATUS eStatus)
{
    HANDLE_CALLBACKS_FAILURE_STATUS(hDrvMain, eStatus);
	WLAN_OS_REPORT(("drvMain_ConfigFwCb: SM_EVENT_FW_CONFIG_COMPLETE\r\n"));
...
static void drvMain_TwdStopCb (TI_HANDLE hDrvMain, TI_STATUS eStatus)
{
    ...
    HANDLE_CALLBACKS_FAILURE_STATUS(hDrvMain, eStatus);
//	WLAN_OS_REPORT(("drvMain_TwdStopCb: %d\r\n", pDrvMain->eSmState));

	if (pDrvMain->eSmState == SM_STATE_STOPPING_ON_FAIL)
    {
        drvMain_SmEvent (hDrvMain, SM_EVENT_FAILURE);
    }
    else if (pDrvMain->eSmState == SM_STATE_STOPPING)// || pDrvMain->eSmState == SM_STATE_STOPPING_ON_FAIL) 
    {
...
static void drvMain_InitFailCb (TI_HANDLE hDrvMain, TI_STATUS eStatus)
{
	WLAN_OS_REPORT(("drvMain_InitFailCb:\r\n"));
    drvMain_SmEvent (hDrvMain, SM_EVENT_FAILURE);
    /* 
...
static void drvMain_InvokeAction (TI_HANDLE hDrvMain)
{
    ...
    if (pNewAction == NULL) 
    {
		...
		WLAN_OS_REPORT(("drvMain_InvokeAction: invalid action\r\n"));
        return;
    }
    ...
    if (pNewAction->eAction == pDrvMain->eLastAction)
    {
		...
		WLAN_OS_REPORT(("drvMain_InvokeAction: Signal the same action\r\n"));
	...
	WLAN_OS_REPORT(("drvMain_InvokeAction: %d \r\n", pNewAction->eAction));
    switch (pNewAction->eAction)
    {
    case ACTION_TYPE_START:    
		//WLAN_OS_REPORT(("drvMain_InvokeAction: ACTION_TYPE_START\r\n"));
		drvMain_SmEvent (hDrvMain, SM_EVENT_START);   
		//WLAN_OS_REPORT(("drvMain_InvokeAction: ACTION_TYPE_START: finished\r\n"));
		break;
    case ACTION_TYPE_STOP:  
        drvMain_SmEvent (hDrvMain, SM_EVENT_STOP);
        break;
    case ACTION_TYPE_SET_ACTIVE_MODE_RX_FILTER:
        drvMain_SmEvent (hDrvMain, SM_EVENT_SET_ACTIVE_MODE_RX_FILTER);
        break;
    case ACTION_TYPE_SET_SUSPEND_MODE_RX_FILTER:
        drvMain_SmEvent (hDrvMain, SM_EVENT_SET_SUSPEND_MODE_RX_FILTER);
        break;
        default:    
...
static TI_STATUS drvMain_ConfigFw (TI_HANDLE hDrvMain)
{
	...
	WLAN_OS_REPORT(("drvMain_ConfigFw: Set MAC address\r\n"));
     ...
     */
	//WLAN_OS_REPORT(("drvMain_ConfigFw: Leave Init Mode\r\n"));
    TWD_ExitFromInitMode (pDrvMain->tStadHandles.hTWD);

    /* Configure the FW from the TWD DB */
	//WLAN_OS_REPORT(("drvMain_ConfigFw: Config FW %d\r\n", GetTickCount()));
    if(TI_NOK == TWD_ConfigFw (pDrvMain->tStadHandles.hTWD))
		return TI_NOK;
	WLAN_OS_REPORT(("drvMain_ConfigFw: finished %d\r\n", GetTickCount()));
...
{
	//WLAN_OS_REPORT(("drvMain_StopActivities: suspend tx\r\n"));
    txPort_suspendTx (pDrvMain->tStadHandles.hTxPort);

    /* Disable External Inputs (IRQs and commands) */
	//WLAN_OS_REPORT(("drvMain_StopActivities: disable interrupts\r\n"));
    TWD_DisableInterrupts(pDrvMain->tStadHandles.hTWD);
	//WLAN_OS_REPORT(("drvMain_StopActivities: disable command handler\r\n"));
...
static void drvMain_ClearActionQueue (TDrvMain *pDrvMain)
{
    ...
    while (1) 
    {
		...
		//WLAN_OS_REPORT(("drvMain_ClearActionQueue: Signal\r\n"));
...
//	WLAN_OS_REPORT(("drvMain_InsertAction: %d\r\n", eAction));
    ...
    if(pDrvMain->eSmState == SM_STATE_FAILED || pDrvMain->eSmState == SM_STATE_STOPPING_ON_FAIL)
    {
		WLAN_OS_REPORT(("drvMain_InsertAction: %d failed, reboot platform\r\n", eAction));
		os_memoryFree (pDrvMain->tStadHandles.hOs, pNewAction, sizeof(TActionObject));
        return TI_NOK;
    }
    ...
    context_EnterCriticalSection (pDrvMain->tStadHandles.hContext);
    stat = que_Enqueue(pDrvMain->hActionQueue, (TI_HANDLE)pNewAction);
    context_LeaveCriticalSection (pDrvMain->tStadHandles.hContext);
//	WLAN_OS_REPORT(("drvMain_InsertAction: enque %d %d\r\n", eAction, stat));
...
//	WLAN_OS_REPORT(("drvMain_InsertAction: rescheduled %d \r\n", eAction));

//	WLAN_OS_REPORT(("drvMain_InsertAction: wait for exec %d \r\n", eAction));
    ...
    if(pDrvMain->eSmState == SM_STATE_FAILED || pDrvMain->eSmState == SM_STATE_STOPPING_ON_FAIL)
	//if(((eAction == ACTION_TYPE_START) && (pDrvMain->eSmState != SM_STATE_OPERATIONAL)) ||
	//   ((eAction == ACTION_TYPE_STOP) && (pDrvMain->eSmState != SM_STATE_STOPPED)))
    {
		WLAN_OS_REPORT(("drvMain_InsertAction: %d failed\r\n", eAction));
        return TI_NOK;
    }
//	WLAN_OS_REPORT(("drvMain_InsertAction: %d success\r\n", eAction));
    return TI_OK;
}
...
//	WLAN_OS_REPORT(("drvMain_SmEvent: %d\r\n", eEvent));
...
//		WLAN_OS_REPORT(("drvMain_SmEvent: Multiple pending events (%d), State = %d, Event = %d\r\n", pDrvMain->uPendingEventsCount, pDrvMain->eSmState, eEvent));
        return;
    }
    ...
    {
//		WLAN_OS_REPORT(("drvMain_SmEvent: pending events (%d), event = %d, State = %d, Event = %d\r\n", pDrvMain->uPendingEventsCount, pDrvMain->ePendingEvent, pDrvMain->eSmState, eEvent));
...
static void drvMain_Sm (TI_HANDLE hDrvMain, ESmEvent eEvent)
{
	...
	//WLAN_OS_REPORT(("drvMain_Sm: State = %d, Event = %d\r\n", pDrvMain->eSmState, eEvent));
    /* 
    ...
    /* Handle RX filtering events regardless of state */
     if(eEvent == SM_EVENT_SET_ACTIVE_MODE_RX_FILTER)
     {
            /* Set Active State filters */
			res = rxData_SetActiveFilter (pDrvMain->tStadHandles.hRxData);            
         	if (res != TI_OK) {
        		WLAN_OS_REPORT(("Failed to set active mode RX filters!\n"));
            }
			//WLAN_OS_REPORT(("drvMain_Sm: SM_EVENT_SET_ACTIVE_MODE_RX_FILTER:Signal\r\n"));
           os_SignalObjectSet (hOs, pDrvMain->pCurrAction->pSignalObject);
            eStatus = TI_OK;
            return;
      }
     else if(eEvent == SM_EVENT_SET_SUSPEND_MODE_RX_FILTER)
     {
            /* Set Active State filters */
			res = rxData_SetSuspendFilter (pDrvMain->tStadHandles.hRxData);            
         	if (res != TI_OK) {
        		WLAN_OS_REPORT(("Failed to set suspend mode RX filters!\n"));
            }
			//WLAN_OS_REPORT(("drvMain_Sm: SM_EVENT_SET_SUSPEND_MODE_RX_FILTER:Signal\r\n"));
           os_SignalObjectSet (hOs, pDrvMain->pCurrAction->pSignalObject);
            eStatus = TI_OK;
            return;
      }
    ...
    case SM_STATE_DOWNLOAD_FW_FILE:
        if (eEvent == SM_EVENT_FW_FILE_READY) 
        {
             ...
             */
			//WLAN_OS_REPORT(("drvMain_Sm: SM_STATE_DOWNLOAD_FW_FILE: SM_EVENT_FW_FILE_READY %d\r\n", pDrvMain->tFileInfo.bLast));
    ...
    case SM_STATE_WAIT_FW_FILE:
        ...
        {
			//WLAN_OS_REPORT(("drvMain_Sm: SM_STATE_WAIT_FW_FILE: SM_EVENT_FW_INIT_COMPLETE\r\n"));
            pDrvMain->eSmState = SM_STATE_DOWNLOAD_FW_FILE;
            eStatus = wlanDrvIf_GetFile (hOs, &pDrvMain->tFileInfo);
        }
        break;
    case SM_STATE_FW_INIT:
         ...
         */
		//WLAN_OS_REPORT(("drvMain_Sm: SM_STATE_FW_INIT\r\n"));
        if (eEvent == SM_EVENT_FW_INIT_COMPLETE) 
        {
            pDrvMain->eSmState = SM_STATE_FW_CONFIG;
/*
			// FW not configured yet
            if (!pDrvMain->bRecovery) 
            {
				...
				WLAN_OS_REPORT(("drvMain_Sm: SM_EVENT_FW_INIT_COMPLETE:Signal\r\n"));
                os_SignalObjectSet (hOs, pDrvMain->pCurrAction->pSignalObject);
            }
*/
          ...
          #endif
			//WLAN_OS_REPORT(("drvMain_Sm: SM_STATE_FW_INIT: SM_EVENT_FW_INIT_COMPLETE\r\n"));
            eStatus = drvMain_ConfigFw (hDrvMain);
/*			if(TI_NOK == eStatus)
			{
				txnQ_DisconnectBus (pDrvMain->tStadHandles.hTxnQ);
				hPlatform_DevicePowerOffSetLongerDelay();
                hPlatform_DevicePowerOn (1);
                pDrvMain->eSmState = SM_STATE_WAIT_NVS_FILE;
                pDrvMain->tFileInfo.eFileType = FILE_TYPE_NVS;
                eStatus = wlanDrvIf_GetFile (hOs, &pDrvMain->tFileInfo);
			}
*/
            if(!pDrvMain->bRecovery && TI_NOK != eStatus) 
            {
                // update the state before unblocking the application so command will not be rejected
                wlanDrvIf_UpdateDriverState (hOs, DRV_STATE_RUNNING);
				//WLAN_OS_REPORT(("drvMain_Sm: SM_EVENT_FW_INIT_COMPLETE:Signal\r\n"));
			...
			//WLAN_OS_REPORT(("drvMain_Sm: SM_STATE_FW_INIT: SM_EVENT_FW_INIT_COMPLETE, %d, %d, %d\r\n", pDrvMain->eSmState, eEvent, eStatus));
        }
        break;
    case SM_STATE_FW_CONFIG:
		...
		//WLAN_OS_REPORT(("drvMain_Sm: SM_STATE_FW_CONFIG\r\n"));
        if (eEvent == SM_EVENT_FW_CONFIG_COMPLETE) 
        {
			//WLAN_OS_REPORT(("drvMain_Sm: SM_STATE_FW_CONFIG: SM_EVENT_FW_CONFIG_COMPLETE\r\n"));
            pDrvMain->eSmState = SM_STATE_OPERATIONAL;
            if (pDrvMain->bRecovery) 
			{
				//WLAN_OS_REPORT(("drvMain_Sm: SM_STATE_FW_CONFIG: SM_EVENT_FW_CONFIG_COMPLETE: Recovery\r\n"));
				pDrvMain->uNumOfRecoveryAttempts = 0;
            ...
            {
				//WLAN_OS_REPORT(("drvMain_Sm: SM_STATE_FW_CONFIG: SM_EVENT_FW_CONFIG_COMPLETE: Start\r\n"));
                sme_Start (pDrvMain->tStadHandles.hSme); 
            }
			//WLAN_OS_REPORT(("drvMain_Sm: SM_STATE_FW_CONFIG: Update state\r\n"));
            tmr_UpdateDriverState (pDrvMain->tStadHandles.hTimer, TI_TRUE);
			//WLAN_OS_REPORT(("drvMain_Sm: SM_STATE_FW_CONFIG: Enable Activities\r\n"));
            drvMain_EnableActivities (pDrvMain);
			//WLAN_OS_REPORT(("drvMain_Sm: SM_STATE_FW_CONFIG: Enable Client\r\n"));
    ...
    case SM_STATE_STOPPING:
		...
		WLAN_OS_REPORT(("drvMain_Sm: SM_STATE_STOPPING\r\n"));
        ...
        if (eEvent == SM_EVENT_STOP_COMPLETE) 
        {
            ...
            else 
            {
				...
				//WLAN_OS_REPORT(("drvMain_Sm: SM_STATE_STOPPING: clearQues\r\n"));
                drvMain_ClearQueuedEvents (pDrvMain);
                scr_notifyFWReset(pDrvMain->tStadHandles.hSCR);
                wlanDrvIf_UpdateDriverState (hOs, DRV_STATE_STOPPED);
                context_EnableClient (pDrvMain->tStadHandles.hContext, pDrvMain->uContextId);
                eStatus = TI_OK;
				WLAN_OS_REPORT(("drvMain_Sm: SM_EVENT_STOP_COMPLETE:Signal\r\n"));
                os_SignalObjectSet (hOs, pDrvMain->pCurrAction->pSignalObject);
            }
        }
    ...
    case SM_STATE_STOPPED:
         ...
         */
		WLAN_OS_REPORT(("drvMain_Sm: SM_STATE_STOPPED \r\n"));
        context_DisableClient (pDrvMain->tStadHandles.hContext, pDrvMain->uContextId);
        if (eEvent == SM_EVENT_START) 
        {
            hPlatform_DevicePowerOn (0);
    ...
    case SM_STATE_STOPPING_ON_FAIL:
		...
		WLAN_OS_REPORT(("drvMain_Sm: SM_STATE_STOPPING_ON_FAIL\r\n"));
        pDrvMain->eSmState = SM_STATE_FAILED;
		//WLAN_OS_REPORT(("drvMain_Sm: SM_STATE_STOPPING_ON_FAIL: disconnect bus\r\n"));
        txnQ_DisconnectBus (pDrvMain->tStadHandles.hTxnQ);
		WLAN_OS_REPORT(("drvMain_Sm: SM_STATE_STOPPING_ON_FAIL: power off\r\n"));
        hPlatform_DevicePowerOff ();

		//TEMP
		//pDrvMain->bRecovery = 0;

        if (!pDrvMain->bRecovery) 
		{
			WLAN_OS_REPORT(("drvMain_Sm: SM_STATE_STOPPING_ON_FAIL: not recovery\r\n"));
//			eStatus = drvMain_StopActivities (pDrvMain);
			TWD_FinalizeOnFailure(pDrvMain->tStadHandles.hTWD); 
            os_SignalObjectSet (hOs, pDrvMain->pCurrAction->pSignalObject);
			//eStatus = drvMain_StopActivities (pDrvMain);
		}
		else if (pDrvMain->uNumOfRecoveryAttempts < MAX_NUM_OF_RECOVERY_TRIGGERS) 
		{
			WLAN_OS_REPORT(("drvMain_Sm: SM_STATE_STOPPING_ON_FAIL:recovery\r\n"));
			pDrvMain->eSmState = SM_STATE_STOPPING;
			eStatus = drvMain_StopActivities (pDrvMain);
		}
        break;
    case SM_STATE_FAILED:
		WLAN_OS_REPORT(("drvMain_Sm: SM_STATE_FAILED\r\n"));
...
//            eStatus = wlanDrvIf_GetFile (hOs, &pDrvMain->tFileInfo);
 ...
 default:
		...
		WLAN_OS_REPORT(("drvMain_Sm: unknown state\r\n"));
        break;  
    }
        ...
        (pDrvMain->eSmState != SM_STATE_STOPPING_ON_FAIL))
    {
		...
		WLAN_OS_REPORT (("drvMain_Sm: faled: eEvent=%u at state=%u, status=%d\r\n", eEvent, pDrvMain->eSmState, eStatus));
        pDrvMain->eSmState = SM_STATE_STOPPING_ON_FAIL;
		
		//WLAN_OS_REPORT(("drvMain_Sm: Updete state\r\n"));
         ...
         */
		//WLAN_OS_REPORT(("drvMain_Sm: Stop Activities\r\n"));
        eStatus = drvMain_StopActivities (pDrvMain);
		//WLAN_OS_REPORT(("drvMain_Sm: stopped %d\r\n", eStatus));
    }
}


ESuspendAction drvMain_GetSuspendAction(TI_HANDLE hDrvMain)
{
    TDrvMain    *pDrvMain   = (TDrvMain *)hDrvMain;

    return(pDrvMain->eSuspendAction); 
}
