...
static TI_STATUS rxData_addRxDataFilter (TI_HANDLE hRxData, TRxDataFilterRequest* request, TI_UINT8 FilterType);
static TI_STATUS rxData_removeRxDataFilter(TI_HANDLE hRxData, TRxDataFilterRequest* request, TI_UINT8 FilterType);
...
TI_STATUS rxData_SetDefaults (TI_HANDLE hRxData, rxDataInitParams_t * rxDataInitParams)
{
    ...
    for (i = 0; i < MAX_DATA_FILTERS; ++i)
    {
        ...
        {
			if (i < MAX_ACTIVE_FILTERS)
			{
				TRACE1(pRxData->hReport, REPORT_SEVERITY_INFORMATION, "Active Filter... i=%d \n",i);
				if (rxData_addRxDataFilter(hRxData, &rxDataInitParams->rxDataFilterRequests[i], FILTER_TYPE_ACTIVE) != TI_OK)
            {
			...
			{
				TRACE1(pRxData->hReport, REPORT_SEVERITY_INFORMATION, "Suspend Filter... i=%d \n",i);
				if (rxData_addRxDataFilter(hRxData, &rxDataInitParams->rxDataFilterRequests[i], FILTER_TYPE_SUSPEND) != TI_OK)
				{
					TRACE1(pRxData->hReport, REPORT_SEVERITY_ERROR, ": Invalid Rx Data Filter configured at init stage (at index %d)!\n", i);
				}
			}
...
TI_STATUS rxData_getParam(TI_HANDLE hRxData, paramInfo_t *pParamInfo)
{
    ...
    switch (pParamInfo->paramType)
    {
        ...
        case RX_DATA_PRINT_RX_DATA_FILTERS:
			rxData_printRxDataFilter(hRxData);
            break;
...
TI_STATUS rxData_setParam(TI_HANDLE hRxData, paramInfo_t *pParamInfo)
{
    ...
    switch (pParamInfo->paramType)
    {
        ...
        case RX_DATA_ADD_RX_DATA_FILTER:
        {
            ...
            return rxData_addRxDataFilter(hRxData, pRequest, FILTER_TYPE_ACTIVE);
        }
        ...
        case RX_DATA_REMOVE_RX_DATA_FILTER:
        {
            ...
            return rxData_removeRxDataFilter(hRxData, pRequest, FILTER_TYPE_ACTIVE);
        }

        case RX_DATA_ADD_RX_SUSPEND_FILTER:
        {
            TRxDataFilterRequest* pRequest = &pParamInfo->content.rxDataFilterRequest;            

            return rxData_addRxDataFilter(hRxData, pRequest, FILTER_TYPE_SUSPEND);
        }

        case RX_DATA_REMOVE_RX_SUSPEND_FILTER:
        {
            TRxDataFilterRequest* pRequest = &pParamInfo->content.rxDataFilterRequest;
            
            return rxData_removeRxDataFilter(hRxData, pRequest, FILTER_TYPE_SUSPEND);
        }
...
static TI_STATUS rxData_addRxDataFilter (TI_HANDLE hRxData, TRxDataFilterRequest* request, TI_UINT8 FilterType)
{
	...
	if (FilterType == FILTER_TYPE_ACTIVE)
	{
    /* find place for insertion */
		for (index = 0; index < MAX_ACTIVE_FILTERS; ++index)
    {
...
static TI_STATUS rxData_removeRxDataFilter (TI_HANDLE hRxData, TRxDataFilterRequest* request, TI_UINT8 FilterType)
{
    ...
    if ( (index < 0) || ( (index == MAX_ACTIVE_FILTERS) && (FilterType == FILTER_TYPE_ACTIVE)) )
    {
	...
	if (FilterType == FILTER_TYPE_SUSPEND)
		index = MAX_ACTIVE_FILTERS;

    TRACE1(pRxData->hReport, REPORT_SEVERITY_INFORMATION, ": Removing filter at index %d.", index);

	/* Do not try to remove a suspend filter from the FW as it is not active while in active mode */
	if (FilterType == FILTER_TYPE_ACTIVE)
	{
    return TWD_CfgRxDataFilter (pRxData->hTWD, 
                                index, 
...
void rxData_printRxDataFilter (TI_HANDLE hRxData)
{
    TI_UINT32 index;
    rxData_t *pRxData = (rxData_t *)hRxData;
	char pattern[RX_DATA_FILTER_MAX_PATTERN_SIZE];
	char mask[RX_DATA_FILTER_MAX_MASK_SIZE];
    ...
    for (index=0; index<MAX_DATA_FILTERS; index++)
     {
        if (pRxData->isFilterSet[index])
        {
			...
			report_Dump (pRxData->filterRequests[index].pattern, pattern, pRxData->filterRequests[index].patternLength);
			report_Dump (pRxData->filterRequests[index].mask, mask, pRxData->filterRequests[index].maskLength);
			if (index < MAX_ACTIVE_FILTERS)
            	WLAN_OS_REPORT (("index=%d, Active filter - offset: %d pattern: %s byte mask: %s\n", index,pRxData->filterRequests[index].offset, pattern,mask));
            else            
            	WLAN_OS_REPORT (("index=%d, Suspend filter- offset: %d pattern: %s byte mask: %s\n", index,pRxData->filterRequests[index].offset, pattern,mask));            
        }
        else
        {
            WLAN_OS_REPORT (("index=%d, No Filter defined\n", index));
        }
     }
}
...
/****************************************************************************
*						rxData_DisableFilter	   							*
*****************************************************************************
* DESCRIPTION:	this function disables an already configured filter.
*				used for Susupend/Acctive filters manipulations
* 
* INPUTS:		hRxData - the object
*				iFilterIndex - the filter index in the firmware
*		
* OUTPUT:		None
* 
* RETURNS:		None
***************************************************************************/
TI_STATUS rxData_DisableFilter (TI_HANDLE hRxData, int iFilterIndex)
{
    rxData_t *pRxData = (rxData_t *)hRxData;

    if (pRxData->isFilterSet[iFilterIndex] == TI_FALSE)
    {
		TRACE0(pRxData->hReport, REPORT_SEVERITY_INFORMATION, "This filter index is not initialized\n");
        return TI_NOK;
	}

	TRACE1(pRxData->hReport, REPORT_SEVERITY_INFORMATION, "filter index = %d is initialized, disable it...\n",iFilterIndex);

    /* Send filter configuration to the firmware */
    return TWD_CfgRxDataFilter (pRxData->hTWD, 
                                iFilterIndex, 
                                REMOVE_FILTER, 
                                FILTER_SIGNAL, 
...
*						rxData_EnableFilter	   								*
*****************************************************************************
* DESCRIPTION:	this function enables an already configured filter.
*				Used for Susupend/Acctive filters manipulations
* 
* INPUTS:		hRxData - the object
*				iFilterIndex - the filter index in the firmware
...
* RETURNS:		None
***************************************************************************/
TI_STATUS rxData_EnableFilter (TI_HANDLE hRxData, int iFilterIndex)
{
    rxData_t *pRxData = (rxData_t *)hRxData;
    ...
    /* parse the filter request into discrete field patterns */
    if (parseRxDataFilterRequest(hRxData, &pRxData->filterRequests[iFilterIndex], &numFieldPatterns, &lenFieldPatterns, fieldPatterns) != TI_OK)
        return TI_NOK;
	...
	TRACE1(pRxData->hReport, REPORT_SEVERITY_INFORMATION, "filter index = %d is initialized, Enable it...\n",iFilterIndex);
...
/****************************************************************************
*						rxData_SetSuspendFilter   							*
*****************************************************************************
* DESCRIPTION:	this function is called when entering supend state
* 				It configures the RxData filters to the supend state filter
* 				that when being received from the air should trigger a wake up
* 
* INPUTS:		hRxData - the object
*		
* OUTPUT:		None
...
TI_STATUS rxData_SetSuspendFilter (TI_HANDLE hRxData)
{
	...
	for (i = 0; i < MAX_ACTIVE_FILTERS ; i++)
	{
		/* Disable active mode filters */
		status = rxData_DisableFilter (hRxData, i);
	}
	/* Enable standby mode filter */
	status = rxData_EnableFilter (hRxData, MAX_ACTIVE_FILTERS);
	
	/* Enable the data filter only in case a suspend filter is initialized */
	TRACE1(pRxData->hReport, REPORT_SEVERITY_INFORMATION, "Suspend filter available status = %x\n", TI_TRUE - status);

	rxData_enableDisableRxDataFilters(hRxData, TI_TRUE - status);
		
    return TI_OK;
}

/****************************************************************************
*						rxData_SetActiveFilter   							*
*****************************************************************************
* DESCRIPTION:	this function is called when exiting from supend state
* 				It configures the RxData filters to the active state values
* 
* INPUTS:		hRxData - the object
*		
* OUTPUT:		None
...
TI_STATUS rxData_SetActiveFilter (TI_HANDLE hRxData)
{
	...
	for (i = 0; i < MAX_ACTIVE_FILTERS ; i++)
	{
		/* Enable active mode filters. The status is ANDed in order to find out if any filter is used */
		status &= rxData_EnableFilter (hRxData, i);
	}
	
	/* Enable the data filters only in case an active moder filter is initialized */
	TRACE1(pRxData->hReport, REPORT_SEVERITY_INFORMATION, "Active filter available status = %x\n", TI_TRUE - status);

	rxData_enableDisableRxDataFilters(hRxData, TI_TRUE - status);

	/* Disable standby mode filter */
	status = rxData_DisableFilter (hRxData, MAX_ACTIVE_FILTERS);
