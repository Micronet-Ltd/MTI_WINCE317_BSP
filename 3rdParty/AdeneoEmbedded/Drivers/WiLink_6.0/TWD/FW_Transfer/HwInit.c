...
#if !defined(BSP_EVM_DM_AM_37X)
	if(os_getMAC(mac, sizeof(mac)))
	{
		pHwInit->aDefaultNVS[11] = (TI_UINT8)(mac[0]&0xFF);
		pHwInit->aDefaultNVS[10] = (TI_UINT8)(mac[0]>>8);
		pHwInit->aDefaultNVS[6]  = (TI_UINT8)(mac[1]&0xFF);
		pHwInit->aDefaultNVS[5]  = (TI_UINT8)(mac[1]>>8);
		pHwInit->aDefaultNVS[4]  = (TI_UINT8)(mac[2]&0xFF);
		pHwInit->aDefaultNVS[3]  = (TI_UINT8)(mac[2]>>8);
	}
#endif
...
//		WLAN_OS_REPORT (("hwInit_LoadFw: \r\n"));
        status = hwInit_LoadFwImageSm (pHwInit);
//		WLAN_OS_REPORT (("hwInit_LoadFw: %d\r\n", status));
...
static TI_STATUS hwInit_LoadFwImageSm (TI_HANDLE hHwInit)
{
    ...
    while (TI_TRUE)
    {
		//WLAN_OS_REPORT (("hwInit_LoadFwImageSm: %d\r\n", pHwInit->uLoadStage));
