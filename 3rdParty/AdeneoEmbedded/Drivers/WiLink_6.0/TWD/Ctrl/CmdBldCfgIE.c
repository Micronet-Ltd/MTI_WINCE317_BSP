...
TI_STATUS cmdBld_CfgIeConnMonitParams (TI_HANDLE hCmdBld, TRroamingTriggerParams *pRoamingTriggerCmd, void *fCb, TI_HANDLE hCb)                                           
{
    ...
    pCfg->BSSLossTimeout     = ENDIAN_HANDLE_LONG(pRoamingTriggerCmd->BssLossTimeout);		// NO_BEACON_DEFAULT_TIMEOUT
    pCfg->TSFMissedThreshold = ENDIAN_HANDLE_LONG(pRoamingTriggerCmd->TsfMissThreshold);	// OUT_OF_SYNC_DEFAULT_THRESHOLD
	WLAN_OS_REPORT(("%s BSS lost timeout (%d) TSF miss (%d)\r\n",__FUNCTION__, pCfg->BSSLossTimeout, pCfg->TSFMissedThreshold));
