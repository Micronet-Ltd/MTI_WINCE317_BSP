...
VOID CuCmd_AddRxSuspendFilter(THandle hCuCmd, ConParm_t parm[], U16 nParms)
{
    CuCmd_t* pCuCmd = (CuCmd_t*)hCuCmd;
    S32 res;
    TRxDataFilterRequest request;
    ...
    CuCmd_ParseMaskString(mask, request.mask, &request.maskLength);
    CuCmd_ParsePatternString(pattern, request.pattern, &request.patternLength);
    res = CuCommon_SetBuffer(pCuCmd->hCuCommon, TIWLN_ADD_RX_SUSPEND_FILTER,
        &request, sizeof(TRxDataFilterRequest));
    if(res == OK)
        os_error_printf(CU_MSG_INFO2, (PS8)"Suspend Filter added.\r\n");
    else
        os_error_printf(CU_MSG_ERROR, (PS8)"ERROR - CuCmd_AddRxSuspendFilter - Couldn't add Rx suspend filter...\r\n");    
}
VOID CuCmd_RemoveRxSuspendFilter(THandle hCuCmd, ConParm_t parm[], U16 nParms)
{
    CuCmd_t* pCuCmd = (CuCmd_t*)hCuCmd;
    S32 res;
    TRxDataFilterRequest request;
    ...
    res = CuCommon_SetBuffer(pCuCmd->hCuCommon, TIWLN_REMOVE_RX_SUSPEND_FILTER,
        &request, sizeof(TRxDataFilterRequest));

    if(res == OK)
        os_error_printf(CU_MSG_INFO2, (PS8)"Suspend Filter Removed.\r\n");
    else
        os_error_printf(CU_MSG_ERROR, (PS8)"ERROR - CuCmd_RemoveRxSuspendFilter - Couldn't remove Rx suspend filter...\r\n");    
}

VOID CuCmd_PrintRxDataFilters(THandle hCuCmd, ConParm_t parm[], U16 nParms)
{
    CuCmd_t* pCuCmd = (CuCmd_t*)hCuCmd;
    U32 uDummyBuf;

    if(OK != CuCommon_GetBuffer(pCuCmd->hCuCommon, TIWLN_PRINT_RX_DATA_FILTERS, &uDummyBuf, sizeof(uDummyBuf))) 
    {
        os_error_printf(CU_MSG_ERROR, (PS8)"ERROR - Failed to print Rx Data Filters!\r\n");
    }
}
...
VOID CuCmd_BIP_SetNVSVersion(THandle hCuCmd, ConParm_t parm[], U16 nParms)
{
    ...
    if( nParms )
    {
        ...
        data.testCmd_u.changeNVSVersion.nvsVersionChange =  (TTestCmdChnageNVSVersion_e)parm[0].value;
