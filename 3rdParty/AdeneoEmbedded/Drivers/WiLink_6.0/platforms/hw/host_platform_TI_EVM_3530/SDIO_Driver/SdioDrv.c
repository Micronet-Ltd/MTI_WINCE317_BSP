...
int SDI_Init (LPCTSTR szContext)
{
	...
	// Vladimir already initialized by xldr
#if 0
...
//    OUTREG16(&pConfig->CONTROL_PADCONF_MMC2_DAT7, (INPUT_ENABLE | PULL_UP | MUX_MODE_0));           /*MMC2_DAT7 is unused (clkin disable)*/
	...
	SETREG32(&pConfigGeneral->CONTROL_DEVCONF1, (MMCSDIO2ADPCLKISEL));         
...
static WINAPI sdiodrv_IST(LPVOID ptr)
{
    ...
    while (TRUE)
    {
        ...
        if (g_drv.BusTxnCB != NULL)
        {
          ...
          {
            RETAILMSG(1, (L"sdiodrv_IST() sdioDrv_InterruptDone STAT=0x%x\n", iStatus));
          }
...
static void sdioDrv_SetClock (unsigned int clock)
{
	...
	if(!(status & ICS)) 
    {
	    RETAILMSG(1, (L"sdioDrv_SetClock() clock not stable!! status=0x%x\n", status));
...
static int sdioDrv_ResetMmc (void)
{
	...
	if (status & SRA)
	{
	    RETAILMSG(1, (L"sdioDrv_ResetMmc() MMC reset FAILED!! status=0x%x\n", status));
                        ...
                        unsigned int uLen)
{
	...
	if (!(uStatus & CC)) 
    {
	    RETAILMSG(1, (L"sdioDrv_ExecuteCmd() SDIO Command error status = 0x%x\n", uStatus));
		return ERROR_1;
	}
	if ((uLen > 0) && (uLen <= 4))
	{
                                    ...
                                    unsigned int bBlkMode)
{
	...
	if (!(status & CC)) 
    {
	    RETAILMSG(1, (L"sdioDrv_SendDataXferCmd() SDIO Command error! STAT = 0x%x\n", status));
		return SUCCESS;
	}
	if(uStatusMask)
	{
                                 ...
                                 unsigned int  uStatusMask)
{
	...
	if (uStatusMask == BRE)  
    {
		...
		if (!(iStatus & TC)) 
		{
			RETAILMSG(1, (L"sdioDrv_DataXferSync() transfer error! STAT = 0x%x\n", iStatus));
            return -2;
		}
    }
                           ...
                           unsigned int  bMore)
{
    ...
    for (i = 0; i < uLen; i++) 
    {
        ...
        if (!(iStatus & CC)) 
        {
			RETAILMSG(1, (L"sdioDrv_ReadSyncBytes() SDIO Command error status = 0x%x\n", iStatus));
            return ERROR_1;
        }
        else
        {
                            ...
                            unsigned int  bMore)
{
    ...
    for (i = 0; i < uLen; i++) 
    {
        ...
        if (!(iStatus & CC)) 
        {
	        RETAILMSG(1, (L"sdioDrv_WriteSyncBytes() SDIO Command error status = 0x%x\n", iStatus));
            return ERROR_1;
        }
