...
#include <ceddk.h>
#include <omap3530_base_regs.h>  // base register PA 
#include <omap3530.h>
...
#define UART2_CTS		144
#define UART2_RTS		145
...
static BOOL SetComPowerState(HANDLE hCom, CEDEVICE_POWER_STATE ps)
{
    ...
    switch (ps)
    {
        ...
        case D2:
			...
			rc = (0 != SetPowerRequirement(g_hci.comPortName, ps, POWER_NAME, NULL, 0));
            RETAILMSG(0, (TEXT("SetComPowerState: DeviceIoControl UART should be in D0 state!!!\r\n")));

            /* Wait for UART state to settle down to D0 */
            //do
            //{
            //    DeviceIoControl(hCom, IOCTL_POWER_GET, NULL, 0, &utps, sizeof(utps), NULL, NULL);
            //    DEBUGMSG(1, (TEXT("SetComPowerState: DeviceIoControl UART state is in D%d state!!!\r\n"),utps));
            //}while(!(utps == D0));
        ...
        case D4:
            ...
            /* Wait for UART state to settle down to D3 */
            //do
            //{
            //    DeviceIoControl(hCom, IOCTL_POWER_GET, NULL, 0, &utps, sizeof(utps), NULL, NULL);
            //    DEBUGMSG(1, (TEXT("SetComPowerState: DeviceIoControl UART state is in D%d state!!!\r\n"),utps));
...
//------------------------------------------------------------------------------
static BOOL
SetComGPIOMode(BOOL Mode)
{
    BOOL rc = TRUE;
      
	if(Mode) {     		
		//Set CTS to GPIO_144 with Mode 4, Pulldown, enabled
		MASKREG32(&(g_hci.pPadConfig->CONTROL_PADCONF_UART2_CTS), BIT0|BIT1|BIT2|BIT3|BIT4, BIT2|BIT3);
	} else {
		//Give GPIO back to CTS with Mode 0, pulldown, enabled
		MASKREG32(&(g_hci.pPadConfig->CONTROL_PADCONF_UART2_CTS), BIT0|BIT1|BIT2|BIT3|BIT4, BIT3);
	}
	return rc;
}

static void
Uart2GpioMode(HCI_CONTEXT *pHCI)
{        
	RETAILMSG(0, (L"HCI: ProcessSleepInd(): SetComPowerState D3 \r\n"));
	RETAILMSG(0, (L"HCI: Uart2GpioMode(): %d, \r\n",BT_CTS_WAKEUP_SysIntr));
	// Enable GPIO Interrupt
	if (BT_CTS_WAKEUP_SysIntr != 0)
		InterruptMask(BT_CTS_WAKEUP_SysIntr, FALSE);
     		 	
    //Set RTS to GPIO_145 with Mode 4, pulldown, enabled
	MASKREG32(&(g_hci.pPadConfig->CONTROL_PADCONF_UART2_CTS), BIT16|BIT17|BIT18|BIT19|BIT20, BIT18|BIT19);
	if(g_hci.hGpio) { 
		GPIOSetMode(g_hci.hGpio, UART2_RTS, GPIO_DIR_OUTPUT );
		GPIOSetBit(g_hci.hGpio, UART2_RTS);
	}
	...
	// Mux Uart CTS-> GPIO_144
	SetComGPIOMode(TRUE);
	RETAILMSG(0, (L"HCI: ProcessSleepInd(): SetComGPIOMode GPIO \r\n")); 
	// Set serial port to D3...
	SetComPowerState(pHCI->hCom, D3);  
}

static void
Gpio2UartMode(HCI_CONTEXT *pHCI)
{		
	...
	MASKREG32(&(g_hci.pPadConfig->CONTROL_PADCONF_UART2_CTS), BIT16|BIT17|BIT18|BIT19|BIT20, BIT19);
	// Mux GPIO_144 -> UART CTS
	SetComGPIOMode(FALSE);
}
...
ProcessSleepInd(HCI_CONTEXT *pHCI)
{
        ...
        (pHCI->btPowerMode == BT_AWAKE_TO_SLEEP))
    {
        ...
        SetComPowerState(g_hci.hCom, D3);
        RETAILMSG(0, (L"HCI: ProcessSleepInd(): SetComPowerState D3 \r\n"));

        BT_WakeupEnabled = TRUE;    //enable ISR
        RETAILMSG(0, (L"HCI: ProcessSleepInd(): 	BT_WakeupEnabled = TRUE \r\n"));
    ...
    )
{
    ...
    if (hFile == INVALID_HANDLE_VALUE)
    {
        ...
        RETAILMSG(0,(TEXT("Failed open script file name '%s'\r\n"),buffer));
        goto cleanUp;
    }
        ...
        (count < sizeof(header)))
    {
        ...
        RETAILMSG(0,(TEXT("Failed read header from script file\r\n")));
        goto cleanUp;
    }
    ...
    while (TRUE)
    {
        ...
        if (!ReadFile(hFile, &action, sizeof(action), &count, NULL))
        {
            ...
            RETAILMSG(0,(TEXT("Failed read action from script file \r\n")));
            goto cleanUp;
        }
        ...
        { 
            RETAILMSG(0,(TEXT("HCI: RunScript(): reach the end of script\r\n"))); 
			break;
		}
        ...
        // Read buffer
            ...
            RETAILMSG(0,(TEXT("Failed read action parameters  \r\n")));
            goto cleanUp;
            }
        ...
        switch (action.code)
        {
            case ACTION_SEND_COMMAND:
                ...
                if (!WritePacket(pHCI, buffer, count, pHCI->driverTimeout))
                {
                        ...
                        RETAILMSG(0,(TEXT("Failed write HCI packet to Bluetooth device  \r\n")));
                        goto cleanUp;
                }
                ...
                if (count == 0)
                {
                    ...
                    RETAILMSG(0,(TEXT("Failed read HCI packet from Bluetooth device  \r\n")));
                        goto cleanUp;
                }
            ...
            case ACTION_SERIAL_PORT:
                ...
                if (count < sizeof(ACTION_SERIAL_PORT_PARAMS))
                {
                    ...
                    RETAILMSG(0,(TEXT("Parameters for ACTION_SERIAL_PORT too small  \r\n")));
            ...
            RETAILMSG(0,(TEXT("Failed set serial port parameters \r\n")));
...
cleanUp:
    ...
    RETAILMSG(0,(TEXT("HCI: -RunScript(): %d\r\n"), rc));
    return rc;
}
...
StartHardware(void)
{
	IFDBG(DebugOut (DEBUG_HCI_INIT, L"+StartHardware\n"));
	if(g_hci.hGpio != NULL) {
...
StopHardware(void)
{
	IFDBG(DebugOut (DEBUG_HCI_INIT, L"+StopHardware\n"));
	if(g_hci.hGpio != NULL) {
		GPIOSetMode(g_hci.hGpio, BT_GPIO_SHUTDOWN, GPIO_DIR_OUTPUT );
    ...
    )
{
    ...
    RETAILMSG(0, (L"Opening port %s (rate %d) for I/O with unit\n",
        g_hci.comPortName, g_hci.baud));
    ...
    if(g_hci.pPadConfig == NULL || g_hci.pPadConfig2 == NULL) {
    	RETAILMSG(0, (L" OMAP2430 BRF6300! OMAP2430_PADCONFS1_REGS:Failed to map PADCONFS1 registers (pa = 0x%08x)\r\n", pa.LowPart));
        goto cleanUp; 
    }
    ...
    if(g_hci.hGpio == NULL)
    {
        RETAILMSG(0, (L"ERROR: HCI_OpenConnection: "L"Failed open GPIO interface (GetLastError() = 0x%08x)\r\n",GetLastError()));
        goto cleanUp;
    }
    ...
    )
{
   ...
   SetEvent(g_hci.hRequestDoneEvent);
    ...
    RETAILMSG(0, (L" +HCI_CloseConnection: Closing Wakeup mode +\r\n"));
    //close wakeup mode
   BT_ReleaseInterruptThread();
    ...
    RETAILMSG(0, (L" +HCI_CloseConnection: Closed Wakeup mode & closed com port +\r\n"));
    ...
    RETAILMSG(0, (L" +HCI_CloseConnection: Unmapping the memory +\r\n"));
