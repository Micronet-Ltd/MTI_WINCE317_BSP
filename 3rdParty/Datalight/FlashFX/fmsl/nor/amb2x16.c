...
/*---------------------------------------------------------------------------
    ...
    "Flash Interface Module For Interleaved MirrorBit Devices In 2x16 Mode"
    This FIM is for parts with uniform sectors.
    Provides the Flash Interface Module (FIM) for the following flash
    parts in an interleaved, 16 bit wide configuration:
...
/*---------------------------------------------------------------------------
                                Revision History
    $Log: amb2x16.c $
    Revision 1.13  2009/04/03 05:40:38Z  keithg
    Fixed bug 2521: removed obsolete READ_BACK_VERIFY.
    Revision 1.12  2007/11/03 23:49:37Z  Garyp
    Updated to use the standard module header.
    Revision 1.11  2006/10/02 17:57:52Z  Garyp
    Updated to use DclMemAllocZero().
    Revision 1.10  2006/09/12 23:19:26Z  rickc
    In WriteBufferedBytes, monitor last address written to;
    previously monitoring first address, which is not to spec.
    Revision 1.9  2006/08/31 21:39:09Z  rickc
    Fixed small bugs found in testing
    Revision 1.8  2006/08/31 17:34:02Z  pauli
    ...
    Revision 1.7  2006/08/31 17:18:07Z  pauli
    Completed updated to use the new FfxFimNorWindowCreate,
    FfxFimNorWindowMap functions.  Eliminated the use of the
    AMD_SEND_CMD macro.
    Revision 1.6  2006/08/31 05:13:01Z  pauli
    Began updating to use the new FfxFimNorWindowCreate,
    FfxFimNorWindowMap functions.
    Revision 1.5  2006/08/11 03:57:19Z  rickc
    Disabled erase suspend and resume by default due to toggle
    issue with MirrorBit parts.  Added ability to vary erase block size.
    Revision 1.4  2006/05/15 17:48:27Z  Garyp
    Eliminated the use of an obsolete field.
    Revision 1.3  2006/02/10 21:03:40Z  Garyp
    Renamed the FIMDEVICE structure instantiation for clarity.
    Revision 1.2  2006/02/08 18:21:17Z  Garyp
    Modified to use new FfxHookWindowMap/Size() functions.  Updated
    debugging code.
    Revision 1.1  2005/10/25 23:17:52Z  Pauli
    Initial revision
    Revision 1.2  2005/10/26 00:17:52Z  Garyp
    Updated to use some renamed fields.
    Revision 1.1  2005/10/14 03:07:54Z  Garyp
    Initial revision
    Revision 1.2  2005/09/20 19:01:10Z  pauli
    Replaced _syssleep with DclOsSleep.
    Revision 1.1  2005/08/03 19:30:28Z  pauli
    Initial revision
    Revision 1.9  2005/05/11 20:48:01Z  garyp
    Modified to use generalized AMD_SEND_CMD macro and standard
    data types.
    Revision 1.8  2005/05/10 16:05:25Z  billr
    Fix compiler warning. Remove inappropriate DclProductionError() (IR 4331).
    Revision 1.7  2005/05/08 20:22:01Z  garyp
    Updated to use new AMD flash command macros.  Formatted for readability.
    No functional changes.
    Revision 1.6  2004/12/30 23:00:17Z  GaryP
    Updated to use indents of 4, hard tabs of 8 (if used), and a standard
    comment style.
    Revision 1.5  2004/08/05 16:32:12Z  billr
    Move optional functions to the end of FIMDEVICE.
    Revision 1.4  2004/08/04 21:54:26Z  billr
    Eliminate ulInterleaved in ExtndMediaInfo, no longer used or needed.
    Revision 1.3  2004/08/04 21:23:37Z  billr
    Eliminate ulFimFlags in ExtndMediaInfo, no longer used or needed.
    Revision 1.2  2004/07/29 02:52:46Z  jaredw
    removed commented out debug code
    Revision 1.1  2004/07/22 21:40:22Z  jaredw
    Initial revision
---------------------------------------------------------------------------*/
...
#define AMD_BUFF_ABORT_HIGH     MAKEFLASHCMD(ASTAT_BUFFERABORT, 0x0) /* DQ1 Write-to-Buffer Abort (high) */
#define AMD_BUFF_ABORT_LOW      MAKEFLASHCMD(0x0, ASTAT_BUFFERABORT) /* DQ1 Write-to-Buffer Abort (low) */
...
*/
#define DEVICE_CODE1_OFFSET_2X16  4     /* 000100b */
#define DEVICE_CODE2_OFFSET_2X16 56     /* 111000b */
#define DEVICE_CODE3_OFFSET_2X16 60     /* 111100b */

#define END_OF_LIST_UL      0xFFFFFFFFUL
#define END_OF_LIST         0xFFFF

#define WRITE_BUFFER_PAGE_SIZE 64       /* in bytes, for the interleave */
...
#if BUFFERED_WRITES
static D_BOOL WriteBufferedBytes(PEXTMEDIAINFO pEMI, D_UINT32 ulStart, D_UINT32 * pulData, D_UINT32 ulLength);
#else
static D_BOOL WriteBytes(PEXTMEDIAINFO pEMI, D_UINT32 ulStart, D_UINT32 * pulData, D_UINT32 ulLength);
#endif
...
{
    {0x01000000UL, 0x00020000UL, {0x0001, 0x227E, 0x220C, 0x2201}},   /* AMD 29LV640MH/L   */
    {0x01000000UL, 0x00020000UL, {0x0001, 0x227E, 0x2213, 0x2201}},   /* AMD 29LV640MU     */
    {0x01000000UL, 0x00020000UL, {0x0001, 0x227E, 0x2213, 0x2201}},   /* AMD 29LV641MH/L   */
    {0x02000000UL, 0x00020000UL, {0x0001, 0x227E, 0x2212, 0x2200}},   /* AMD 29LV128M      */
    {0x04000000UL, 0x00020000UL, {0x0001, 0x227E, 0x2212, 0x2201}},   /* AMD 29LV256M      */
    {0x08000000UL, 0x00040000UL, {0x0001, 0x227E, 0x2223, 0x2201}},   /* Spansion 29GL512N */
    ...
    */
        ...
        uDeviceIndex++)
    {
            ...
            (D_UCHAR *)(&(ChipIdentAmdM[uDeviceIndex].IdCode)), sizeof(ID_4_CYCLE)) == 0)
        {
            ...
            FFXPRINTF(1, ("FIM amb2x16:  Mfg=%X Dev1=%X Dev2=%X Dev3=%X supported\n",
    ...
    if(ChipIdentAmdM[uDeviceIndex].ulChipSize == END_OF_LIST_UL)
    {
        ...
        *pMedia = AMDCMD_READ_MODE;
        ...
        FFXPRINTF(1, ("FIM amb2x16:  Mfg=%X Dev1=%X Dev2=%X Dev3=%X unsupported\n",
                    idCode1stChip.uManufCode, idCode1stChip.uDeviceCode1,
                    idCode1stChip.uDeviceCode2, idCode1stChip.uDeviceCode3));
...
#if BUFFERED_WRITES
            fSuccess = WriteBufferedBytes(pEMI, ulStart, pulDataPtr, ulThisLength);
#else
            fSuccess = WriteBytes(pEMI, ulStart, pulDataPtr, ulThisLength);
#endif
            if(!fSuccess)
            {
                DclError();
    ...
    D_UINT32        ulExpectedData)
{
    ...
    */
    if((ulFlashStatus & AMD_BUFF_ABORT_HIGH) && (ulFlashStatus & AMD_BUFF_ABORT_LOW))
    {
        ulAbortMask = HIGH_WORD | LOW_WORD;
    }
    else if(ulFlashStatus & AMD_BUFF_ABORT_HIGH)
    {
        ulAbortMask = HIGH_WORD;
    }
    else if(ulFlashStatus & AMD_BUFF_ABORT_LOW)
    {
        ulAbortMask = LOW_WORD;
    }
    ...
    */
    ...
    if(!ulAbortMask)            /* no Write-to-Buffer Abort detected */
    {
    ...
    *pLatch1 = ((AMDCMD_UNLOCK1 & ulAbortMask) | (AMDCMD_READ_MODE & ~ulAbortMask));
    *pLatch2 = ((AMDCMD_UNLOCK2 & ulAbortMask) | (AMDCMD_READ_MODE & ~ulAbortMask));
    ...
    FFXFIMBOUNDS   *pBounds)
{
    ...
    */
    if(!FfxFimNorWindowCreate(pEMI->hDev, ulAddress, pBounds, (volatile void **)&pMedia))
        return FALSE;
    ulIdCodes = *(D_UINT32 *)pMedia;
    uLowChipID = (D_UINT16)(ulIdCodes & LOW_WORD);
    if(uLowChipID == (D_UINT16)(ulIdCodes >> WORD_LENGTH))
    {
        lpChipID->uManufCode = uLowChipID;
    }
    else
    {
        return FALSE;
    ...
    */
    if(!FfxFimNorWindowCreate(pEMI->hDev, ulAddress + DEVICE_CODE1_OFFSET_2X16, pBounds, (volatile void **)&pMedia))
        return FALSE;
    ulIdCodes = *(D_UINT32 *)pMedia;
    uLowChipID = (D_UINT16)(ulIdCodes & LOW_WORD);
    if(uLowChipID == (D_UINT16) (ulIdCodes >> WORD_LENGTH))
    {
        lpChipID->uDeviceCode1 = uLowChipID;
    }
    else
    {
        return FALSE;
    ...
    */
    if(!FfxFimNorWindowCreate(pEMI->hDev, ulAddress + DEVICE_CODE2_OFFSET_2X16, pBounds, (volatile void **)&pMedia))
        return FALSE;
    ulIdCodes = *(D_UINT32 *)pMedia;
    uLowChipID = (D_UINT16)(ulIdCodes & LOW_WORD);
    if(uLowChipID == (D_UINT16)(ulIdCodes >> WORD_LENGTH))
    {
        lpChipID->uDeviceCode2 = uLowChipID;
    }
    else
    {
        return FALSE;
    ...
    */
    if(!FfxFimNorWindowCreate(pEMI->hDev, ulAddress + DEVICE_CODE3_OFFSET_2X16, pBounds, (volatile void **)&pMedia))
        return FALSE;
    ulIdCodes = *(D_UINT32 *)pMedia;
    uLowChipID = (D_UINT16)(ulIdCodes & LOW_WORD);
    if(uLowChipID == (D_UINT16)(ulIdCodes >> WORD_LENGTH))
    {
        lpChipID->uDeviceCode3 = uLowChipID;
    }
    else
    {
        return FALSE;
    ...
    D_UINT32        ulLength)
{
    ...
    */
        ...
        uBufferIndex += (ucByteCount / FLASH_BUS_BYTES))
    {
        ...
        */
        pMedia[uBufferIndex] = (((ucByteCount / FLASH_BUS_BYTES) - 1) * 0x10001L);
        ...
        */
        for(uDWordIndex = 0;
            ...
            uDWordIndex++)
        {
            pMedia[uBufferIndex + uDWordIndex] = pulData[uBufferIndex + uDWordIndex];
        }
        ...
        */
        if(!DidOperationComplete(&pMedia[uBufferIndex + uDWordIndex - 1],
                                 pulData[uBufferIndex + uDWordIndex - 1], FALSE, &timer))
        {
            if(!HandleProgramFailure(pEMI, ulStart,
                 &pMedia[uBufferIndex + uDWordIndex - 1], pulData[uBufferIndex + uDWordIndex - 1]))
            {
                DclError();
                return FALSE;
            }
    ...
    D_UINT32        ulLength)
{
    ...
    */
    for(uProgramIndex = 0;
        uProgramIndex < (ulLength / sizeof(D_UINT32));
        ++uProgramIndex, ++pulData)
    {
        /*  Write command sequence coded in-line since timing critical
        */
#if !UNLOCK_BYPASS
        ...
        *pLatch1 = AMDCMD_PROGRAM;
        ...
        pMedia[uProgramIndex] = *pulData;

        DclTimerSet(&timer, WRITE_TIMEOUT);

        if(!DidOperationComplete(&pMedia[uProgramIndex], *pulData, FALSE, &timer))
        {
            if(!HandleProgramFailure(pEMI, ulStart, &pMedia[uProgramIndex], *pulData))
            {
                DclError();
                return FALSE;
            }
