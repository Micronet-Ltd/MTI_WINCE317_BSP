...
/*---------------------------------------------------------------------------
    ...
    This module contains the FIM code for Intel StrataFlash which is
    organized in a 2x16 interleaved fashion.

    $DOCTODO
    Change mount to use structure for full ID code test (not ORing)
    Mount - start search for additional devices at ulDeviceSize (nec2x16)
...
/*---------------------------------------------------------------------------
                                Revision History
    $Log: isf2x16.c $
    Revision 1.20  2009/04/03 20:51:41Z  davidh
    Function headers updated for AutoDoc.
    Revision 1.19  2009/04/03 05:40:39Z  keithg
    Fixed bug 2521: removed obsolete READ_BACK_VERIFY.
    Revision 1.18  2008/07/31 16:32:26Z  keithg
    Clarified comments with professional customer facing terms.
    Revision 1.17  2007/11/02 02:09:23Z  Garyp
    Eliminated the inclusion of limits.h.
    Revision 1.16  2007/08/03 00:12:10Z  timothyj
    Changed units of reserved space and maximum size to be in terms
    of KB instead of bytes.
    Revision 1.15  2007/03/01 21:49:24Z  timothyj
    ...
    Revision 1.14  2006/10/13 23:37:15Z  rickc
    Merged from 3.1 release branch
    Revision 1.13  2006/10/07 01:59:27Z  Garyp
    Corrected to eliminate warnings generated by the RealView tools.
    Revision 1.12.1.2  2006/10/13 23:33:17Z  rickc
    Fixed high boot block support
    Revision 1.12.1.1  2006/09/07 03:26:21Z  rickc
    Duplicate revision
    Revision 1.12  2006/09/07 03:26:21Z  Pauli
    ...
    Revision 1.2  2006/02/10 01:01:42Z  Garyp
    Modified to use new FfxHookWindowMap/Size() functions.  Updated debugging
    code.
    Revision 1.1  2005/10/14 02:08:02Z  Pauli
    Initial revision
    Revision 1.24  2005/06/06 23:40:26Z  tonyq
    Modify UnlockBlocks function to unlock the last block of the address range
    supplied.
    Revision 1.23  2005/05/16 21:43:12Z  Pauli
    Removed invalid asserts for pMedia.  The flash can be memory mapped to
    zero (0).
    Revision 1.22  2005/05/13 02:36:04Z  garyp
    Cleaned up the erase zone size define.
    Revision 1.21  2005/05/12 01:59:26Z  garyp
    Added various K3/K18 parts that should have been in there to start with.
    Revision 1.20  2005/05/08 17:07:21Z  garyp
    ...
    Revision 1.17  2005/01/23 00:44:39Z  billr
    Can't include limits.h in a kernel build.
    Revision 1.16  2004/12/30 17:32:49Z  GaryP
    Updated to use indents of 4, hard tabs of 8 (if used), and a standard
    comment style.
    Revision 1.15  2004/12/18 06:55:42Z  garys
    reset blocks after unlock to put in read array mode
    Revision 1.14  2004/10/01 01:24:34  jaredw
    Added in L18 flash support to FIM.
    Revision 1.13  2004/09/25 01:29:30Z  GaryP
    Eliminated the use of D_STATIC.  Documentation update.
    Revision 1.12  2004/08/31 22:14:42Z  PaulI
    Added pulMedia initialization as well as ulLength == 0 check in function
    unlockBlocks.
    Revision 1.11  2004/08/10 23:45:18Z  Pauli
    Removed serveral asserts of the media pointer, which are not valid
    if the flash starts at address 0.
    Revision 1.10  2004/08/10 20:35:11Z  Pauli
    Removed some bogus asserts.
    Revision 1.9  2004/08/05 22:37:44Z  jaredw
    Added support for 28F128K3 strata flash.
    Revision 1.8  2004/08/05 16:35:08Z  billr
    Move optional functions to the end of FIMDEVICE.
    Revision 1.7  2004/08/04 21:57:23Z  billr
    Eliminate ulInterleaved in ExtndMediaInfo, no longer used or needed.
    Revision 1.6  2004/08/04 21:25:54Z  billr
    Eliminate ulFimFlags in ExtndMediaInfo, no longer used or needed.
    Revision 1.5  2004/07/26 19:08:06Z  jaredw
    Added initialization to pulMedia to satisfy VxWorks compiler warning
    as well as added a check to return FALSE if ulLength is zero.
    Revision 1.4  2004/06/28 22:27:07Z  BillR
    Thread-safe timeouts are in milliseconds, not microseconds.  Removed dead
    code.
    Revision 1.3  2004/06/15 21:57:05Z  BillR
    New interface to multi-threaded FIMs.
    Revision 1.2  2003/05/06 19:40:20Z  garys
    Revision 1.1.1.4  2003/05/06 19:40:20  garys
    added check for return value from FfxHookAccessBegin()
    Revision 1.1.1.3  2004/01/03 03:12:40  garyp
    Changed so that the uDeviceType characteristics are initialized before
    FfxHookAccessBegin(ACCESS_MOUNTS) is called.
    Revision 1.1.1.2  2003/12/17 22:32:50Z  garyp
    Added the prototypes for standard functions.
    Revision 1.1  2003/12/17 18:43:50Z  garyp
    Initial revision
---------------------------------------------------------------------------*/
...
#define MAX_WRITE_BUFF_32       0x40   /* 2 x J3/J5 StrataFlash max */
#define MAX_WRITE_BUFF_64       0x80   /* 2 x K3/K18 StrataFlash max or L30/L18 */
...
*/
#define ZONE_SIZE               (0x00020000UL * FLASH_INTERLEAVE) /* 256KB */
#define BOOT_ZONE_SIZE          (0x00008000UL * FLASH_INTERLEAVE) /*  64KB */
    ...
    PEXTMEDIAINFO   pEMI)
{
    ...
    *pMedia = INTLCMD_IDENTIFY;
    ...
    {
        FFXPRINTF(1, ("FIM isf2x16:  Invalid flash ID: Data0/1=%08lx/%08lx\n",
            ID.data0, ID.data1));
    ...
    */
    for(i = 0; ChipTable[i].ulChipSize != D_UINT32_MAX; i++)
    {
           ...
           (ID.idDev == ChipTable[i].idDev))
        {
           pEMI->ulDeviceSize = ChipTable[i].ulChipSize * FLASH_INTERLEAVE;
            ...
            FFXPRINTF(1, ("FIM isf2x16:  Mfg/Dev=%04x/%04x supported\n", ID.idMfg, ID.idDev));
    ...
    {
        FFXPRINTF(1, ("FIM isf2x16:  Mfg/Dev=%04x/%04x unsupported\n", ID.idMfg, ID.idDev));
    ...
    */
    ...
    if(ChipTable[i].uFlags & ID_BUFF64)
        pEMI->pFimExtra->uMaxWriteBuff = MAX_WRITE_BUFF_64;
    else
        pEMI->pFimExtra->uMaxWriteBuff = MAX_WRITE_BUFF_32;
    ...
    if(ChipTable[i].uLowBootBlocks || ChipTable[i].uHighBootBlocks)
    {
        ...
        FFXPRINTF(1, ("FIM isf2x16: BootBlockSize=%lUKB Low=%U High=%U\n",
    ...
    void           *pBuffer)
{
    ...
    DclAssert(pEMI);
    DclAssert(ulStart % sizeof(FLASHDATA) == 0L);
    DclAssert(uLength);
    DclAssert(pBuffer);
    ...
    PEXTMEDIAINFO   pEMI)
{
    ...
    */
        ...
        u += pEMI->pFimExtra->uMaxWriteBuff / FLASH_BUS_BYTES)
    {
        ...
        */
        pMedia[u] = (((ucByteCount / FLASH_BUS_BYTES) - 1) * 0x10001L);
