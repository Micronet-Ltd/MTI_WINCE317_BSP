...
/*---------------------------------------------------------------------------
    ...
    $DOCTODO
    Test with E28F640 device.
    Test with 2 or more E28F320 devices.
    Optimize FIM for Strata CIM ?
---------------------------------------------------------------------------*/

/*---------------------------------------------------------------------------
                                Revision History
    $Log: isfx16.c $
    Revision 1.22  2009/04/03 20:58:38Z  davidh
    Function headers updated for AutoDoc.
    Revision 1.21  2009/04/03 05:40:40Z  keithg
    Fixed bug 2521: removed obsolete READ_BACK_VERIFY.
    Revision 1.20  2008/07/31 16:32:28Z  keithg
    Clarified comments with professional customer facing terms.
    Revision 1.19  2007/11/03 23:49:37Z  Garyp
    Updated to use the standard module header.
    Revision 1.18  2007/08/03 00:12:32Z  timothyj
    Changed units of reserved space and maximum size to be in terms
    of KB instead of bytes.
    Revision 1.17  2007/03/01 21:49:50Z  timothyj
    ...
    Revision 1.16  2006/10/13 23:37:37Z  rickc
    Merged from 3.1 release branch
    Revision 1.15  2006/10/07 01:56:38Z  Garyp
    Corrected to eliminate warnings generated by the RealView tools.
    Revision 1.14.1.2  2006/10/13 23:33:50Z  rickc
    Fixed high boot block support
    Revision 1.14.1.1  2006/09/20 23:47:08Z  rickc
    Duplicate revision
    Revision 1.14  2006/09/20 23:47:08Z  Garyp
    Fixed a bogus comment.
    Revision 1.13  2006/09/07 03:26:19Z  Pauli
    ...
    Revision 1.12  2006/09/06 02:48:23Z  Pauli
    Changed UnlockBlocks() to use FfxFimNorWindowCreate() as it is
    used during mount and cannot use FfxFimNorWindowMap().
    Revision 1.11  2006/08/29 16:48:52Z  pauli
    Updated to use the new FfxFimNorWindowCreate/FfxFimNorWindowMap functions.
    Revision 1.10  2006/07/31 20:33:45Z  timothyj
    Added P30 family
    Revision 1.9  2006/06/07 01:55:03Z  timothyj
    Added P30 family
    Revision 1.8  2006/06/07 01:55:03Z  Pauli
    Updated boot block handling to work with non boot block parts.
    Revision 1.7  2006/05/20 22:00:40Z  Garyp
    Modified to use a more flexible mechanism for handling boot blocks, which
    is compatible with NORWRAP FIM.
    Revision 1.6  2006/04/26 20:30:54Z  billr
    Eliminate pointer arithmetic, use array indexing instead.
    Revision 1.5  2006/03/16 04:07:24Z  Garyp
    Fixed syntax errors with the previous checkin.
    Revision 1.4  2006/03/16 02:41:03Z  Garyp
    Added missing L18/Tyax code stemming from the v2.0 merge.
    Revision 1.3  2006/02/10 21:08:28Z  Garyp
    Renamed the FIMDEVICE structure instantiation for clarity.
    Revision 1.2  2006/02/08 18:36:05Z  Garyp
    Modified to use new FfxHookWindowMap/Size() functions.  Updated debugging
    code.
    Revision 1.1  2005/10/14 02:08:02Z  Pauli
    Initial revision
    Revision 1.9  2005/07/20 21:50:40Z  billr
    Add code overlooked in L18/Tyax merge.
    Revision 1.8  2005/07/20 20:45:26Z  Rickc
    added prototype
    Revision 1.7  2005/07/19 23:48:01  cheryl
    Added support for L18/Tyax
    Revision 1.6  2005/05/16 21:42:26Z  Pauli
    Removed invalid asserts for pMedia.  The flash can be memory mapped to
    zero (0).
    Revision 1.5  2005/05/13 02:36:04Z  garyp
    Cleaned up the erase zone size define.
    Revision 1.4  2005/05/09 19:05:30Z  garyp
    Updated to use ID_ENDOFLIST.
    Revision 1.3  2005/05/08 16:47:32Z  garyp
    ...
    comment style.
    Revision 1.1  2004/08/10 21:32:56Z  jaredw
    Initial revision
---------------------------------------------------------------------------*/
...
static D_BOOL WriteBufferedBytes( PFLASHDATA pMedia, D_UINT16 * puData, D_UINT32 ulLength, PEXTMEDIAINFO pEMI);
...
#define MAX_WRITE_BUFF_32       0x20   /* 1 x J3/J5 StrataFlash max of 32 bytes*/
#define MAX_WRITE_BUFF_64       0x40   /* 1 x K3/K18 StrataFlash max or L30/L18 max of 64 bytes */

/*  All parts supported in this FIM have 128KB erase zones, and any
    of them that support boot blocks (L18s) have 4 32KB boot block
    zones -- multiply appropriately for the interleave value.
*/
#define ZONE_SIZE               (0x00020000UL * FLASH_INTERLEAVE) /* 128KB */
#define BOOT_ZONE_SIZE          (0x00008000UL * FLASH_INTERLEAVE) /*  32KB */
    ...
    PEXTMEDIAINFO   pEMI)
{
    ...
    *pMedia = INTLCMD_IDENTIFY;
    ...
    {
        FFXPRINTF(1, ("FIM isfx16:   Invalid flash ID: Data0/1=%04x/%04x\n",
            ID.data0, ID.data1));
    ...
    for(i = 0; ChipTable[i].ulChipSize != D_UINT32_MAX; i++)
    {
           ...
           (ID.idDev == ChipTable[i].idDev))
        {
            ...
            FFXPRINTF(1, ("FIM isfx16:   Mfg/Dev=%04x/%04x supported\n", ID.idMfg, ID.idDev));
    ...
    {
        FFXPRINTF(1, ("FIM isfx16:   Mfg/Dev=%04x/%04x unsupported\n", ID.idMfg, ID.idDev));
    ...
    */
    ...
    if(ChipTable[i].uLowBootBlocks || ChipTable[i].uHighBootBlocks)
    {
        ...
        FFXPRINTF(1, ("FIM isfx16:  BootBlockSize=%lUKB Low=%U High=%U\n",
    ...
    while(uLength)
    {
        ...
        */
        ...
        while(ulWindowSize && uLength)
        {
            ...
            fSuccess = WriteBufferedBytes(pMedia, puDataPtr, ulThisLength, pEMI);
            if(!fSuccess)
            {
                DclError();
                break;
