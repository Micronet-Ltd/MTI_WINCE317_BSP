...
/*---------------------------------------------------------------------------
    ...
    This module contains the FIM code for Intel Sibley Wireless Flash which is
    organized in a 16-bit wide, non-interleaved fashion.
...
/*---------------------------------------------------------------------------
    ...
    Revision 1.9  2006/09/07 03:26:21Z  Pauli
    ...
    Revision 1.8  2006/09/06 02:48:28Z  Pauli
    Changed UnlockBlocks() to use FfxFimNorWindowCreate() as it is
    used during mount and cannot use FfxFimNorWindowMap().
    Corrected a copy/paste bug in WriteControl() where the passed
    in start was mapped instead of the adjusted offset.
    Revision 1.7  2006/08/29 16:49:20Z  pauli
    Updated to use the new FfxFimNorWindowCreate/FfxFimNorWindowMap
    functions.
    Revision 1.6  2006/05/20 22:00:40Z  Garyp
    Minor fixes to deal with renamed or obsolete structure fields.
    Revision 1.5  2006/02/11 00:13:00Z  Garyp
    Renamed the FIMDEVICE structure instantiation for clarity.
    Revision 1.4  2006/02/08 18:36:05Z  Garyp
    Modified to use new FfxHookWindowMap/Size() functions.  Updated
    debugging code.
    Revision 1.3  2006/01/31 20:04:08Z  Rickc
    Added ST 58PR512J
    Revision 1.2  2006/01/25 20:35:22Z  tonyq
    Corrected comments to state that bits 0-9 are used to determine the offset
    into a page to Read/Write Read/WriteControl calls.
    Revision 1.1  2006/01/11 21:46:56Z  billr
    Initial revision
    Revision 1.3  2005/11/17 01:03:28Z  Rickc
    Added revision control header
    Revision 1.2  2005/11/14 18:34:38  billr
    Fix compiler warnings in some environments.
    Revision 1.1  2005/08/27 19:45:12 cheryl
    Initial revision
---------------------------------------------------------------------------*/
...
#include "intelcmd_iswf.h"
#include "intelcmd.h"

#define FLASH_PAGE_SIZE                1024 * FLASH_INTERLEAVE

/* For control mode, input buffer must have 16 * FLASH_INTERLEAVE valid bytes
   and then next 16 * FLASH_INTERLEAVE are invalid bytes
*/
#define FLASH_CONTROL_VALID_SIZE       16 * FLASH_INTERLEAVE /* valid size in Bytes */
#define FLASH_CONTROL_INVALID_SIZE     16 * FLASH_INTERLEAVE /* invalid size in Bytes */
...
/*  Datasheet specifies 4 seconds, increment to 6 seconds here.
*/
#define ERASE_TIMEOUT           (6 * 1000L)
...
static D_BOOL WriteBufferedBytes( PFLASHDATA pMedia, D_UINT16 *puData, D_UINT32 ulLength, PEXTMEDIAINFO pEMI, D_UINT32 ulFlashOffset, D_BOOL bIsControl);
static D_BOOL   UnlockBlocks(D_UINT32 ulLength, FFXFIMBOUNDS *pBounds, PEXTMEDIAINFO pEMI);


/*  offset to check block status after sending the identify command
*/
#define iBLOCK_LOCK_STATUS_OFFSET  4
#define iBLOCK_LOCKED              0x0001L
#define iBLOCK_LOCKED_DOWN         0x0002L


#define MAX_WRITE_BUFF_1024     0x400 /* 1 x M18 StrataFlash max 1024 bytes */
...
*/
#define ZONE_SIZE               (0x00040000UL * FLASH_INTERLEAVE) /* 256KB */
    ...
    PEXTMEDIAINFO   pEMI)
{
    ...
    *pMedia = INTLCMD_IDENTIFY;
    ...
    {
        FFXPRINTF(1, ("FFX: FIM iswfx16:   Invalid flash ID: Data0/1=%04x/%04x\n",
            ID.data0, ID.data1));
    ...
    for(i = 0; ChipTable[i].ulChipSize != D_UINT32_MAX; i++)
    {
           ...
           (ID.idDev == ChipTable[i].idDev))
        {
            ...
            FFXPRINTF(1, ("FFX: FIM iswfx16:   Mfg/Dev=%04x/%04x supported\n", ID.idMfg, ID.idDev));
    ...
    {
        FFXPRINTF(1, ("FFX: FIM iswfx16:   Mfg/Dev=%04x/%04x unsupported\n", ID.idMfg, ID.idDev));
    ...
    */
    ...
    /*  Unlock all blocks if necessary
    */
    ...
    DclAssert( (ChipTable[i].uFlags & ID_BUFF1024) != 0);
    pEMI->pFimExtra->uMaxWriteBuff = MAX_WRITE_BUFF_1024;
...
/*-------------------------------------------------------------------
    ...
    Read a given number of bytes of data from the media in object mode.
...
/*-------------------------------------------------------------------
    ...
    Writes a given number of bytes of data out to the media. (object mode)
    ...
    while(uLength)
    {
        ...
        */
        while(ulWindowSize && uLength)
        {
            ...
            */
            ulWindowSize    -= ulThisLength;
            ulFlashOffset   += (ulThisLength + FLASH_CONTROL_INVALID_SIZE);
...
/*    ulBaseLatch = ulStart & (~(ZONE_SIZE - 1));  Unused ? */
    ...
    */
    if(ulLength < ZONE_SIZE || ulLength % ZONE_SIZE != 0 ||
        ulStart >= pEMI->ulTotalSize || ulLength > pEMI->ulTotalSize - ulStart)
    {
        return FALSE;
    }
    ...
    if(pFimExtra->ulEraseResult == ERASE_IN_PROGRESS)
    {
        ...
        *pMedia = INTLCMD_READ_STATUS;
        ...
        /*  It's possible that  the chip finished its erase before
            ...
            If the chip is showing error status, there's no point
            in trying to resume.
        */
...
/*-------------------------------------------------------------------
    ...
    Parameters:
        ...
        ulFlashOffset - used to help find program buffer alignment (DO NOT USE TO ACCESS ACTUAL ADDRESS)
    ...
    D_BOOL          bIsControl)
{
    ...
    */
    if((pMedia[uFlashIndex] & INTLSTAT_DONE) != INTLSTAT_DONE)
    {
        DclError();
        return FALSE;
    }
    ...
    */
    if(bIsControl)
    {
        uByteBuffSize = (D_UINT16)(FLASH_CONTROL_VALID_SIZE - (ulFlashOffset %
                (FLASH_CONTROL_VALID_SIZE + FLASH_CONTROL_INVALID_SIZE)));
    }
    else
    {
    ...
    */
    while(ulLength)
    {
        ...
        */
        pMedia[uFlashIndex] = INTLCMD_BUFFERED_WRITE;
        ...
        */
        ...
        /*  In 16 bit mode, we write out words instead of bytes
        */
        pMedia[uFlashIndex] = ((uByteCount / FLASH_BUS_BYTES) - 1);
        ...
        */
        pMedia[uFlashIndex] = INTLCMD_CONFIRM;
        ...
        */
        ...
        if(!IsStateMachineReady(&pMedia[uFlashIndex]))
            return FALSE;

        /*  Check for error
        */
        if(pMedia[uFlashIndex] & INTLSTAT_WRITE_FAIL)
        {
            ...
            /* give better error */
            if((pMedia[uFlashIndex] & INTLSTAT_OBJ_TO_CTL) == INTLSTAT_OBJ_TO_CTL)
            {
                FFXPRINTF(1, ("Attempted write with Object mode data to control mode region\n"));
            }
            else if((pMedia[uFlashIndex] & INTLSTAT_REWRITE_OBJ) == INTLSTAT_REWRITE_OBJ)
            {
                FFXPRINTF(1, ("Attempted rewrite to object mode region\n"));
            }
            else if((pMedia[uFlashIndex] & INTLSTAT_ILLEGAL_CMD) == INTLSTAT_ILLEGAL_CMD)
            {
                FFXPRINTF(1, ("Write using illegal command\n"));
            }
            DclError();
        ...
        */
        ...
        /*  uFlashIndex update is dependant on memory access type.
        */
        if(bIsControl)
        {
            /*  Must increment past invalid flash area.
            */
            uFlashIndex += (uByteCount+FLASH_CONTROL_INVALID_SIZE)/FLASH_BUS_BYTES;

            /*  Make sure uByteBuffSize is set to maximum size if it is
                not already.
            */
            uByteBuffSize = FLASH_CONTROL_VALID_SIZE;
        }
        else
        {
    ...
    PEXTMEDIAINFO   pEMI)
{
    ...
    */
        ...
        ulIndex += pEMI->ulEraseZoneSize)
    {
        ...
        */
        *pMedia = INTLCMD_IDENTIFY;
        ulBlockStatus = *(PFLASHDATA)((D_UINT32)pMedia + iBLOCK_LOCK_STATUS_OFFSET);

        if((ulBlockStatus & iBLOCK_LOCKED) || (ulBlockStatus & iBLOCK_LOCKED_DOWN))
        {
            /*  Clear the error status, reset to read mode and return.
            */
            ResetFlash(pMedia);
