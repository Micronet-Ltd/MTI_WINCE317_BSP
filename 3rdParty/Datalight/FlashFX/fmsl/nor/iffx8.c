...
/*---------------------------------------------------------------------------
    ...
    organized in an 8-bit wide, non-interleaved fashion.
---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------
    ...
    Revision 1.5  2006/08/30 00:40:46Z  pauli
    Updated to use the new FfxFimNorWindowCreate/FfxFimNorWindowMap
    functions.
    Revision 1.4  2006/05/20 21:32:08Z  Garyp
    ...
    Revision 1.2  2006/02/08 18:36:07Z  Garyp
    Modified to use new FfxHookWindowMap/Size() functions.  Updated debugging
    code.
    Revision 1.1  2005/10/14 02:08:02Z  Pauli
    Initial revision
    Revision 1.9  2005/05/16 19:54:43Z  garyp
    Removed some bogus asserts.  The media pointer can legally be zero on
    some platforms.
    Revision 1.8  2005/05/13 02:36:05Z  garyp
    Added the DEV_NOT_MLC flag.
    Revision 1.7  2005/05/09 19:05:30Z  garyp
    Updated to use ID_ENDOFLIST.
    Revision 1.6  2005/05/08 17:07:21Z  garyp
    ...
    PEXTMEDIAINFO   pEMI)
{
    ...
    */
    ...
    /*  ID first chip.  Reset twice to avoid receiving 0x80 from a previous
        operation in the case of a write interruption.
    */
    ResetFlash(pMedia);
    ResetFlash(pMedia);
    *pMedia = INTLCMD_IDENTIFY;
    ...
    {
        FFXPRINTF(1, ("FIM iffx8:    Invalid flash ID: Data0/1=%02x/%02x\n", ID.data0, ID.data1));
    ...
    */
    for(i = 0; ChipTable[i].ulChipSize != D_UINT32_MAX; i++)
    {
           ...
           (ID.idDev == ChipTable[i].idDev))
        {
            ...
            FFXPRINTF(1, ("FIM iffx8:    Mfg/Dev=%02x/%02x supported\n", ID.idMfg, ID.idDev));
    ...
    */
    if(ChipTable[i].ulChipSize == D_UINT32_MAX)
    {
        ...
        */
        ...
        FFXPRINTF(1, ("FIM iffx8:    Mfg/Dev=%02x/%02x unsupported\n", ID.idMfg, ID.idDev));
        ...
        */
        while(ulWindowSize && uLength)
        {
            ...
            */
            ...
            {
                if(pcDataPtr[u] != (D_UCHAR) 0xFF) /*don't waste cylces writing 0xFF */
                {
    ...
    D_UINT32        ulLength)
{
    ...
    */
    if(ulLength < ZONE_SIZE
       ...
       || ulStart >= pEMI->ulTotalSize
       || ulLength > pEMI->ulTotalSize - ulStart)
    {
        return FALSE;
    }
    ...
    if(pFimExtra->ulEraseResult == ERASE_IN_PROGRESS)
    {
        ...
        *pMedia = INTLCMD_READ_STATUS;
        ...
        /*  It's possible that the chip finished its erase before
            ...
            If one of the chips is showing error status, there's no
            point in trying to resume.
        */
