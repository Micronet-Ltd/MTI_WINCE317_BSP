...
/*---------------------------------------------------------------------------
    ...
    uses Boot Blocks and is organized in a x16 fashion.
...
*/
#define AMD_ERASE_CMD2      MAKEFLASHCMD(ACMD_ERASE2)

/*  Chip specific status indicators
*/
#define AMD_DONE            MAKEFLASHCMD(ASTAT_OK)   /* DQ7 Data# polling */
#define AMD_FAIL            MAKEFLASHCMD(ASTAT_FAIL)/* DQ5 exceeded timing limits */
#define AMD_ERASE_BEGUN     MAKEFLASHCMD(ASTAT_ERASEBEGUN)  /* DQ3 */

/*  Latch address offsets
*/
#define LATCH_OFFSET1       0xAAAAUL
#define LATCH_OFFSET2       0x5554UL
...
#else
#define WAIT_FOR_WRITE_DONE(MediaPtr, CurrentData, pt)                      \
   while((*(MediaPtr) & AMDSTAT_DONE) != ((FLASHDATA)((CurrentData) & AMDSTAT_DONE)))   \
   {                                                                        \
      if(DclTimerExpired(pt))                                               \
      {                                                                     \
         /* One more check for done in case we got scheduled out */         \
         if((*(MediaPtr) & AMDSTAT_DONE) != ((FLASHDATA)(CurrentData) & AMDSTAT_DONE))  \
         {                                                                  \
            DclError();                                                   \
            /* Just in case we were in unlock bypass mode */                \
            *(PFLASHDATA)MediaPtr = AMDCMD_IDENTIFY;                        \
    ...
    PEXTMEDIAINFO   pEMI)
{
    ...
    *pLatch1 = AMDCMD_IDENTIFY;
    ...
    {
        FFXPRINTF(1, ("FIM asbx16:  Invalid flash ID: Data0/1=%04x/%04x\n",
            ID.data0, ID.data1));
    ...
    */
    for(i = 0; ChipTable[i].uIDFlags != IDFLAG_ENDOFLIST; i++)
    {
        ...
        {
            pEMI->ulDeviceSize = ChipTable[i].ulChipSize;

            pEMI->uDeviceType  = DEV_NOR | DEV_MERGE_WRITES;
            pEMI->uDeviceType  = pEMI->uDeviceType | ChipTable[i].uDevNotMlc;

            pEMI->ulEraseZoneSize = ChipTable[i].ulBlockSize;

            FFXPRINTF(1, ("FIM asbx16:  Mfg/Dev=%04x/%04x supported\n", ID.idMfg, ID.idDev));
    ...
    {
        FFXPRINTF(1, ("FIM asbx16:  Mfg/Dev=%04x/%04x unsupported\n", ID.idMfg, ID.idDev));
    ...
    */
    ...
    if(ChipTable[i].ulBootBlockSize)
    {
        ...
        FFXPRINTF(1, ("FIM asbx16: BootBlockSize=%lUKB Low=%U High=%U\n",
            ...
            */
                ...
                ++uProgramIndex, ++puDataPtr)
            {
                ...
                *pLatch1 = AMDCMD_PROGRAM;
                ...
                pMedia[uProgramIndex] = *puDataPtr;
                DclTimerSet(&timer, WRITE_TIMEOUT);

                /*  Reset flash and return FALSE if the operation fails
                */
                WAIT_FOR_WRITE_DONE(&pMedia[uProgramIndex], *puDataPtr, &timer)
            }
        ...
        */
        switch (*pMedia & AMDSTAT_MASK)
        {
            case (AMDSTAT_DONE | AMDSTAT_ERASEBEGUN):
                ...
                /*  erase is done, ErasePoll() will report it
                */
                break;
            case (AMDSTAT_DONE):
    ...
    if(pFimExtra->ulEraseResult == ERASE_IN_PROGRESS)
    {
        ...
        */
        ...
        else
        {
            ...
            */
            if(((ulFlashStatus & AMD_DONE) != AMD_DONE) &&
                (*pMedia & AMD_FAIL))
            {
    ...
    DCLTIMER       *ptimer)
{
    ...
    */
    while((ulFlashStatus & AMDSTAT_DONE) != (ulExpectedData & AMDSTAT_DONE))
    {
        /*  Check for error status
        */
        if(((ulFlashStatus & AMD_DONE) != (ulExpectedData & AMD_DONE)) &&
           (ulFlashStatus & AMD_FAIL))
        {
            /*  According to flowchart diagrams in the AMD and NEC data sheets,
                we must do a separate read to make sure the operation hasn't
                just completed.
