...
/*---------------------------------------------------------------------------
    ...
    Revision 1.1  2011/11/22 17:14:56Z  glenns
    Initial revision
...
#include <nandparam.h>
#define NTMNAME "NTECLRNAND"

#define NAND_ONFI_ID_SIZE           (4)
#define CMD_READ_MODE               (0x0)


#define ECLRNAND_CUSTOM_SPARE_SIZE (128)
#define ECLRNAND_TAG_OFFSET        (0x2)
#define ECLRNAND_TAG_LENGTH        (0x3)
#define ECLRNAND_FLAGS_OFFSET      (0x10)
#define ECLRNAND_FACTORYBAD_OFFSET (0x0)
#define ECLRNAND_WRITTEN_WITH_ECC  (0)
...
*/
#define ECLRNAND_DEVICE_ESAMPLE     (1)
...
*/
#define SET_CHIP_SELECT(pg, mod)             (FfxHookNTECLRNandSetChipSelect(hNTM->pNC, CHIP_OF_PAGE(hNTM, pg), (mod)))
#define CHIP_READY_WAIT(ucWaitFor, ucCommand)(FfxHookNTECLRNandReadyWait(hNTM->pNC, (ucWaitFor), (ucCommand)))
#define SET_CHIP_COMMAND(x)                  (FfxHookNTECLRNandSetChipCommand(hNTM->pNC, (x)))
#define SET_CHIP_COMMAND_NOWAIT(x)           (FfxHookNTECLRNandSetChipCommandNoWait(hNTM->pNC, (x)))
#define NAND_DATA_IN(x, y)                   (FfxHookNTECLRNandDataIn(hNTM->pNC, (x), (y)))
#define NAND_STATUS_IN()                     (FfxHookNTECLRNandStatusIn(hNTM->pNC))
#define NAND_DATA_OUT(x, y)                  (FfxHookNTECLRNandDataOut(hNTM->pNC, (x), (y)))
#define NAND_ADDR_OUT(addr)                  (FfxHookNTECLRNandAddressOut(hNTM->pNC, (addr)))
...
static D_BOOL       ECLRNANDCheckOperationStatus(NTMHANDLE hNTM, D_UCHAR ucOperation, D_UINT32 ulPageNum, FFXIOSTATUS *pIOStat);
static FFXSTATUS    LUNStatus(NTMHANDLE hNTM, D_UINT32 ulPageIndex, D_UCHAR ucWaitfor);
static D_BOOL       ECLRNANDNtmTagDecode(D_BUFFER *pDest, const D_BUFFER *pSrc);
static D_BOOL       ECLRNANDNtmTagEncode(D_BUFFER *pDest, const D_BUFFER *pSrc);
...
static void         SetLogicalPageAddress(NTMHANDLE hNTM, D_UINT32 ulPageNum, D_UINT32 ulPageOffset, D_BOOL fRead, unsigned nPlane);
...
/*-------------------------------------------------------------------
    ...
    into the NTM.
-------------------------------------------------------------------*/
NANDTECHNOLOGYMODULE FFXNTM_eclrnand =
{
    Create,
    Destroy,
    PageRead,
    ...
    const NTMINFO     **ppNtmInfo)
{
    ...
    pNTM->pNC = FfxHookNTECLRNandCreate(hDev);
    if(!pNTM->pNC)
        goto CreateCleanup;
...
*/
    ...
    */
    ...
    pNTM->uTagOffset    = ECLRNAND_TAG_OFFSET;
    pNTM->uFlagsOffset  = ECLRNAND_FLAGS_OFFSET;
...
CreateCleanup:
    ...
    {
        if(pNTM->hHook)
            FfxHookNTECLRNandDestroy(pNTM->pNC);
    ...
    NTMHANDLE           hNTM)
{
    ...
    if(hNTM->pNC)
        FfxHookNTECLRNandDestroy(hNTM->pNC);
    ...
    D_BOOL              fUseEcc)
{
    ...
    DCLALIGNEDBUFFER    (spare, data, ECLRNAND_CUSTOM_SPARE_SIZE);    
    D_UINT32            ulChipPage;
    ...
    */
    ...
    ioStat.ffxStat = CHIP_READY_WAIT((DEVICE_READY | DEVICE_READY_READ_ERASE), CMD_DEV_STATUS);
    if(ioStat.ffxStat != FFXSTAT_SUCCESS)
        goto PageReadCleanup;
    ...
    while(ulCount)
    {
        ...
        */
        ioStat.ffxStat = SET_CHIP_COMMAND_NOWAIT(CMD_QUEUED_READ);
        if(ioStat.ffxStat != FFXSTAT_SUCCESS)
            break;

        SetLogicalPageAddress(hNTM, ulChipPage, 0, FALSE, PLANE0);

        ioStat.ffxStat = SET_CHIP_COMMAND_NOWAIT(CMD_QUEUED_READ_START);
        if(ioStat.ffxStat != FFXSTAT_SUCCESS)
            break;
        
        /*  Micron Enhanced Clear NAND: Send Operation Status sequence and check for any errors
        */
        {
            FFXIOSTATUS ioValid = INITIAL_PAGEIO_STATUS(FFXSTAT_FIMUNCORRECTABLEDATA); 
            if(!ECLRNANDCheckOperationStatus(hNTM, OP_READ, ulChipPage, &ioValid))
            {
        ...
        if(pTags)
        {
            ...
            */
            ECLRNANDNtmTagDecode(pTags, &spare.data[ECLRNAND_TAG_OFFSET]);
    ...
    */
    ...
    while(ulCount)
    {
        if(hNTM->pChipInfo->pChipClass->ResetBeforeProgram)
        {
            ...
            if(ioStat.ffxStat == FFXSTAT_SUCCESS)
                ioStat.ffxStat = CHIP_READY_WAIT(NANDSTAT_READY, CMD_STATUS);

            if(ioStat.ffxStat != FFXSTAT_SUCCESS)
                break;
        }

        ioStat.ffxStat = LUNStatus(hNTM, ulChipPage, LUN_PROGRAM_READY);
        if(ioStat.ffxStat  != FFXSTAT_SUCCESS)
        {
            return ioStat;
        }
        ...
        */
        ...
        {
            if (pTags)
                ECLRNANDNtmTagEncode(&(spare.data[ECLRNAND_TAG_OFFSET]), pTags);
            spare.data[ECLRNAND_FLAGS_OFFSET] = ECLRNAND_WRITTEN_WITH_ECC;
        ...
        */
        NAND_DATA_OUT((NAND_IO_TYPE*)&spare.data, hNTM->NtmInfo.uSpareSize);        
    ...
    D_UINT32            ulCount)
{
    ...
    */
    ...
    while(ulCount)
    {
        ioStat.ffxStat  = LUNStatus(hNTM, ulChipPage, LUN_READ_ERASE_READY);
        if(ioStat.ffxStat  != FFXSTAT_SUCCESS)
        {
            return ioStat;
        }
        ...
        */
        ...
        SetLogicalPageAddress(hNTM, ulChipPage, 0, TRUE, PLANE0);
    ...
    */
    ...
    while(ulCount)
    {
        ...
        */
        {
            FFXIOSTATUS ioValid = INITIAL_PAGEIO_STATUS(FFXSTAT_FIMUNCORRECTABLEDATA); 
            ECLRNANDCheckOperationStatus(hNTM, OP_READ, ulChipPage, &ioValid);
        }
    ...
    while(nLength)
    {
        ...
        */
...

        {
            ...
            */
            (void)ECLRNANDNtmTagDecode(pTags, &spare.data[ECLRNAND_TAG_OFFSET]);
        }
...
{    
    ...
    */
    ...
    if(ioStat.ffxStat == FFXSTAT_SUCCESS)
    {
        ...
        */
        DclMemSet(spare.data, ERASED8, hNTM->NtmInfo.uSpareSize);
        ECLRNANDNtmTagEncode(&(spare.data[ECLRNAND_TAG_OFFSET]), pTag);
    ...
    D_UINT32        ulBlock)
{
    ...
    */
    ...
    if(ioStat.ffxStat == FFXSTAT_SUCCESS)
    {
        ...
        ioStat.ffxStat = LUNStatus(hNTM, ulRowIndex, LUN_COMMAND_IDLE);
    ...
    D_UINT32            ulPage)
{
       ...
       
    {
        ...
        */
        if(!FfxNtmHelpIsRangeErased1Bit(&pIO[ECLRNAND_TAG_OFFSET], ECLRNAND_TAG_LENGTH)) 
            ioStat.op.ulPageStatus |= PAGESTATUS_SET_TAG_WIDTH(LEGACY_TAG_SIZE);

        ulFlags += pIO[ECLRNAND_FLAGS_OFFSET];
    ...
    unsigned        nPlane)
{
    ...
    */
    ...
    {
        SetLogicalPageAddress(hNTM, ulChipPage, nOffset, TRUE, nPlane);
        ...
        if(ffxStat == FFXSTAT_SUCCESS)
        {
            ...
            */
            if(ffxStat == FFXSTAT_SUCCESS)
            {
                ...
                ECLRNANDCheckOperationStatus(hNTM, OP_READ, ulChipPage, &ioValid);
                if(ioValid.ffxStat != FFXSTAT_SUCCESS)
                    return ioValid.ffxStat;
    ...
    unsigned        nPlane)
{
    ...
    ffxStat = SET_CHIP_COMMAND_NOWAIT(0x85);
    if(ffxStat == FFXSTAT_SUCCESS)
    {
...
/*-------------------------------------------------------------------
    Local: ECLRNANDCheckOperationStatus()

    Get the Operation status.  The pIOStat->ffxStat and 
    ...
    FFXIOSTATUS    *pIOStat)
{
    ...
    /*  Micron Enhanced Clear NAND: Issue Device status and wait (Spec Fig 41)
    */
    pIOStat->ffxStat = CHIP_READY_WAIT((DEVICE_READY | DEVICE_OPSTAT_AVAIL), CMD_DEV_STATUS);
    
    if(pIOStat->ffxStat != FFXSTAT_SUCCESS)
        return FALSE;

    pIOStat->ffxStat = SET_CHIP_COMMAND_NOWAIT(CMD_GET_NEXT_OPERATION_STATUS);
    ...
    if(pIOStat->ffxStat == FFXSTAT_SUCCESS)
    {
        ...
        */
        ...
        pIOStat->ffxStat = SET_CHIP_COMMAND_NOWAIT(CMD_OPERATION_STATUS);
        ...
        while(!DclTimerExpired(&t))
        {
            ...
            if(Status & OP_STAT_VALID)
                break;
        }

        /* Handles the multithreading case as well
        */
        Status = NAND_STATUS_IN();
        Status = BENDSWAP(Status);	  /* unsafely swaps volatile memory ptrs */
        ...
        if(!(Status & OP_STAT_VALID))
        {
            pIOStat->ffxStat = FFXSTAT_FIMTIMEOUT;
            return FALSE;
        }

        else if(Status == OP_STAT_VALID)
            return TRUE;

        /* Some thing went wrong. Chance of Block Retirement
        */
        switch(ucOperation)
        {
            case OP_READ:
                if(Status & OP_STAT_REFRESH_BLOCK)
                {
                ...
                else if(Status & OP_STAT_OPERATION_ERROR)
                {
                    if(!(Status & OP_STAT_PAGE_NOT_PROGRAMMED) &&  !(Status & OP_STAT_RETIRE_BLOCK))
                    {
                        FFXPRINTF(1, ("Read : FFXSTAT_FIMUNCORRECTABLEDATA"));
            ...
            default:
                ...
                */
                if(Status & OP_STAT_RETIRE_BLOCK)
                {
                    FFXPRINTF(1, ("Write/Erase : Indication of Retire Block"));
...
/*-------------------------------------------------------------------
    ...
    Return Value:
        ...
        NULLs.
-------------------------------------------------------------------*/
static D_BOOL ECLRNANDNtmTagDecode(
    D_BUFFER       *pDest,
    const D_BUFFER *pSrc)
{
	...
	FFXTRACEPRINTF((MAKETRACEFLAGS(FFXTRACE_NTM, 3, TRACEINDENT),
        NTMNAME"-ECLRNANDNtmTagDecode() pDest=%P pDest=%P\n",
        pDest, pSrc));
    ...
    */
    ...
    {
        FFXPRINTF(2, ("ECLRNANDNtmTagDecode() Unrecoverable tag error, Src=%02X %02X %02X %02X Corrected=%02X %02X %02X %02X\n",
    ...
    FFXTRACEPRINTF((MAKETRACEFLAGS(FFXTRACE_NTM, 3, TRACEUNDENT),
    NTMNAME"-ECLRNANDNtmTagDecode() returning\n"));
...
-------------------------------------------------------------------*/
static D_BOOL ECLRNANDNtmTagEncode(
    D_BUFFER          *pDest,
    const D_BUFFER    *pSrc)
{
    FFXTRACEPRINTF((MAKETRACEFLAGS(FFXTRACE_NTM, 3, TRACEINDENT),
...
/*-------------------------------------------------------------------
    ...
    Parameters:
        hNTM        - The NTM handle
        ulPageIndex - PageIndex inside the LUN.
        ucWaitfor   - On which bits to wait for.
    ...
    D_UCHAR         ucWaitfor)
{
    ...
    while(!DclTimerExpired(&t))
    {
        ffxStat = SET_CHIP_COMMAND_NOWAIT(CMD_LUN_STATUS);
...
#if 0
    /*  If we do not support Clear Nand products, the device has to
        be either ONFI 2.3 compliant or support EZNAND feature. If not,
        return No support.
    */
    ...
    FFXFIMBOUNDS   *pBounds)
{
    ...
    */
    ...
    /*  Currently more than one volumes are not supported. This needs
        to be acheived by using VOLUME SELECT command and related
        information later. Any refactoring for supporting more than
        one volume should go here.
    */
    nVolumes++;
    ...
    */
    ...
    {
        /*  We have the parameter page. Fill in FFXNANDCHIPCLASS and
            FFXNANDCHIP structures
        */
    ...
    unsigned        nChip)
{
    ...
    ffxStat = FfxHookNTECLRNandSetChipSelect(hNTM->pNC, nChip, MODE_READ);
    if(ffxStat == FFXSTAT_SUCCESS)
    {
        ffxStat = SET_CHIP_COMMAND_NOWAIT(CMD_RESET);
        if(ffxStat == FFXSTAT_SUCCESS)
        {
            ffxStat = CHIP_READY_WAIT(NANDSTAT_READY, CMD_STATUS);
            if(ffxStat == FFXSTAT_SUCCESS)
            {
...

{
    ...
    if(ffxStat == FFXSTAT_SUCCESS)
    {
        ...
        if(ffxStat == FFXSTAT_SUCCESS)
        {
            ...
            NAND_ADDR_OUT(0x0);
            ffxStat = SET_CHIP_COMMAND_NOWAIT(CMD_GET_NEXT_OPERATION_STATUS);          
            if(ffxStat == FFXSTAT_SUCCESS)
            {
                DCLTIMER        t;
                volatile NAND_IO_TYPE Status;
                ...
                ffxStat = SET_CHIP_COMMAND_NOWAIT(CMD_OPERATION_STATUS);          
                if(ffxStat == FFXSTAT_SUCCESS)
                {
                    DclTimerSet(&t, NAND_TIME_OUT);
                    while(!DclTimerExpired(&t))
    ...
    D_UINT32        ulAbsolutePage)
{
    ...
    if(ffxStat == FFXSTAT_SUCCESS)
    {
        if (CMD_ERASE_BLOCK_START == ucCommand)
        {            
            ucStatusCommand = OP_ERASE;
        }
        else
        {            
            ucStatusCommand = OP_WRITE;
        }

        {
            FFXIOSTATUS ioValid = INITIAL_PAGEIO_STATUS(FFXSTAT_FIMUNCORRECTABLEDATA);

            ECLRNANDCheckOperationStatus(hNTM, ucStatusCommand, ulAbsolutePage, &ioValid);
            ffxStat = ioValid.ffxStat;
        }
    }
