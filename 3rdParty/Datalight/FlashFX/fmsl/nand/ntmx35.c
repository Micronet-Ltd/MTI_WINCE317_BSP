...
/*---------------------------------------------------------------------------
    ...
    the Freescale i.MX35 and related chips.

    Currently Only native page sizes of 512 2KB and 4KB for NAND is supported.  
...
/*---------------------------------------------------------------------------
                                Revision History
    $Log: ntmx35.c $
    Revision 1.7  2012/03/05 02:18:26Z  garyp
    Minor typecast fixes to operate properly with 64-bit pointers.
    Revision 1.6  2012/02/18 01:08:59Z  garyp
    Declared some variables as static to avoid global namespace contention.
    However the better long-term fix awaits.
    Revision 1.5  2012/02/09 21:49:08Z  Glenns
    Remove unused statically declared variable.
    Revision 1.4  2011/11/29 23:41:54Z  glenns
    Fix compiler warnings about signed-unsigned mismatches.
    Revision 1.3  2011/11/22 23:03:06Z  glenns
    Fix GNU compiler warning and hungarian notation fixup.
    Revision 1.2  2011/11/22 14:36:59Z  jimmb
    Update to the ntms from GL
---------------------------------------------------------------------------*/
...
#include "nand.h"
#include "ntm.h"
#include <hardware/hwmx35.h>

#define NTMNAME "NTMX35"
#define MAX_MX35_EDC_RS 8
...
/*  Spare area Format
    ...
    by the MX35 NFC is shown below.

    HW ECC spare area format(512 page size)
     ...
     4  |     BI (1)      |                 |
        +-----------------+-----------------+
     6  | M-S ECC byte 1  | Reserved  (4)   |
        +-----------------+-----------------+
     8  | M-S ECC byte 3  | M-S ECC byte 2  |
        +-----------------+-----------------+
    10  | M-S ECC byte 5  | M-S ECC byte 4  |
        +-----------------+-----------------+
    12  | M-S ECC byte 7  | M-S ECC byte 6  |
        +-----------------------------------+
    14  | M-S ECC byte 9  | M-S ECC byte 8  |
        +-----------------------------------+

     Keep offset 5 free for bad block marker usage(Bad block Info)     
        9 Bytes of Ecc from offset 7-15
        User Area ofset 0-5
        6 th offset BI.
    -------------------------------------------------------------------
    
    If the flash is large-block NAND (2KB pages), the spare area as viewed
    by the MX35 NFC shown below.
    ...
    Keep offset 0 and 1 free for bad block marker usage.
         offset 2-6,16-22,32-38,48-54 are user Area.
         offset 7-15,23-31,39-47,55-63 are Ecc Positions(4 * 9 = 36 Bytes)

    Here's the table mention for 2K-page spare layout:

Address                 F E D C B A 9 8        7 6 5 4 3 2 1 0

0xNFC_BASE+0x1000 (SB0) Reserved for user(MSB) Reserved for user (LSB)
0xNFC_BASE+0x1002 (SB0) Reserved for user      Reserved for user
0xNFC_BASE+0x1004 (SB0) Reserved for user      Reserved for user
0xNFC_BASE+0x1006 (SB0) ECC Byte (1)           Reserved 
0xNFC_BASE+0x1008 (SB0) ECC Byte (3)           ECC Byte (2)
0xNFC_BASE+0x100A (SB0) ECC Byte (5)           ECC Byte (4)
0xNFC_BASE+0x100C (SB0) ECC Byte (7)           ECC Byte (6) 
0xNFC_BASE+0x100E (SB0) ECC Byte (9)           ECC Byte (8) 
0xNFC_BASE+0x1010-0xNFC_BASE_103F(SB0) Not in Use

This means the last 9 byte per 16 byte spare block are used for ecc.
...
#define MX35_SPARE_INDEX_TAGSTART_16         (0)  /* For 512_PAGE: offset 0,1,2,3 Are used for UserArea
                                                     offset 7, 8, 9, 10, 11, 12, 13, 14, 15 are Ecc pos 
                                                  */ 
#define MX35_SPARE_INDEX_TAGSTART_64         (48)
#define MX35_SPARE_INDEX_TAGSTART_128_218    (112)
...
struct tagNTMDATA
{
    ...
    NTMX35OPERATIONS   *pNANDOps;   /* NAND device specific operations functions  */
...
/*-------------------------------------------------------------------
    ...
    into the NTM.
-------------------------------------------------------------------*/
NANDTECHNOLOGYMODULE FFXNTM_mx35 =
{
    Create,
    Destroy,
    PageRead,
...
static NTMX35OPERATIONS NativeOps =
{
    PageReadEcc,
    PageReadNoEcc,
    PageWriteEcc,
...
static NTMX35OPERATIONS * IdentifyFlash(NTMHANDLE hNTM);
static NTMX35OPERATIONS * ConfigController(PNANDCTL pNC, const FFXNANDCHIP *pChip);
...
static void NFCSetEccMode(D_BOOL fUseEcc);
...
static D_UINT16 uEdcCapability;
static unsigned int NumofSpareBuf;
    ...
    const NTMINFO     **ppNtmInfo)
{
    ...
    if (pNTM)
    {
        ...
        pNTM->pNC = FfxHookNTMX35Create(hDev,&pNTM->Params);
        if(pNTM->pNC)
        {
            ...
            if (pNTM->pNANDOps)
            {
                ...
                */
                ...
                if( pNTM->pChipInfo->pChipClass->uEdcRequirement > MAX_MX35_EDC_RS )
                {
                    /* This chip requires more correction capabilities
                        than this NTM can handle - fail the create.
                    */
                    FFXPRINTF(1, ("NTMX35-Create() Insufficient EDC capabilities.\n"));
                    goto CreateCleanup;
                }

                /*  MX35 controller is capable of correcting 4 or 8  bit per
                    512-byte segment. Assign and verify it so:
                    In case of working with NAND devices of 512B+16B, 2K+64B
                    or 4K+128B(ie 16B of spare buffer) then, only ECC_MODE 
                    of 4 bits is allowed11 and then first 7B of relavent spare
                    buffers are reserved for user and the rest 9B are reserved
                    for ECC.
                */

                pNTM->NtmInfo.uEdcRequirement    = pNTM->pChipInfo->pChipClass->uEdcRequirement;
                pNTM->NtmInfo.uEdcCapability     = uEdcCapability; /* Default 4 */
                ...
                if(pNTM->NtmInfo.uPageSize == NAND512_PAGE)
                {
                    ...
                    pNTM->Params.nTagOffset     = MX35_SPARE_INDEX_TAGSTART_16;
                }
                else if(pNTM->NtmInfo.uPageSize == NAND2K_PAGE)
                { 
                    DclAssert((pNTM->pChipInfo->bFlags & CHIPFBB_MASK) == CHIPFBB_OFFSETZERO);
                    pNTM->Params.nTagOffset     = MX35_SPARE_INDEX_TAGSTART_64;
                }
                else if(pNTM->NtmInfo.uPageSize == NAND4K_PAGE)
                {
                    NumofSpareBuf            = SPARE_SEGMENTS;
                    pNTM->Params.nTagOffset  = MX35_SPARE_INDEX_TAGSTART_128_218;
                    if(pNTM->NtmInfo.uSpareSize > 128)
                        pNTM->NtmInfo.uEdcCapability = uEdcCapability; /* 8 */
                }
                else
                {
                    DclError();
...
static NTMX35OPERATIONS * IdentifyFlash(NTMHANDLE hNTM)
{
    ...
    */
                         ...
                         &NFC.pBase, sizeof NFC.pBase))
    {
        ...
        */
        if (FfxHookNTMX35SetParameters(hNTM->pNC, CHIPINT_8BIT, NAND512_PAGE))
        {
                ...
                */
                ...
                if (hNTM->pChipInfo)
                {
                    /*  Ensure that the flash that was found is compatible with the
                        spare area format supported by the MX35 and NTM with regard
                    ...
                    */
                    ...
                    FFXPRINTF(1, ("NTMX35:IdentifyFlash() Factory bad block marker method incompatible with this NTM.\n"));
                 }
    ...
    D_BOOL              fUseEcc)
{
    ...
    DclProfilerEnter("NTMX35-PageRead", 0, ulCount);
    ...
    D_BOOL              fUseEcc)
{
    ...
    DclProfilerEnter("NTMX35-PageWrite", 0, ulCount);
    ...
    D_UINT32            ulCount)
{
    ...
    DclProfilerEnter("NTMX35-RawPageRead", 0, ulCount);
    ...
    D_UINT32            ulCount)
{
    ...
    DclProfilerEnter("NTMX35-RawPageWrite", 0, ulCount);
...
-------------------------------------------------------------------*/
static FFXIOSTATUS SpareReadNative(
    D_UINT32 ulPhysicalPage,
    D_BUFFER *pSpare)
{
    ...
    */
    ...
    if (NFCCommandOut(CMD_READ_MAIN)
        && NFCColumnOut(MX35_PAGE_COLUMN, 2)  /* Page address of page size bytes of native page area */
        && NFCRowOut(ulPhysicalPage, NFC.uAddrsBytes) 
...
/*-------------------------------------------------------------------
    ...
    Return Value:
        ...
        uncorrectable error; timeout)
-------------------------------------------------------------------*/
static FFXIOSTATUS SpareWriteNative(
    D_UINT32 ulPhysicalPage,
    D_BUFFER *pSpare)
{
    FFXIOSTATUS status = INITIAL_PAGEIO_STATUS(FFXSTAT_FIMTIMEOUT);
    ...
    */
        ...
        && ReadStatus(&uChipStatus))
    {
        ...
        {
            FFXPRINTF(1, ("SpareWriteNative: error status %02x\n", uChipStatus));
        }
    }
...
/*-------------------------------------------------------------------
    ...
    Write a  page from large-block NAND.
    ...
    unsigned           nOffSet)
{
    ...
    */
    ...
    { 
        unsigned int uEccStatus = DCLMEMGET16 (ECC_STATUS_RESULT_1);
        /*  0000 No Error
            0001 to 1000 Indicates the number of symbor errors Correctabele
            1111 Unknown number of Errors (uncorrectabele)
        */
        if(((uEccStatus & 0xF000) > NANDFC_ECC_STATUS_RESULT_NOSER4_4SB_ERR) ||
           ((uEccStatus & 0x0F00) > NANDFC_ECC_STATUS_RESULT_NOSER3_4SB_ERR))
        {
            FFXTRACEPRINTF((MAKETRACEFLAGS(FFXTRACE_NTM, 1, 0),
                        "Uncorrectable error in tag: page %lu\n", ulPage));
            DclMemSet(pTag, 0, nTagSize);
        }
        else if (uEccStatus == ECC_STATUS_NO_ERROR )
        {
            status.ffxStat = FFXSTAT_SUCCESS;
        ...
        {
            /*  A correctable error occurred in the spare area.
                The controller has already corrected the data.
            */
            
                
            ulNumBitErrors = FfxErrMgrDecodeCorrectedBits(&status);
            ulNumBitErrors++;
            FfxErrMgrEncodeCorrectedBits(ulNumBitErrors, &status);
            status.op.ulPageStatus |= PAGESTATUS_DATACORRECTED;
            DecodeTagHwEcc(pTag, nTagSize, spare.data,nOffSet);
        }
        if(NumofSpareBuf == SPARE_SEGMENTS)
        {
             unsigned int uEccStatus = DCLMEMGET16(ECC_STATUS_RESULT_2);
                 
             if(((uEccStatus & 0xF000) > NANDFC_ECC_STATUS_RESULT_NOSER8_4SB_ERR) ||
                ((uEccStatus & 0x0F00) > NANDFC_ECC_STATUS_RESULT_NOSER7_4SB_ERR))
             {
                 FFXTRACEPRINTF((MAKETRACEFLAGS(FFXTRACE_NTM, 1, 0),"Uncorrectable error in tag.\n"));
                 DclMemSet(pTag, 0, nTagSize);
             }
             else if (uEccStatus == ECC_STATUS_NO_ERROR)
             {
    ...
    D_UINT8         bFbbType)
{
    FFXIOSTATUS     ioStat;
    DCLALIGNEDBUFFER (spare, data, SPARE_BUFFER_SIZE);
    D_BUFFER        *pNativeSpare;

    /*  All  page flash supported by this NTM uses the OFFSETZERO
    ...
    */
    ...
    do
    {
        /*  Read the spare area.
        */
        ioStat = SpareReadNative(ulPhysicalPage, spare.data);

        
        if (ioStat.ffxStat != FFXSTAT_SUCCESS)
        {
            FFXPRINTF(1, ("NTMX35:ReadBlockStatus() SpareReadNative() failed.\n"));
             break;
        }
        
        pNativeSpare = spare.data;
        ...
        /*  If there are any zero bits in the first two bytes (word offset 0),
            ...
            by the MX35 NAND flash controller), the block is bad.
            ...
            both.
        */
        if ((pNativeSpare[NSOFFSETZERO_FACTORYBAD_OFFSET] != ERASED8)
                && ((NFC.ucWidthBytes == 1) || (pNativeSpare[NSOFFSETZERO_FACTORYBAD_OFFSET + 1] != ERASED8)))
        {
    ...
    switch (ulBlockStatus & BLOCKSTATUS_MASKTYPE)
    {
        case BLOCKSTATUS_FACTORYBAD:
        {
            ...
            */
            for (i = 0; i < 2; ++i)
            {
                ioStat = SpareWriteNative(ulPhysicalPage, spare.data);
                if (ioStat.ffxStat == FFXSTAT_SUCCESS)
                    break;
    ...
    D_UINT32        ulBuffLen)
{
    ...
    switch(id)
    {
        default:
            ...
            */
            FFXPRINTF(1, ("NTMX35:ParameterGet() unhandled parameter ID=%x\n", id));
            break;
    }
    ...
    D_UINT32        ulBuffLen)
{
    ...
    switch(id)
    {
        default:
            /*  Not a parameter ID we recognize, and no place else to pass it
            */
            FFXPRINTF(1, ("NTMX35:ParameterSet() unhandled parameter ID=%x\n", id));
            break;
    }
...
/*-------------------------------------------------------------------
    ...
    Return Value:
        ...
        to operate, NULL if not.
-------------------------------------------------------------------*/
static NTMX35OPERATIONS * ConfigController(
    PNANDCTL            pNC,
    const FFXNANDCHIP  *pChip)
{
    NTMX35OPERATIONS   *pNANDOps = NULL;
    unsigned nPagesPerBlock;
    D_UINT32 ulPagesPerBlock = 0;
    unsigned uEccMode;
    ...
    NFC.uAddrsBytes =
        ...
        / 8;
    nPagesPerBlock = (pChip->pChipClass->ulBlockSize/pChip->pChipClass->uPageSize);
    if(pChip->pChipClass->uSpareSize > 128)
    {
        uEccMode       = NFC_ECC_MODE_8BIT;
        uEdcCapability = 8;
        
    }
    else
    {
       uEccMode       = NFC_ECC_MODE_4BIT;
       uEdcCapability = 4;
    }
    if(nPagesPerBlock == 32)
    {
        ulPagesPerBlock = PPB_32;
    }
    else if(nPagesPerBlock == 64)
    {
        ulPagesPerBlock = (PPB_64);
    }
    else if (nPagesPerBlock == 128)
    {
        ulPagesPerBlock = PPB_128;
    }
    else if(nPagesPerBlock == 256)
    {
        ulPagesPerBlock = PPB_256;
    }
    ...
    */
    ...
    {
        FFXPRINTF(1, ("NTMX35:ConfigController() unknown bus width.\n"));
        return NULL;
    }
        ...
        */
        ...
        /*  Disable interrupt 
        */
        DCLMEMPUT16(NAND_FLASH_CONFIG1, (DCLMEMGET16(NAND_FLASH_CONFIG1)| INT_MASK));
 
        /*  Disable spare enable
        */
        DCLMEMPUT16(NAND_FLASH_CONFIG1,(DCLMEMGET16(NAND_FLASH_CONFIG1)& ~NFC_SP_EN));
        ...
        */
        DCLMEMPUT16(NFC_CONFIGURATION,(BLS_1 | BLS));

        /*  Set up configuration bits
        */  

        DclAssert(ulPagesPerBlock);           
        NFC.uConfig1 |= (FP_INT | (ulPagesPerBlock) | NF_CE) | INT_MASK | NF_LITTLE | uEccMode | DMA_MODE;
        DCLMEMPUT16(NAND_FLASH_CONFIG1,NFC.uConfig1); 

        /*  Set spare Size
        */
        DCLMEMPUT16(SPAS,(pChip->pChipClass->uSpareSize/2));
        ...
        */
        if (NFCCommandOut(CMD_RESET))
        {
            ...
            */
           if (!FfxHookNTMX35SetParameters(pNC, NFC.ucWidthBytes, pChip->pChipClass->uPageSize))
           {
               FFXPRINTF(1, ("NTMX35:ConfigController() FfxHookNTMX35SetParameters returned FALSE.\n"));
               return NULL;
            }
        }
    return pNANDOps;
    ...
    unsigned          nOffSet)
{
    ...
    */
    if (NFCPageIn(pMain, spare.data))
    {
        ...
        */
        if(((uEccStatus & 0x000F) > NANDFC_ECC_STATUS_RESULT_NOSER1_4SB_ERR) ||
           ((uEccStatus & 0x00F0) > NANDFC_ECC_STATUS_RESULT_NOSER2_4SB_ERR) ||
           ((uEccStatus & 0x0F00) > NANDFC_ECC_STATUS_RESULT_NOSER3_4SB_ERR) ||
           ((uEccStatus & 0xF000) > NANDFC_ECC_STATUS_RESULT_NOSER4_4SB_ERR))
        {
            FFXPRINTF(1,("Uncorrectable error in Page%x\r\n",uEccStatus));
        ...
        {
            unsigned int uEccStatus = DCLMEMGET16(ECC_STATUS_RESULT_2);
            if(((uEccStatus & 0x000F) > NANDFC_ECC_STATUS_RESULT_NOSER5_4SB_ERR) ||
               ((uEccStatus & 0x00F0) > NANDFC_ECC_STATUS_RESULT_NOSER6_4SB_ERR) ||
               ((uEccStatus & 0x0F00) > NANDFC_ECC_STATUS_RESULT_NOSER7_4SB_ERR) ||
               ((uEccStatus & 0xF000) > NANDFC_ECC_STATUS_RESULT_NOSER8_4SB_ERR))
            {
                FFXPRINTF(1,("Uncorrectable error in Page%x\r\n",uEccStatus));
                pStatus->ffxStat = FFXSTAT_FIMUNCORRECTABLEDATA;
            }
...
{
    D_BOOL result = TRUE;
    static D_UINT32 aulBounce[16];
    static D_UINT32 aulBouncePage[4];
    ...
    */
    ...
    DCLMEMPUT16(NAND_FLASH_CONFIG2,0x0);
