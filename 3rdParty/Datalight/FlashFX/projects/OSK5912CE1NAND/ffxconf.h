...
/*---------------------------------------------------------------------------
    ...
    Revision 1.14  2009/04/03 05:12:30Z  keithg
    Work around for bug 2561: Removed FFXCONF_READAHEADENABLED
    Revision 1.13  2008/03/27 02:56:53Z  Garyp
    ...
    Revision 1.12  2007/11/03 23:50:35Z  Garyp
    Updated to use the standard module header.
    Revision 1.11  2007/09/13 20:28:54Z  thomd
    default MTSTRESS param /time
    Revision 1.10  2007/09/13 16:20:03Z  thomd
    Updated FFX_MTTESTPARAMS
    Revision 1.9  2007/08/03 14:12:36Z  timothyj
    Changed device and disk settings from byte offsets to units of kilobytes.
    Revision 1.8  2007/02/01 01:03:29Z  Garyp
    Added device timing and latency configuration settings.
    Revision 1.7  2006/12/20 22:26:08Z  johnb
    Updated device settings for DEMO Board and added default
    setting for FFXCONF_DRIVERAUTOFORMAT to FALSE if
    FFXCONF_FORMATSUPPORT is FALSE
    Revision 1.6  2006/10/13 01:47:34Z  Garyp
    ...
    Revision 1.5  2006/08/02 17:52:17Z  timothyj
    Removed deprecated FFX_RESERVED_SPACE macro
    Revision 1.4  2006/07/20 18:42:39Z  chrisl
    Turned off FAT and changed default to Reliance.
    Revision 1.3  2006/04/04 00:59:50Z  michaelm
    removed mention of fimlist.pdf and instead refer to website
    Revision 1.2  2006/03/10 00:06:52Z  timothyj
    Disabled automatic unit tests.
    Removed deprecated comments surrounding DCLTESTPARAMS regarding execution
    time.
    Revision 1.1  2006/03/09 22:59:34Z  timothyj
    Initial revision
...
/*-------------------------------------------------------------------
    ...
    will support.  At least one of these features must be TRUE.
    Disable flash technologies which are not being used to reduce
    the size of the device driver.
-------------------------------------------------------------------*/
...
#define FFX_DEV0_NTMS           {&FFXNTM_onenand, NULL}
#define FFX_DEV0_SETTINGS       {(D_BUFFER*)0x08000000, 0x0000400, 0, 0x0001FC00}
...
/*-----------------------------------------------------------------
    ...
    FfxHookHardwareInit() function in fhinit.c.  The resulting
    numbers can then be plugged in here to avoid the overhead of
    automatic calculation at run-time.

    FFX_DELAY_COUNT is a loop count, and FFX_DELAY_MICROSEC is
    the number of microseconds of delay that loop count
    produces. FFX_DELAY_MICROSEC must be a power of two such
    that FFX_DELAY_MICROSEC == (1UL << FFX_DELAY_SHIFT).

    Additionally, FFX_DELAY_COUNT * FFX_DELAY_MICROSEC must not
    overflow an unsigned long int.

    The sample values here represent a hypothetical system with the
    ability to execute the delay loop 300 times per microsecond.
    They are are given for purposes of illustration only and are
    unlikely to be correct for any particular system.
-----------------------------------------------------------------*/
