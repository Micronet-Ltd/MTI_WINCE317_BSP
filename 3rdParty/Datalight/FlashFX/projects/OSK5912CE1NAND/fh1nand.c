...
/*---------------------------------------------------------------------------
                                Revision History
    $Log: fh1nand.c $
    Revision 1.10  2010/02/13 18:00:49Z  garyp
    Updated to use the slightly revamped virtual memory mapping interface.
    Revision 1.9  2010/02/12 21:42:19Z  garyp
    Updated to use the slightly revamped virtual memory mapping interface.
    Revision 1.8  2009/12/11 23:02:34Z  garyp
    Minor header and structure updates.
    Revision 1.7  2009/10/14 01:23:29Z  keithg
    Removed now obsolete fxio.h include file.
    Revision 1.6  2009/07/24 01:10:00Z  garyp
    ...
    Revision 1.5  2008/09/29 19:53:57Z  glenns
    Added support for FlexOneNAND to be sure main page data is blank in the
    page register when doing spare-only writes.
    Revision 1.4  2007/11/03 23:50:34Z  Garyp
    Updated to use the standard module header.
    Revision 1.3  2006/10/04 02:33:58Z  Garyp
    Updated to use DclMemAllocZero().  Updated to use the new printf macros.
    Revision 1.2  2006/08/04 16:19:15Z  timothyj
    Changed parameter to OneNANDDumpChipState() to pONC from (obsolete)
    &OneNANDCtl
    Revision 1.1  2006/03/09 21:17:22Z  timothyj
    Initial revision
---------------------------------------------------------------------------*/
...
#if D_DEBUG
static void OneNANDDumpEMIFSState(const NTMHOOKHANDLE hHook);
#endif
...
/*-------------------------------------------------------------------
    OneNANDDumpEMIFSState() - debug-only helper function to dump
    the state of the OMAP5912's EMIFS settings for the chip
    select region to which the OneNAND device is attached.
-------------------------------------------------------------------*/
static void OneNANDDumpEMIFSState(
    NTMHOOKHANDLE       hHook)
{
    FFXSTATUS           ffxStat;
    D_UINT32           *pEMIFS;

    /*  Display the EMIFS configuration values
    */
    #define EMIFS_REGS_ADDRESS 0xFFFECC00
    #define EMIFS_REGS_LENGTH  0x100

    #define EMIFS_CCS2_OFFSET  0x06
    #define EMIFS_ACS2_OFFSET  0x16

    ffxStat = DclOsVirtualPointerMap((void**)&pEMIFS, EMIFS_REGS_ADDRESS, EMIFS_REGS_LENGTH, FALSE);
	if (ffxStat != FFXSTAT_SUCCESS)
	{
        DclError();
        return;
    ...
    FFXPRINTF(1, ("OneNANDDumpEMIFSState() CCS2: %lX  ACS2: %lX\n",
        pEMIFS[EMIFS_CCS2_OFFSET],
        pEMIFS[EMIFS_ACS2_OFFSET]));

    DclOsVirtualPointerUnmap(pEMIFS, EMIFS_REGS_ADDRESS, EMIFS_REGS_LENGTH);
}
