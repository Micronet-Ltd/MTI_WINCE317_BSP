...
/*---------------------------------------------------------------------------
    ...
    This module contains implementations of the OneNAND NTM Hooks functions.
---------------------------------------------------------------------------*/

/*---------------------------------------------------------------------------
                                Revision History
    $Log: fh1nand.c $
    Revision 1.8  2010/02/12 21:42:19Z  garyp
    Updated to use the slightly revamped virtual memory mapping interface.
    Revision 1.7  2009/12/11 23:02:55Z  garyp
    Minor header and structure updates.
    Revision 1.6  2009/10/19 23:33:35Z  garyp
    Updated to use DclCpuMemCpyAligned32_32() for writes.
    Revision 1.5  2009/10/17 23:17:07Z  garyp
    Updated to use DclCpuMemCpyAligned32_32().  Updated to use some cleaner
    looping logic.
    Revision 1.4  2009/10/14 01:23:30Z  keithg
    Removed now obsolete fxio.h include file.
    Revision 1.3  2009/07/20 17:56:43Z  garyp
    Documentation updated -- no functional changes.
    Revision 1.2  2009/03/09 04:03:46Z  garyp
    Merged from the v4.0 branch.
    Revision 1.1.1.2  2009/03/09 04:03:46Z  garyp
    Cleaned up the pipeline read code.
    Revision 1.1  2009/02/25 18:55:06Z  garyp
    Initial revision
---------------------------------------------------------------------------*/
    ...
    D_UINT16                    uCount)
{
    ...
    DclAssert(((D_UINTPTR)pSrcBuffer & 3) == 0); 
        ...
        */
        ...
        while(uCount)
        {
            ...
            *pDstByteBuffer++ = (D_UINT8)(uSrc16 >> 8);
            ...
            uCount -= sizeof(ONENAND_IO_TYPE);
        }
    }
    else
  #endif
    {
        /*  If both buffers are at least 4-byte aligned, and there is at
            least 32-bytes to copy, use our optimized ARM copy function.
        */
        if((uCount >= 32) && !((D_UINTPTR)pDstBuffer & 3)) 
        {
            D_UINT16    uChunks = uCount >> 5;  /* # of 32-byte chunks */

            /*  Copy 32-byte chunks, using buffers that are 32-bit aligned.
            */
            DclCpuMemCpyAligned32_32(pDstBuffer, (const void*)pSrcBuffer, uChunks); 

            /*  Keep only the amount remaining to copy, if any
            */
            uCount &= 32-1;
            ...
            pDstBuffer += (uChunks << 5) / sizeof(ONENAND_IO_TYPE);
            pSrcBuffer += (uChunks << 5) / sizeof(ONENAND_IO_TYPE);
        }

        /*  Copy anything due to misaligned buffers, or any trailing data
            not a part of the 32-byte chunks copied above.
        */            
        while(uCount)
        {
    ...
    D_UINT16                    uCount)
{
    ...
    DclAssert(((D_UINTPTR)pDstBuffer & 3) == 0); 
        ...
        */
      ...
      #endif
        {
            /*  If both buffers are at least 4-byte aligned, and there is at
                least 32-bytes to copy, use our optimized ARM copy function.
            */
            if((uCount >= 32) && !((D_UINTPTR)pSrcBuffer & 3)) 
            {
                D_UINT16    uChunks = uCount >> 5;  /* # of 32-byte chunks */

                /*  Copy 32-byte chunks, using buffers that are 32-bit aligned.
                */
                DclCpuMemCpyAligned32_32((void*)pDstBuffer, pSrcBuffer, uChunks); 
...
#if 0
/*-------------------------------------------------------------------
    Public: FfxHookNTOneNANDBusyWait()
    ...
    Returns:
        ...
        any failure.
-------------------------------------------------------------------*/
FFXSTATUS FfxHookNTOneNANDBusyWait(
    NTMHOOKHANDLE       hHook)
{
    DCLTIMER            t;
    ONENAND_IO_TYPE     IntStatus = 0x8000;
    FFXSTATUS           ffxStat;
    ...
    */
    ...
    /*  Loop while interrupt indication is signalled from the OneNAND chip's register
    */
    while (((IntStatus & ONENAND_REG_INTSTATUS_INT) != 0) && (!DclTimerExpired(&t)))
    {
        IntStatus = FfxHookNTOneNANDReadRegister(hHook, ONENAND_REG_INTERRUPT);
	...
	/*  If the interrupt clears (regardless of whether the timeout has
	    elapsed) then the function was successful.  Otherwise, to get here,
	    the timeout must have occurred.

        Read the register again in case the processor was busy between the
        time the interrupt cleared and the loop above was able to read it,
        and therefore time passed, but the device was indeed ready.
    */
    IntStatus = FfxHookNTOneNANDReadRegister(hHook, ONENAND_REG_INTERRUPT);

	if ((IntStatus & ONENAND_REG_INTSTATUS_INT) == 0)
 	    ffxStat = FFXSTAT_SUCCESS;
	else
	    ffxStat = FFXSTAT_FIMTIMEOUT;

    FFXTRACEPRINTF((MAKETRACEFLAGS(FFXTRACE_NTMHOOK, 3, 0),
        "FfxHookNTOneNANDBusyWait() hHook=%P returning %lX\n", hHook, ffxStat));
