...
/*---------------------------------------------------------------------------
    ...
    This module contains an implementation of an FMD-to-FlashFX interface
    for the TI SDP3430.
---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------
                                Revision History
    $Log: fmd_flashfx.c $
    Revision 1.3  2010/07/18 00:50:27Z  garyp
    Updated so the "AltUnlock" functionality is optional, and must be
    explicitly turned on.
    Revision 1.2  2010/01/11 02:58:43Z  garyp
    Initialize the AutoUnlock flag.  Documentation updated.
    Revision 1.1  2009/12/14 03:13:00Z  garyp
    Initial revision
---------------------------------------------------------------------------*/
...
#define MAGIC_RESERVED_BLOCK_COUNT  (7)     /* Fix this... */
    ...
    BLOCK_ID    blockId)
{
    ...
    if(blockId < MAGIC_RESERVED_BLOCK_COUNT)
    ...
    DWORD       status)
{
    ...
    if(blockId < MAGIC_RESERVED_BLOCK_COUNT && (status & BLOCK_STATUS_RESERVED))
        rc = TRUE;
