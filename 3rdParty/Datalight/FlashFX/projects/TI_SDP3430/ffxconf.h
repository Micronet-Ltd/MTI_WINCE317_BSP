...
/*---------------------------------------------------------------------------
                                Revision History
    $Log: ffxconf.h $
    Revision 1.3.1.2  2010/12/20 22:32:57Z  glenns
    Removed FFXCONF_SOFTLOCKSUPORT, as this feature is not
    supported in this product version.
    Revision 1.3  2010/07/05 23:32:40Z  garyp
    Added a second, commented out Device definition.
    Revision 1.2  2010/01/23 22:55:55Z  garyp
    Eliminated the FFX_DISK0_FORMAT setting.  Disk format settings
    for Windows CE are now handled entirely in the registry.
    Revision 1.1  2009/12/19 04:36:54Z  garyp
    Initial revision
---------------------------------------------------------------------------*/
...
/*  NOTE: For the IOCTL_FMD_RAW_WRITE_BLOCKS to function when a request
          is made to write to an unmapped block, a Disk is created on
          the fly.  This means that FFX_MAX_DISKS must be at least one
          value larger than the number of Disks which are defined here.
*/
#define FFX_MAX_DISKS           (4)     /* Max Disks Supported */

/*  DISK0: Read-only, system disk (no file system, no allocator support)
*/
#define FFX_DISK0_ALLOCATOR     {FFX_ALLOC_NONE}
#define FFX_DISK0_COMPACTION    {FFX_DEFAULT_COMPACTION_SETTINGS}

/*  DISK1: Application store (default settings)
*/
#define FFX_DISK1_ALLOCATOR     {FFX_DEFAULT_ALLOCATOR_SETTINGS}
#define FFX_DISK1_COMPACTION    {FFX_DEFAULT_COMPACTION_SETTINGS}

/*  DISK2: User store (default settings)
*/
#define FFX_DISK2_ALLOCATOR     {FFX_DEFAULT_ALLOCATOR_SETTINGS}
#define FFX_DISK2_COMPACTION    {FFX_DEFAULT_COMPACTION_SETTINGS}
...
/*  This must be set to FALSE, otherwise FlashFX will mistake the
    default MS format of the flash, and think all the blocks are bad.
*/
