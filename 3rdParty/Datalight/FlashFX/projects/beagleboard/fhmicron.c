...
/*---------------------------------------------------------------------------
    ...
    Micron NTM, as used on the Beagleboard.
---------------------------------------------------------------------------*/

/*---------------------------------------------------------------------------
                                Revision History
    $Log: fhmicron.c $
    Revision 1.16  2010/08/04 00:46:06Z  garyp
    Updated to use the GPMC register offset defined in hwomap35x.h.
    Revision 1.15  2010/07/16 15:31:20Z  garyp
    Updated so the hooks module provides the maximum number of chips
    count to the NTM.
    Revision 1.14  2010/07/05 20:06:03Z  garyp
    Added the function FfxHookNTMicronConfigure().  Refactored to work
    in multichip configurations.
    Revision 1.13  2010/06/25 16:43:00Z  glenns
    Add support for Micron M60 boot blocks
    Revision 1.12  2010/02/12 21:52:53Z  garyp
    Updated to use the slightly revamped virtual memory mapping interface.
    Revision 1.11  2010/01/10 22:35:40Z  garyp
    Updated to specify the alignment requirement.  Removed some
    (apparently) unnecessary code.
    Revision 1.10  2009/12/14 03:36:15Z  garyp
    Use some renamed GPMC registers.  General cleanup.
    Revision 1.9  2009/12/13 00:16:50Z  garyp
    ...
    Revision 1.8  2009/12/01 23:32:21Z  glenns
    Add fixes to enable OMAP35xx Prefetch and HW EDC simultaneously.
    Revision 1.7  2009/11/12 21:53:29Z  garyp
    Eliminated an obsolete header.
    Revision 1.6  2009/10/18 01:37:26Z  garyp
    Added another missing "static" keyword.
    Revision 1.5  2009/10/17 23:17:07Z  garyp
    Updated to use DclCpuMemCpyAligned32_32().
    Revision 1.4  2009/10/13 21:13:07Z  garyp
    Minor cleanup to some looping logic.  Added a missing "static".
    Revision 1.3  2009/10/09 00:20:32Z  garyp
    Removed unnecessary prefetch "StopEngine" commands.  Removed
    some debugging code.  Fixed a prefetch/ECC command order issue
    in FfxHookNTMicronPageWrite().
    Revision 1.2  2009/10/06 23:20:33Z  garyp
    Added the FfxHookNTMicronPageRead/Write() functions.  Added support for
    hardware ECC and prefectch mode.
    Revision 1.1  2009/09/23 23:21:32Z  garyp
    Initial revision
---------------------------------------------------------------------------*/
    ...
    FFXMICRONPARAMS    *pParams)
{
    ...
    */
    ...
    ffxStat = DclOsVirtualPointerMap((void**)&hNtmHook->pulGPMCBase, 
                                     OMAP35X_GPMC_PHYSICAL_OFFSET, 
                                     OMAP35X_GPMC_PHYSICAL_SIZE, FALSE);
    if (ffxStat != FFXSTAT_SUCCESS)
    {
        FFXPRINTF(1, ("FfxHookNTMicronCreate() Unable to map address %lX with length %lX, Status=%lX\n", 
            OMAP35X_GPMC_PHYSICAL_OFFSET, OMAP35X_GPMC_PHYSICAL_SIZE, ffxStat));
    ...
    NTMHOOKHANDLE hNtmHook)
{
    ...
    DclOsVirtualPointerUnmap((void*)hNtmHook->pulGPMCBase, OMAP35X_GPMC_PHYSICAL_OFFSET, OMAP35X_GPMC_PHYSICAL_SIZE);
    
  #if USE_PREFETCH_MODE  
    {
        unsigned    nChip;
        
        for(nChip = 0; nChip < MAX_CHIPS; nChip++)
        {
            if(hNtmHook->afFIFOInited[nChip])
                DclOsVirtualPointerUnmap((void*)hNtmHook->apFIFO[nChip], OMAP35X_GPMC_NAND_FIFOADDRESS(nChip), 0x1000); 
        }
    }
  #endif
    ...
    D_BYTE             *pabID)
{
  ...
  #if USE_PREFETCH_MODE  
    ffxStat = DclOsVirtualPointerMap((void**)&hNtmHook->apFIFO[nChip], OMAP35X_GPMC_NAND_FIFOADDRESS(nChip), 0x1000, FALSE);
    if(ffxStat != FFXSTAT_SUCCESS)
    {
        FFXPRINTF(1, ("FfxHookNTMicronConfigure() Unable to map address %lX, Status=%lX\n", 
            OMAP35X_GPMC_NAND_FIFOADDRESS(nChip), ffxStat));

        return ffxStat;
    }

    hNtmHook->afFIFOInited[nChip] = TRUE;
    ...
    D_BUFFER       *pabECC)
{
  ...
  #if USE_PREFETCH_MODE  
    {
        ...
        /*  Force the chip to NO IDLE Mode while Reads and Writes are happening.
            Otherwise the GPMC goes to idle mode and some time does n't respond 
