...
/*---------------------------------------------------------------------------
                                Revision History
    $Log: ffxconf.h $
    Revision 1.6.1.2  2010/12/20 22:53:40Z  glenns
    Removed FFXCONF_SOFTLOCKSUPORT, as this feature is not
    supported in this product version.
    Revision 1.6  2010/11/22 16:13:07Z  glenns
    Added FFXCONF_QUICKMOUNTSUPPORT option.
    Revision 1.5  2010/01/23 22:55:52Z  garyp
    Eliminated the FFX_DISK0_FORMAT setting.  Disk format settings
    for Windows CE are now handled entirely in the registry.
    Revision 1.4  2009/09/24 19:40:06Z  garyp
    Added some missing settings and tweaked a few options.
    Revision 1.3  2009/09/23 22:09:53Z  garyp
    Default to use the Micron NTM.
    Revision 1.2  2009/08/07 02:09:34Z  garyp
    Removed obsolete read-ahead stuff.
    Revision 1.1  2009/05/28 14:03:32Z  johnb
    Initial revision
---------------------------------------------------------------------------*/
...
#define FFX_DEV0_SETTINGS       {FFX_BADADDRESS, 0x00010000, 0, FFX_REMAINING}
...
#else
#define FFX_DEV0_BBM            {TRUE,FFX_FORMAT_UNFORMAT,FFX_BBMFORMAT_PRESERVE_BADBLOCKS,100}
#endif
...
#define FFX_LICENSE_ID			1366870067UL
#define FFX_ACTIVATION_KEY      601503850UL
