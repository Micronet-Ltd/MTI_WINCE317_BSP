...
/*---------------------------------------------------------------------------
                                Revision History
    $Log: flashfx_loader.h $
    Revision 1.6  2009/08/07 23:21:10Z  garyp
    ...
    Revision 1.5  2009/05/21 17:40:34Z  garyp
    Updated documentation.  Updated to use standard type names.  Cleaned
    up some code formatting.  No functional changes.
    Revision 1.4  2009/04/02 15:56:44Z  thomd
    Updated to include fxstatus.h
    Revision 1.3  2009/03/05 17:59:34Z  thomd
    Updated to initialize D_PRODUCTNUM if it is not already initialized.
    Revision 1.2  2007/11/03 23:49:38Z  Garyp
    Updated to use the standard module header.
    Revision 1.1  2007/10/10 05:07:32Z  brandont
    Initial revision
---------------------------------------------------------------------------*/

#ifndef FLASHFX_LOADER_H_INCLUDED
#define FLASHFX_LOADER_H_INCLUDED


/*-------------------------------------------------------------------
    If both flashfx_loader.h and fxapi.h are necessary, the former
    must be first.
-------------------------------------------------------------------*/
#if !defined(DL4GR_H_INCLUDED) && !defined(FLASHFX_H_INCLUDED) && defined(FXAPI_H_INCLUDED)
#error "flashfx_loader.h must be included before fxapi.h"
#endif
...
/*-------------------------------------------------------------------
    If neither dcl.h nor dcl_loader.h have already been included
...
-------------------------------------------------------------------*/
      ...
      /*  In the event that FlashFX is a sub-product, and we're being
          ...
          is one level deeper.
      */
      #include "../../dcl/include/dcl_loader.h"
    #else
...
/*
    This file cannot include the prototype for FfxReaderIoCreate()
    without requiring dlreaderio.h be included.  The prototype
    resides in the public header fxreaderio.h.
*/


#endif /* FLASHFX_LOADER_H_INCLUDED */
