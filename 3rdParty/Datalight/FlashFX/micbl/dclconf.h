...
/*---------------------------------------------------------------------------
                                Revision History
    $Log: dclconf.h $
    Revision 1.9  2009/06/27 21:04:09Z  garyp
    Added the DCL_SERVICES setting.  Updated documentation.
    Revision 1.8  2009/02/07 23:58:46Z  garyp
    Merge from the v4.0 branch.  Updated to initialize DCL_HEAP_SIZE.
    Revision 1.7  2007/12/14 23:15:40Z  Garyp
    Updated to include Datalight Shell settings.
    Revision 1.6  2007/11/03 23:31:38Z  Garyp
    Added the standard module header.
    Revision 1.5  2007/04/01 23:11:55Z  Garyp
    ...
    output.
    Revision 1.3  2006/10/26 18:09:06Z  Garyp
    ...
    Revision 1.1  2005/07/06 02:42:54Z  Pauli
    Initial revision
---------------------------------------------------------------------------*/
...
/*-------------------------------------------------------------------
    ...
    The DCL_HEAP_SIZE setting defines the number of bytes to use
    for the heap.

    Note that the heap is subject to fragmentation.  If the system
    is configured to include the various tests and tools, such as
    the Datalight Shell, this will require more memory.  Typically
    it is recommended that one start with a heap size which is
    plenty large enough, and then reduce it as needed later.
-------------------------------------------------------------------*/
#define DCL_HEAP_SIZE                   (128 * 1024UL)
...
-------------------------------------------------------------------*/
#define DCL_OSFEATURE_CONSOLEINPUT      0
#define DCLCONF_COMMAND_SHELL           0

#if DCLCONF_COMMAND_SHELL
  /*  In DEGUG mode, the shell with both TESTS and TOOLS enabled will
      be too large to fit in the standard 1MB bootloader.  Building
      in release mode, or disabling the tests make the image fit.
  */
  #define DCLCONF_SHELL_TESTS           FALSE
  #define DCLCONF_SHELL_TOOLS           0
#endif
...
#else
  /*  Typically DCLCONF_MEMORYTRACKING is TRUE for debug builds, however
      we are setting it smaller to reduce the binary size.
  */
  #define DCLCONF_MEMORYTRACKING        FALSE   /* Should be TRUE for checkin   */
...
#define DclMemCpy                   memcpy
#define DclMemCpyAligned            memcpy
#define DclMemSetAligned            memset
