...
/*---------------------------------------------------------------------------
    ...
    This module contains the default implementations of the Windows CE Boot-
    loader project hooks for the PXA320 NTM.
...
/*---------------------------------------------------------------------------
                                Revision History
    $Log: fhpxa320.c $
    Revision 1.9  2010/06/19 17:09:07Z  garyp
    Fixed to include ntmicron.h.
    Revision 1.8  2010/06/19 02:08:04Z  garyp
    Added support for Micron M60 flash, as well as Toshiba timings. 
    Revision 1.7  2010/01/10 21:41:17Z  garyp
    Updated to not use MMU related functions which may not exist.
    Revision 1.6  2009/12/11 22:53:38Z  garyp
    Per consultation and experimentation, updated the flash timing parameters.
    Revision 1.5  2009/12/02 17:53:28Z  garyp
    Refactored to provide more flexible customization capabilities at startup
    -- to better accommodate different flash types.
    Revision 1.4  2009/10/14 23:59:50Z  keithg
    Removed now obsolete fxio.h include file.
    Revision 1.3  2008/01/25 07:43:09Z  keithg
    ...
    FFXPXA320PARAMS    *pParams)
{
    ...
    /*  NOTE: Once DclOsVirtualPointerMap() is implemented properly for
              ...
              For now this code assume that the MMU is off.  If that is
              not the case, this module should be copied into the project
              directory, and modified to use the proper virtual address/

              OALPAtoVA() may be used for that, if implemented...
    */
    
/*  if(MMU is on)   
    {
        FFXPRINTF(1, ("MMU is On\n"));
        hNtmHook->pulPXA320Base = OALPAtoVA(PXA3XX_DFC_REGS_OFFSET, FALSE);
    }
    else  */
    {
        FFXPRINTF(1, ("MMU is Off\n"));
        hNtmHook->pulPXA320Base = (volatile D_UINT32*)PXA3XX_DFC_REGS_OFFSET;
    }
