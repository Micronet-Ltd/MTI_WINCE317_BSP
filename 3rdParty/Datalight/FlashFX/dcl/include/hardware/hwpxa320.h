...
/*---------------------------------------------------------------------------
                                Revision History
    $Log: hwpxa320.h $
    Revision 1.3  2009/12/12 19:12:51Z  garyp
    Added the PXA320_DFC_DEFAULT_TR0 and PXA320_DFC_DEFAULT_TR1 macros.
    Revision 1.2  2009/12/03 02:54:19Z  garyp
    Corrected to use quoted includes rather than angle brackets.
    Revision 1.1  2008/12/11 19:23:50Z  garyp
    Initial revision
---------------------------------------------------------------------------*/

#ifndef HWPXA320_H_INCLUDED
#define HWPXA320_H_INCLUDED
...
/*---------------------------------------------------------
    Timers
---------------------------------------------------------*/
#define PXA320_TICK_DIVISOR         (3250)


/*---------------------------------------------------------
    Timings are in terms of cycles of a 104 MHz clock,
    which is 13 cycles per 125 nanoseconds.  The value
...
---------------------------------------------------------*/
#define PXA320_NSEC(nsec) (((((nsec) * 13) + 124) / 125) - 1)
...
---------------------------------------------------------*/
#define PXA320_tCH_CS_WH_WP_RH_RP(tCH, tCS, tWH, tWP, tRH, tRP)     \
    ( (PXA320_NSEC(tCH) << 19)                                      \
    | (PXA320_NSEC(tCS) << 16)                                      \
    | (PXA320_NSEC(tWH) << 11)                                      \
    | (PXA320_NSEC(tWP) << 8)                                       \
...
---------------------------------------------------------*/
#define PXA320_tR_WHR_AR(tR, tWHR, tAR)                             \
    ( (PXA320_NSEC(tR) << 16)                                       \
    | (PXA320_NSEC(tWHR) << 4)                                      \
    |  PXA320_NSEC(tAR) )
...
/*---------------------------------------------------------
    Define the default (slow) timing values, sufficient 
    for ID'ing flash.
---------------------------------------------------------*/
#define PXA320_DFC_DEFAULT_TR0  (PXA320_tCH_CS_WH_WP_RH_RP(75, 75, 75, 75, 75, 75))
#define PXA320_DFC_DEFAULT_TR1  (PXA320_tR_WHR_AR(58505, 145, 145))




#endif  /* HWPXA320_H_INCLUDED */
