...
/*---------------------------------------------------------------------------
                                Revision History
    $Log: fsiotest.c $
    Revision 1.9  2009/11/02 18:28:53Z  garyp
    Enhanced the error handling logic.
    Revision 1.8  2009/06/27 22:41:49Z  garyp
    Updated to create a DCL Instance for use by the program.
    Revision 1.7  2009/02/09 06:24:52Z  garyp
    Removed a reference to an obsolete field.
    Revision 1.6  2007/11/28 22:45:08Z  Garyp
    Modified to use the new generalized CE output redirection functions.
    Revision 1.5  2007/11/03 23:31:37Z  Garyp
    Added the standard module header.
    Revision 1.4  2007/10/16 20:04:14Z  Garyp
    Fixed to use a proper printf() format string.
    Revision 1.3  2007/08/06 00:43:44Z  garyp
    Modified to always return a valid errorlevel value as defined in dlerrlev.h.
    Revision 1.2  2006/11/18 01:31:37Z  Garyp
    Updated to display an error message if the /P option is not used.
    Revision 1.1  2005/10/03 13:04:00Z  Pauli
    Initial revision
---------------------------------------------------------------------------*/
    ...
    int                     nCmdShow)
{
    ...
    if(dclStat == DCLSTAT_SUCCESS)
    {
        ...
        */
        ...
        if(strlen(achBuffer) && !strstr(achBuffer, "/?") &&
            !strstr(achBuffer, "/p:") && !strstr(achBuffer, "/P:"))
        {
            DclPrintf("ERROR: The /P:'path' option must be used to specify the\n");
            DclPrintf("       directory in which this test is to be run.\n\n");

            dclStat = DCLSTAT_BADSYNTAX;
        }
        else
        {
