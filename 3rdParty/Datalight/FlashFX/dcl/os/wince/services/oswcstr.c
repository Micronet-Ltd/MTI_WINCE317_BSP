...
/*---------------------------------------------------------------------------
                                Revision History
    $Log: oswcstr.c $
    Revision 1.7  2009/04/10 20:52:21Z  garyp
    Documentation updated -- no functional changes.
    Revision 1.6  2007/11/03 23:31:37Z  Garyp
    Added the standard module header.
    Revision 1.5  2007/08/01 00:20:45Z  Garyp
    Updated the functionality of this interface to support querying for the
    required buffer length to hold a converted string.  Changed the behavior
    to no longer make special efforts to remove the terminating NULL from the
    returned length.
    Revision 1.4  2006/02/19 01:48:28Z  Garyp
    Undid the previous change and re-enabled the return value reduction.
    Revision 1.3  2006/01/17 21:47:50Z  Garyp
    Added DclOsAnsiToWcs().  Modified the return from DclOsWcsToAnsi()
    so it is not reduced by one.
    Revision 1.2  2006/01/04 02:03:55Z  Garyp
    Updated to use some renamed settings.
    Revision 1.1  2005/10/02 03:58:22Z  Pauli
    Initial revision
---------------------------------------------------------------------------*/
    ...
    int             iLen)
{
    ...
    return WideCharToMultiByte(CP_ACP, 0, pUnicodeStr, iLen, pOutBuff, nOutBuffLen, NULL, NULL);
}
    ...
    int             iLen)
{
    ...
    return MultiByteToWideChar(CP_ACP, 0, pAnsiStr, iLen, pOutBuff, nOutBuffLen);
}
