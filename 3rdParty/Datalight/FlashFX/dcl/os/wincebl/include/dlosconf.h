...
/*---------------------------------------------------------------------------
    ...
    of the WinCE Boot Loader OS interface.
---------------------------------------------------------------------------*/

/*---------------------------------------------------------------------------
                                Revision History
    $Log: dlosconf.h $
    Revision 1.17  2009/11/30 17:01:44Z  johnb
    Added length parameter (_len) to DclMemCmp() definition
    Revision 1.16  2009/11/18 01:37:43Z  garyp
    Limit the number of default services for this OS abstraction.
    Revision 1.15  2009/06/27 21:08:18Z  garyp
    Added the DCL_OS_DEFAULT_SERVICES setting.
    Revision 1.14  2009/05/23 17:29:04Z  garyp
    Documentation updates -- no functional changes.
    Revision 1.13  2009/02/12 20:27:52Z  garyp
    Minor cleanup of how the OS requestor characteristics are defined.
    Revision 1.12  2008/08/19 21:29:56Z  keithg
    ...
    Revision 1.11  2008/05/29 20:18:58Z  garyp
    Merged from the WinMobile branch.
    Revision 1.10.1.2  2008/05/29 20:18:58Z  garyp
    Removed an unnecessary include.  Fixed to properly condition whether
    DclMemCpy() is defined or not.
    Revision 1.10  2007/11/03 23:31:38Z  Garyp
    Added the standard module header.
    Revision 1.9  2007/07/31 00:43:02Z  Garyp
    Added settings for configuring the external requestor interface.
    Revision 1.8  2007/03/06 03:23:06Z  Garyp
    Removed the DCL_OUTPUTBUFFERLEN setting, which is now configured
    in dclconf.h as DCLOUTPUT_BUFFERLEN.
    Revision 1.7  2006/12/19 19:45:27Z  timothyj
    Added DCLTHREADID definition necessary for compilation
    (threads are unused in the WINCEBL environment).
    Revision 1.6  2006/12/11 21:48:53Z  Garyp
    Added the DCL_OSFEATURE_CONSOLEINPUT setting.
    Revision 1.5  2006/08/18 19:25:38Z  Garyp
    ...
    value.
    Revision 1.4  2006/08/07 23:58:33Z  Garyp
    Modified DCL_ALIGNTYPE/SIZE to default to 8 if the system supports native
    64-bit types, and 4 otherwise.
    Revision 1.3  2006/08/04 23:39:34Z  timothyj
    Enabled mapping of DclMemCpy() to the DCL version (rather than the EBOOT
    library provided memcpy, which does not handle unaligned pointers on at
    least some ARM platforms)
    Revision 1.2  2006/01/02 08:31:20Z  Garyp
    Renamed several settings for consistency.  Straightened up the docs.
    Revision 1.1  2005/10/22 05:34:28Z  Pauli
    Initial revision
---------------------------------------------------------------------------*/
...
#ifndef DclMemCmp
#define DclMemCmp(_ptr1, _ptr2, _len)       memcmp(_ptr1, _ptr2, _len)
#endif

/*  Some versions of memcpy in FULLLIBC.LIB don't handle alignment.
    Use our internal version instead.
*/
#ifndef DclMemCpy
...
-------------------------------------------------------------------*/
#define DCL_OSTARGET_WINCEBL            (1)
#define DCL_OSNAME                      "Windows CE Bootloader"

#define DCL_OSFEATURE_UNICODE           FALSE
#define DCL_OSFEATURE_THREADS           FALSE
#ifndef DCL_OSFEATURE_CONSOLEINPUT
#define DCL_OSFEATURE_CONSOLEINPUT      FALSE
#endif
...
-------------------------------------------------------------------*/
#define DCL_OS_DEFAULT_SERVICES     DclOutputServiceInit,       \
