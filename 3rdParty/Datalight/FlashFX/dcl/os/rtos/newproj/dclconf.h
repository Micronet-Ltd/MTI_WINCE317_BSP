...
/*---------------------------------------------------------------------------
                                Revision History
    $Log: dclconf.h $
    Revision 1.9  2009/06/27 21:05:14Z  garyp
    Added the DCL_SERVICES setting.  Updated documentation.
    Revision 1.8  2009/02/07 23:58:45Z  garyp
    Merge from the v4.0 branch.  Updated to initialize DCL_HEAP_SIZE.
    Revision 1.7  2007/11/03 23:31:29Z  Garyp
    Added the standard module header.
    Revision 1.6  2007/05/21 18:27:31Z  Garyp
    Grammar fixes.
    Revision 1.5  2007/04/01 23:12:43Z  Garyp
    Added DCLCONF_MUTEXTRACKING.
    Revision 1.4  2007/03/06 21:41:17Z  Garyp
    Updated to use refactored and more flexible options for configuring debug
    output.
    Revision 1.3  2006/10/26 22:52:49Z  Garyp
    Added newly standardized output, shell, and timestamp settings.
    Revision 1.2  2005/12/08 20:43:25Z  Garyp
    Added standardized DCL settings.
    Revision 1.1  2005/07/06 02:42:52Z  Pauli
    Initial revision
---------------------------------------------------------------------------*/
...
-------------------------------------------------------------------*/
#define DCL_HEAP_SIZE                   (256 * 1024UL)


/*-------------------------------------------------------------------
    Tick Characteristics

    Define the timer characteristics for this particular hardware.

    DCL_TICK_RESOLUTION must be the number of microseconds per
    timer tick.

    DCL_TICK_MODULUS must be the number of possible tick values.
    If the tick counter wraps at 0xFFFFFFFF, the tick modulus must
    be zero.
-------------------------------------------------------------------*/
#define DCL_TICK_RESOLUTION    (0UL)
#define DCL_TICK_MODULUS       (0UL)
