...
/*---------------------------------------------------------------------------
                                Revision History
    $Log: dlsizetoulkb.c $
    Revision 1.1  2009/09/10 21:30:52Z  keithg
    Initial revision
---------------------------------------------------------------------------*/
...
/*-------------------------------------------------------------------
    ...
    *If there is no postfix, KB is assumed!*
    ...
    Since the KB value which is returned is 32-bits wide, the maximum
    effective return value is the equivalent to 4 terabytes - 1024.
    ...
    D_UINT32   *pulResult)
{
    ...
    */
    ...
    if(pachNum)
    {
        if((DclToUpper(*pachNum) == 'G') && (DclToUpper(*(pachNum+1)) == 'B'))
        {
            pachNum += 2;
        ...
        {
            /*  "GB" or "MB" were not specified, so it must be "KB"
            */
