...
/*---------------------------------------------------------------------------
    ...
    Provides Unicode common and simple case folding data!
    This file's data contents created from:
        # CaseFolding-5.1.0.txt
        # Date: 2008-03-03, 21:57:14 GMT [MD]
        # Unicode Character Database
        # Copyright (c) 1991-2008 Unicode, Inc.

    Original data parsed with the following SED expresion:
    s/^\([0-9A-F]*\); \([CS]\); \([0-9A-F]*\); # \([A-Z ]*\)/{ 0x\1, '\2', 0x\3 }, \/\* \4 \*\//p

---------------------------------------------------------------------------*/

/*---------------------------------------------------------------------------
                                Revision History
    $Log: dltutf_casedata.c $
    Revision 1.4  2009/05/26 15:56:04Z  keithg
    Moved dltutf_casedata.h into the public include tree.
    Revision 1.3  2009/05/15 09:49:13Z  brandont
    Corrected nested comments.
    Revision 1.2  2009/05/15 01:14:15Z  keithg
    Added negative cases for UTF case folding.
    Revision 1.1  2009/05/14 17:17:28Z  keithg
    Initial revision
---------------------------------------------------------------------------*/
...
const struct _tagUnicodeCaseData gcasUnicodeCaseTestData[] =
{
    ...
    { 0x00000, '\0', 0x00000 }  /* END OF LIST MARKER */
};


/*  The following list represents those scalars that would
    be mapped to a longer sequence if full case folding is
    implemented.
*/
const D_UINT32 gcaulUnicodeFullCaseChars[] =
{
