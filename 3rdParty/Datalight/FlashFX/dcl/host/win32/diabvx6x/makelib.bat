...
REM $Log: makelib.bat $
REM Revision 1.1  2005/04/24 16:25:16Z  Pauli
REM Initial revision
REM Revision 1.1  2005/04/24 17:25:16Z  Garyp
REM Initial revision
REM Revision 1.1  2005/04/24 17:25:16Z  pauli
REM Initial revision
REM Revision 1.1  2005/04/24 17:25:16Z  GaryP
REM Initial revision
REM -------------------------------------------------------------------------
...
if "%1" == "" goto Cleanup
for %%a in (%1) do call %B_LIB% -r %0 %%a

if "%2" == "" goto Cleanup
for %%a in (%2) do call %B_LIB% -r %0 %%a

if "%3" == "" goto Cleanup
for %%a in (%3) do call %B_LIB% -r %0 %%a

if "%4" == "" goto Cleanup
for %%a in (%4) do call %B_LIB% -r %0 %%a

if "%5" == "" goto Cleanup
for %%a in (%5) do call %B_LIB% -r %0 %%a

if "%6" == "" goto Cleanup
for %%a in (%6) do call %B_LIB% -r %0 %%a

if "%7" == "" goto Cleanup
for %%a in (%7) do call %B_LIB% -r %0 %%a

if "%8" == "" goto Cleanup
for %%a in (%8) do call %B_LIB% -r %0 %%a

if "%9" == "" goto Cleanup
for %%a in (%9) do call %B_LIB% -r %0 %%a
