@echo off

REM -------------------------------------------------------------------------
REM                             Description
REM
REM This batch file is used to validate the CPU type and set the P_CPUFAMILY
REM value for a ToolSet.  The first and only parameter is the CPU type.  On
REM exit, the P_CPUFAMILY value will be set to the CPU family name, or NULL
REM if the CPU type is not valid for the ToolSet.
REM
REM If the CPU type is not valid, the batch file should display a brief list
REM of valid CPU types.
REM -------------------------------------------------------------------------

REM -------------------------------------------------------------------------
REM                             Revision History
REM $Log: cpucheck.bat $
REM Revision 1.2  2007/05/02 21:10:44Z  Garyp
REM Minor documentation and message cleanup.
REM Revision 1.1  2005/12/01 01:49:00Z  Pauli
REM Initial revision
REM Revision 1.2  2005/12/01 01:48:59Z  Pauli
REM Merge with 2.0 product line, build 173.
REM Revision 1.3  2005/11/09 17:04:10Z  Pauli
REM Changed PAUSE to B_PAUSE for consistency with the build process.
REM Revision 1.2  2005/11/09 16:37:51Z  Pauli
REM Updated to correctly adhere to the "/nopause" option.
REM Revision 1.1  2005/01/09 20:58:22Z  GaryP
REM Initial revision
REM -------------------------------------------------------------------------

@echo %ECHO%

set P_CPUFAMILY=
if "%1" == "xscale" set P_CPUFAMILY=arm
if "%1" == "arm9" set P_CPUFAMILY=arm9
if "%1" == "arm926ejs" set P_CPUFAMILY=arm9
if not "%P_CPUFAMILY%" == "" goto Fini

echo.
echo cpucheck.bat:  This ToolSet does not recognize the CPU type "%1"
echo.
echo The following CPU types are supported:  "xscale", "arm9", "arm926ejs"
echo.
%B_PAUSE%

:Fini
