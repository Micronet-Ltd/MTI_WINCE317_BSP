...
/*---------------------------------------------------------------------------
    ...
    This module contains the low level Media Manager code to support ISWF
    flash.
---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------
                                Revision History
    $Log: mmiswf.c $
    Revision 1.28  2009/08/04 01:02:13Z  garyp
    Merged from the v4.0 branch.  Minor datatype changes.  Documentation
    updated -- no functional changes.
    Revision 1.27  2009/04/01 01:36:28Z  keithg
    Fixed spelling error in function header.
    Revision 1.26  2009/03/31 16:32:17Z  davidh
    Function headers updated for AutoDoc
    Revision 1.25  2009/01/27 03:14:22Z  billr
    ...
    Revision 1.24  2009/01/12 17:57:30Z  billr
    Fix compiler warnings.
    Revision 1.23  2009/01/08 18:49:40Z  jimmb
    Supplied patch from BillR 2009-01-08
    Revision 1.22  2009/01/07 01:18:23Z  billr
    FfxMMISWFReadHeader() now returns a full FFXIOSTATUS.
    Revision 1.21  2008/06/03 19:43:58Z  thomd
    Added fFormatNoErase
    Revision 1.20  2008/03/17 18:14:50Z  Garyp
    Made allocator/VBF code conditional on FFXCONF_ALLOCATORSUPPORT and
    FFXCONF_VBFSUPPORT, respectively.
    Revision 1.19  2008/01/30 22:33:52Z  Garyp
    Updated to use VBF_EUH_BUFFER_SIZE rather than the misused BLOCK_SIZE_MIN.
    Revision 1.18  2008/01/13 07:20:39Z  keithg
    Function header updates to support autodoc.
    Revision 1.17  2007/11/05 19:14:23Z  Garyp
    Corrected the debug trace messages -- no functional changes.
    Revision 1.16  2007/11/02 02:26:03Z  Garyp
    Eliminated the inclusion of limits.h.
    Revision 1.15  2007/11/01 01:00:06Z  Garyp
    ...
    Revision 1.14  2007/10/08 19:54:51Z  pauli
    Removed DataBlockAddress() which was only needed to support using the old
    FML interface which was removed in the last revision.
    Revision 1.13  2007/10/05 20:15:13Z  Garyp
    Updated to use the modern FML interface.  Added trace instrumentation.
    Revision 1.12  2007/07/13 22:42:36Z  timothyj
    Minor comments.
    Revision 1.11  2007/04/19 23:58:05Z  timothyj
    Added fLargeAddressing parameter, to indicate whether the on-media format
    should use large flash array compatible addressing (Always false for ISWF).
    Revision 1.10  2007/04/11 22:52:54Z  timothyj
    Moved DataBlockAddress() function that became ISWF-specific (obsolete for
    NAND) after large flash array changes into this module, from mediamgr.c.
    Revision 1.9  2006/10/06 00:57:42Z  Garyp
    Modified to no longer use FfxFmlOldErase().
    Revision 1.8  2006/05/08 08:51:45Z  Garyp
    Eliminated partitionStartUnit from the EUH (now a reserved field).  Updated
    to use the new write interruption test interfaces.
    Revision 1.7  2006/03/05 01:57:21Z  Garyp
    Modified the usage of FfxFmlDiskInfo() and FfxFmlDeviceInfo().
    Revision 1.6  2006/02/26 02:16:37Z  Garyp
    Split out the NOR and NAND tag handling logic.
    Revision 1.5  2006/02/21 02:53:10Z  Garyp
    ...
    Revision 1.4  2006/02/16 00:15:59Z  Garyp
    Fixed math issues in ReadHeader.
    Revision 1.3  2006/02/10 23:30:30Z  Garyp
    Updated to use new FML interface.
    Revision 1.2  2006/01/13 18:11:25Z  billr
    Fix debug build.
    Revision 1.1  2006/01/11 22:46:54Z  billr
    Initial revision
    Revision 1.5  2005/11/14 18:34:41Z  billr
    Fix compiler warnings in some environments.
    Revision 1.4  2005/09/01 17:14:20Z  billr
    Replace TABs with spaces.
    Revision 1.3  2005/08/31 22:46:18Z  Cheryl
    use achSig instead of VBF_SIGNATURE
    Revision 1.2  2005/08/30 20:29:23Z  Cheryl
    Update ISWF_ALLOC_BUFFER_DIM to use control page size for ISWF
    Revision 1.1  2005/08/27 20:36:46Z  Cheryl
    Initial revision
---------------------------------------------------------------------------*/
...
static D_BOOL   IsValidISWFAlloc(const FFXNORTAG alloc);
...
/*-------------------------------------------------------------------
    ...
    Read data from an allocation unit in ISWF flash.  (Allocation
...
/*-------------------------------------------------------------------
    ...
    Writes data to a unit in ISWF flash. (Allocation units must
...
{
    FFXIOSTATUS             ioStat = { 0, FFXSTAT_BADPARAMETER };
...
/*-------------------------------------------------------------------
    ...
    ISWF flash.  Allocation entries are read, validated, and
        ...
        */
        ...
        for(ii = 0; ii < nCount; ++ii)
        {
            ...
            */
            ...
            else if(IsValidISWFAlloc(buffer.alloc[ii]))
            {
...
/*-------------------------------------------------------------------
    ...
    Mark allocations discarded on ISWF flash.  This is done by
    ...
    Doing so may violate the restriction on some ISWF flash parts
    ...
    */
    while(nAllocCount)
    {
        ...
        */
        ...
        /*  Calculate the address each time since it might cross a
            page boundary.
        */
...
static D_UINT32 AllocEntryAddress(
    ...
    unsigned            nBlock)
{
    D_UINT32            ulStartAllocBlock = 0;
    D_UINT32            ulAllocOffset = 0;
    D_UINT32            ulLnuAddress = ~(D_UINT32) 0;
    D_UINT32            ulControlOffsetMask = 0;

    /*  Allocation entries will require control mode reading/writing
    */
    /*  ISWF FMLREAD/WRITE_CONTROLDATA for control mode require
        an address that has an offset range of 0 - 1/2 page size
    */

    DclAssert(pVBF);
    DclAssert(pVBF->hMM->uPageSize == 0
        || pVBF->uAllocationBlockSize % pVBF->hMM->uPageSize == 0L);

    /*  control mode offset mask
    */
    ulControlOffsetMask = (pVBF->hMM->uPageSize >> 1) - 1;

    /*  Linear byte address of start of the allocations
        (starting block of unit plus 1 for EUH)
    */
    ulStartAllocBlock = (((lnu * pVBF->uTotalBlocksPerUnit) + 1)
                         * pVBF->hMM->uPageSize);

    /*  offset of entry
    */
    ulAllocOffset = nBlock * FFX_NOR_TAGSIZE;

    /*  starting address +
        + page if sitting on page boundary +
        offset in range
    */
    ulLnuAddress = (ulStartAllocBlock
                + ((ulAllocOffset & ~ulControlOffsetMask) << 1)
                + (ulAllocOffset & ulControlOffsetMask));
...
/*-------------------------------------------------------------------
    Local: IsValidISWFAlloc()

    Tests an image of an allocation entry read from the media to
    see whether it is a valid ISWF allocation entry.
    ...
    Return Value:
        ...
        otherwise.
-------------------------------------------------------------------*/
static D_BOOL IsValidISWFAlloc(
...
-------------------------------------------------------------------*/
FFXIOSTATUS FfxMMISWFReadHeader(
    FFXFMLHANDLE        hFML,
    D_UINT32            ulPage,
    EUH                *pEUH)
{
    ...
    /*  These addresses are into the EUH block.  Address is ok, reads
        are smaller than uPageSize/2 no reason to fix up.
    */
...
/*-------------------------------------------------------------------
    Protected: FfxMMISWFFormatEraseUnit()

    Format a ISWF erase unit by erasing it and writing a new EUH.
    ...
    Return Value:
        ...
        operation.
-------------------------------------------------------------------*/
FFXSTATUS FfxMMISWFFormatEraseUnit(
    FFXFMLHANDLE        hFML,
    ...
    D_BOOL              fFormatNoErase)
{
    ...
    */
    ...
    /*  These addresses are reads/writes into EUH, reads are small thus no need to
        fix up control address
    */
...
/*-------------------------------------------------------------------
    Protected: FfxMMISWFCalcEUHDataUnitBlocks()
    ...
    Return Value:
        ...
        On ISWF, space is also needed for an allocation entry
        corresponding to each data block.  The allocation entry
        pages are in control mode thus are 1/2 the block size
        which is in object mode.  After accounting for the EUH,
        divide the available space by the sum of the block size
        and the allocation entry size to find out how many can
        be accommodated.
-------------------------------------------------------------------*/
unsigned FfxMMISWFCalcEUHDataUnitBlocks(
    const EUH  *pEUH)
{
    DclAssert(pEUH);

    /*  Must account for allocation entries being in control mode.
        The allocation block size is 1/2 block size
    */
    return((unsigned)(((D_UINT32)(pEUH->uUnitTotalBlocks - 1) * (pEUH->uAllocBlockSize>>1))
                    / ( (pEUH->uAllocBlockSize>>1) + FFX_NOR_TAGSIZE)));
}


/*-------------------------------------------------------------------
    Protected: FfxMMISWFDeviceInit()
